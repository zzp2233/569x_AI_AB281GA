...
void          nopoll_conn_shutdown (noPollConn * conn)
{
    ...
    nopoll_log (conn->ctx, NOPOLL_LEVEL_CRITICAL, "shutting down connection id=%d (session: %d, role: %s)",
...
#define SHA_BUF_SZ  20
    ...
    tiny_sha1_update( &sha1_ctx, (uint8_t *)accept_key, strlen (accept_key) );
...
read_pending_header:
    ...
    if (msg->payload_size == 0)
    {
        ...
        nopoll_log (conn->ctx, NOPOLL_LEVEL_CRITICAL, "Found incoming frame with payload size 0, shutting down id=%d the connection", conn->id);
        nopoll_msg_unref (msg);
        nopoll_conn_shutdown (conn);
        return NULL;
    } /* end if */
...
read_payload:
    ...
    if (msg->op_code == NOPOLL_CLOSE_FRAME)
    {
        ...
        {
            nopoll_log (conn->ctx, NOPOLL_LEVEL_CRITICAL, "Close frame received id=%d with content bytes=%d, peer status=%d, peer reason=%s, reading reason..",
        ...
        int          timeout)
{
    ...
    while (! nopoll_conn_is_ready (conn) && total_timeout > 0)
    {
        ...
        total_timeout = total_timeout - 5000;
    } /* end if */
