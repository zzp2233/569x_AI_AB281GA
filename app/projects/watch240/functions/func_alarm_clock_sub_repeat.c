...
{
    {.str_idx = STR_SELECT_ALL,  .item_mode = COMPO_LISTBOX_ITEM_MODE_NORMAL,},
    {.str_idx = STR_SUNDAY,     .item_mode = COMPO_LISTBOX_ITEM_MODE_SWITCH, .vidx = SYS_CTL_ACLOCK_SUN},
    {.str_idx = STR_MONDAY,     .item_mode = COMPO_LISTBOX_ITEM_MODE_SWITCH, .vidx = SYS_CTL_ACLOCK_MON},
    {.str_idx = STR_TUESDAY,    .item_mode = COMPO_LISTBOX_ITEM_MODE_SWITCH, .vidx = SYS_CTL_ACLOCK_TUE},
    {.str_idx = STR_WEDNESDAY,  .item_mode = COMPO_LISTBOX_ITEM_MODE_SWITCH, .vidx = SYS_CTL_ACLOCK_WED},
    {.str_idx = STR_THURSDAY,   .item_mode = COMPO_LISTBOX_ITEM_MODE_SWITCH, .vidx = SYS_CTL_ACLOCK_THU},
    {.str_idx = STR_FRIDAY,     .item_mode = COMPO_LISTBOX_ITEM_MODE_SWITCH, .vidx = SYS_CTL_ACLOCK_FRI},
    {.str_idx = STR_SATURDAY,   .item_mode = COMPO_LISTBOX_ITEM_MODE_SWITCH, .vidx = SYS_CTL_ACLOCK_SAT},
};
#else
static const compo_listbox_item_t tbl_weeks_list[] =
{
...
#define SLEF_LIST_HEI_OFFSET  30
    ...
    //新建按钮
    compo_button_t *btn;
    btn = compo_button_create_page_by_image(frm,listbox->page, UI_BUF_I332001_PUBLIC_RECTANGLE02_BIN);
    compo_setid(btn, COMPO_ID_BTN_REPETAT_YES);
    compo_button_set_pos(btn, GUI_SCREEN_CENTER_X,last_y);
    //确定文本
    compo_textbox_t* yes_txt = compo_textbox_create_for_page(frm,listbox->page, strlen(i18n[STR_OK]));
...
#define SLEF_LIST_HEI_OFFSET  30
    //新建列表
    compo_listbox_t *listbox = compo_listbox_create(frm, COMPO_LISTBOX_STYLE_TITLE_NORMAL);
    compo_listbox_set_bgimg(listbox, UI_BUF_I335001_20_ALARM_CLOCK_6_QUANTITY_LIMIT_REMINDER_ICON_SPACER_208X67_X16_Y46_BIN);
    compo_listbox_set(listbox, tbl_weeks_list, WEEKS_LIST_CNT);


    compo_setid(listbox, COMPO_ID_LISTBOX);
    compo_listbox_set_sta_icon(listbox, UI_BUF_I335001_20_ALARM_CLOCK_4_3_SETTING_TIME_ICON_CHECKED_22X17_X202_Y71_Y138_Y205_Y272_Y339_Y406_Y473_00_BIN, 0);
    compo_listbox_set_bithook(listbox, bsp_sys_get_ctlbit);
    compo_listbox_set_focus_byidx(listbox, 1);
    ...
    //新建按钮
    compo_button_t *btn;
    btn = compo_button_create_page_by_image(frm,listbox->page, UI_BUF_I335001_20_ALARM_CLOCK_6_QUANTITY_LIMIT_REMINDER_ICON_YES_208X52_X16_Y222_BIN);
    compo_setid(btn, COMPO_ID_BTN_REPETAT_YES);
    compo_button_set_pos(btn, GUI_SCREEN_CENTER_X,last_y);
    for (int i=0; i<7; i++)         //获取当前闹钟设置的星期
    {
...
compo_form_t *func_alarm_clock_sub_repeat_form_create(void)
{
    ...
    listbox->item_height = 103;
    compo_listbox_set(listbox, tbl_weeks_list, WEEKS_LIST_CNT);

    for (u8 i=0; i<LISTBOX_ITEM_CNT; i++)
    {
        widget_set_pos(listbox->item_bgimg[i],0, listbox->item_height - 1);
    ...
    u32 res_addr = (ALARM_GET_CYCLE(sys_cb.alarm_edit_idx) == 0x7f) ? UI_BUF_I341001_20_ALARM_CLOCK_OPEN_BIN : UI_BUF_I341001_20_ALARM_CLOCK_GRAY_BUTTON_BIN;
    everyday_switch = (ALARM_GET_CYCLE(sys_cb.alarm_edit_idx) == 0x7f) ? true :false;
    compo_picturebox_t *pic = compo_picturebox_create_for_page(frm, listbox->page, res_addr);
    compo_picturebox_set_pos(pic, 286 + 58 / 2, 112/2);
    compo_setid(pic, COMPO_ID_PIC_SWITCH);
    ...
    compo_setid(btn, COMPO_ID_BTN_REPETAT_YES);
    compo_button_set_pos(btn, GUI_SCREEN_CENTER_X,last_y);
    for (int i=0; i<WEEKS_LIST_CNT -1; i++)         //获取当前闹钟设置的星期
    {
        bsp_sys_set_ctlbit(SYS_CTL_ACLOCK_SUN + i,(ALARM_GET_CYCLE(sys_cb.alarm_edit_idx) >> i) & 1);
    }
...
static void func_alarm_clock_sub_repeat_button_click(void)
{
    ...
    f_alarm_clock_sub_repeat_t *f_aclock = (f_alarm_clock_sub_repeat_t *)func_cb.f_cb;
    compo_listbox_t *listbox = f_aclock->listbox;
    compo_picturebox_t *pic = compo_getobj_byid(COMPO_ID_PIC_SWITCH);
    week_idx = compo_listbox_select(listbox, ctp_get_sxy());
    if (week_idx >= 0)
    {
        if (week_idx == 0)
        {
            if(everyday_switch)
            {
                ...
                for (int i=0; i<WEEKS_LIST_CNT -1; i++)         //获取当前闹钟设置的星期
                {
                    bsp_sys_set_ctlbit(SYS_CTL_ACLOCK_SUN + i,1);
                }
            ...
            {
                everyday_switch = true;
                for (int i=0; i<WEEKS_LIST_CNT -1; i++)         //获取当前闹钟设置的星期
                {
                    bsp_sys_set_ctlbit(SYS_CTL_ACLOCK_SUN + i,0);
                }
            }
            for (int i= 1; i<WEEKS_LIST_CNT; i++)         //获取当前闹钟设置的星期
            {
                bsp_sys_reverse_ctlbit(tbl_weeks_list[i].vidx);
            }
        }
        else
        {
            bsp_sys_reverse_ctlbit(tbl_weeks_list[week_idx].vidx);
            u8 index = 0;
            for(u8 i = 1; i<WEEKS_LIST_CNT; i++)
            {
                index = index + bsp_sys_get_ctlbit(tbl_weeks_list[i].vidx);
            }
            everyday_switch = (index == 7) ? true : false;
        }
        compo_listbox_update(listbox);
        u32 res_add = everyday_switch ? UI_BUF_I341001_20_ALARM_CLOCK_OPEN_BIN : UI_BUF_I341001_20_ALARM_CLOCK_GRAY_BUTTON_BIN;
        compo_picturebox_set(pic, res_add);
    }
    ...
    switch (id)
    {
        ...
        case COMPO_ID_BTN_REPETAT_YES:
            ...
            for (int i=0; i<7; i++)
            {
                sys_cb.alarm_edit_cycle |= (bsp_sys_get_ctlbit(SYS_CTL_ACLOCK_SUN + i) << i);
            }
//        printf("%s-->alarm_edit_cycle[0x%x]:%x\n", __func__, sys_cb.alarm_edit_idx, sys_cb.alarm_edit_cycle);
...
compo_form_t *func_alarm_clock_sub_repeat_form_create(void)
{
    ...
    //新建按钮
    compo_button_t *btn;
    btn = compo_button_create_page_by_image(frm,listbox->page, UI_BUF_I343001_COMM_OK_BIN);
    compo_setid(btn, COMPO_ID_BTN_REPETAT_YES);
    compo_button_set_pos(btn, GUI_SCREEN_CENTER_X,last_y);
    for (int i=0; i<7; i++)         //获取当前闹钟设置的星期
    {
...
#define SLEF_LIST_HEI_OFFSET  30
    ...
    compo_setid(listbox, COMPO_ID_LISTBOX);
    compo_listbox_set_sta_icon(listbox, UI_BUF_I338001_1_START_LANGUAGE_OK_BIN, 0);
    compo_listbox_set_bithook(listbox, bsp_sys_get_ctlbit);
    compo_listbox_set_focus_byidx(listbox, 1);
    ...
    //新建按钮
    compo_button_t *btn;
    btn = compo_button_create_page_by_image(frm,listbox->page, UI_BUF_I338001_20_ALARM_CLOCK_COMFIRM_BIN);
    compo_setid(btn, COMPO_ID_BTN_REPETAT_YES);
    compo_button_set_location(btn, GUI_SCREEN_CENTER_X,last_y,80,80);
    for (int i=0; i<7; i++)         //获取当前闹钟设置的星期
    {
        bsp_sys_set_ctlbit(SYS_CTL_ACLOCK_MON + i,(ALARM_GET_CYCLE(sys_cb.alarm_edit_idx) >> i) & 1);
    }
...
#define SLEF_LIST_HEI_OFFSET  30
    ...
    compo_setid(listbox, COMPO_ID_LISTBOX);
    compo_listbox_set_sta_icon(listbox, UI_BUF_I340001_ALARM_SELECT01_BIN, UI_BUF_I340001_ALARM_SELECT00_BIN);
    compo_listbox_set_bithook(listbox, bsp_sys_get_ctlbit);
    compo_listbox_set_focus(listbox, 80);
    ...
    //新建按钮
    compo_button_t *btn;
    btn = compo_button_create_page_by_image(frm,listbox->page, UI_BUF_I340001_PUBLIC_RECTANGLE02_BIN);
    compo_setid(btn, COMPO_ID_BTN_REPETAT_YES);
    compo_button_set_pos(btn, GUI_SCREEN_CENTER_X,last_y);
    //确定文本
    compo_textbox_t* yes_txt = compo_textbox_create_for_page(frm,listbox->page, strlen(i18n[STR_OK]));
    compo_textbox_set_location(yes_txt, GUI_SCREEN_CENTER_X, last_y,
                               gui_image_get_size(UI_BUF_I340001_PUBLIC_RECTANGLE02_BIN).wid - gui_image_get_size(UI_BUF_I340001_PUBLIC_RECTANGLE02_BIN).hei,
                               gui_image_get_size(UI_BUF_I340001_PUBLIC_RECTANGLE02_BIN).hei);
    compo_textbox_set(yes_txt, i18n[STR_OK]);
...
#define SLEF_LIST_HEI_OFFSET  30
    ...
    compo_setid(listbox, COMPO_ID_LISTBOX);
    compo_listbox_set_sta_icon(listbox, UI_BUF_I342001_20_ALARM_CLOCK_SELECT_BIN, 0);
    compo_listbox_set_bithook(listbox, bsp_sys_get_ctlbit);
    compo_listbox_set_focus_byidx(listbox, 1);
    ...
    //新建按钮
    compo_button_t *btn;
    btn = compo_button_create_page_by_image(frm,listbox->page, UI_BUF_I342001_20_ALARM_CLOCK_CONFIRM_BIN);
    compo_setid(btn, COMPO_ID_BTN_REPETAT_YES);
    compo_button_set_pos(btn, GUI_SCREEN_CENTER_X,last_y);
    for (int i=0; i<7; i++)         //获取当前闹钟设置的星期
    {
...
static void func_alarm_clock_sub_repeat_enter(void)
{
    ...
    s32 first_y = compo_listbox_gety_byidx(listbox, 1);
    s32 last_y = compo_listbox_gety_byidx(listbox, WEEKS_LIST_CNT - 1);
    compo_listbox_move_init_modify(listbox, first_y-20, last_y);
#endif
