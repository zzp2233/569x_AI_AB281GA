...
bool compo_cardbox_icon_get_visible(compo_cardbox_t *cardbox, u8 idx)
{
    bool visible = false;
    if (cardbox->icon[idx] && idx < CARD_ICON_MAX)
    {
        visible = widget_get_visble(cardbox->icon[idx]);
    }
    return visible;
}
...
//                    printf("area[%d,%d] ", text_area.wid, textbox_rect.wid);
                    if (text_area.wid > textbox_rect.wid)
                    {
                        ...
                        //{
                        cardbox->roll_cb[i].offset = 0;//text_area.wid/2 - textbox_rect.wid/2;
