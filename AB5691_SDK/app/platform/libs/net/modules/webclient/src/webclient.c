                                     ...
                                     const char *url, const char **request)
{
    ...
    *request = path_ptr ? path_ptr : "/";
    ...
    /* get host address ok. */
    {
        char *host_addr_new = webclient_malloc(host_addr_len + 1);
...
static int webclient_open_tls(struct webclient_session *session, const char *URI)
{
    ...
    session->tls_session = (MbedTLSSession *) webclient_calloc(1, sizeof(MbedTLSSession));
    ...
    session->tls_session->buffer = webclient_malloc(session->tls_session->buffer_len);
...
static int webclient_connect(struct webclient_session *session, const char *URI)
{
    ...
    if (session->is_tls == RT_FALSE && res == NULL)
    {
        rc = -WEBCLIENT_ERROR;
        goto __exit;
    }
...
int webclient_header_fields_add(struct webclient_session *session, const char *fmt, ...)
{
    ...
    va_start(args, fmt);
    length = webclient_header_fields_add_valist(session, fmt, args);
    va_end(args);
    return length;
}
...
static int webclient_send_header(struct webclient_session *session, int method)
{
    ...
    /* get and echo request header data */
    {
        ...
        header_str = session->header->buffer;
        LOG_D("%s", header_str);
        for(; (header_ptr = strstr(header_str, "\r\n")) != NULL; )
        {
...
int webclient_handle_response(struct webclient_session *session)
{
    ...
    {
        uint16_t len = session->header->size;
        char *line = webclient_malloc(len);
...
struct webclient_session *webclient_session_create(size_t header_sz)
{
    ...
    /* create session */
    session = (struct webclient_session *) webclient_calloc(1, sizeof(struct webclient_session));
    if (session == NULL)
    {
...
{
    struct timeval timeout;
    int second = os_tick_from_ms(millisecond) / 1000;
...
static int webclient_clean(struct webclient_session *session)
{
    ...
    if (session->req_url)
    {
        webclient_free(session->req_url);
...
int webclient_close(struct webclient_session *session)
{
    ...
    {
        webclient_free(session->header->buffer);
    }
    ...
    *response = NULL;
    ...
    if (session->content_length < 0)
    {
        ...
        while (1)
        {
            ...
            new_resp = webclient_realloc(response_buf, result_sz + 1);
            if (new_resp == NULL)
            {
    ...
    else
    {
        ...
        response_buf = webclient_calloc(1, result_sz + 1);
...
int webclient_request_header_add(char **request_header, const char *fmt, ...)
{
    ...
    {
        header = webclient_calloc(1, WEBCLIENT_HEADER_BUFSZ);
        if (header == NULL)
        {
...
{
    return netlib_malloc(size, WEBCLIENT_HEAP_TAG);
}
void *webclient_calloc(size_t nitems, size_t size)
{
    return netlib_calloc(nitems, size, WEBCLIENT_HEAP_TAG);
}
void *webclient_realloc(void *p, size_t new_size)
{
    return netlib_realloc(p, new_size, WEBCLIENT_HEAP_TAG);
}
