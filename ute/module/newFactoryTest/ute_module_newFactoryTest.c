...
void uteModuleNewFactoryTestEverySecond(void)
{
    ...
    else if (uteModuleNewFactoryTestData.moduleType == FACTORY_MODULE_HEART_CHECK_BIO)
    {
        uteModuleFactoryTestGetVkBioData(&uteModuleNewFactoryTestData.u.vcxx.bioA, &uteModuleNewFactoryTestData.u.vcxx.bioB, &uteModuleNewFactoryTestData.u.vcxx.bioDiff);
        ...
        uteModuleNewFactoryTestData.u.vcxx.isPass = false;
        if (uteModuleNewFactoryTestData.u.vcxx.testMode == NEW_FACTORY_VCXX_TEST_MODE_BIO_B)
        {
            if (uteModuleNewFactoryTestData.u.vcxx.bioDiff <= bio_max || uteModuleNewFactoryTestData.u.vcxx.bioDiff >= bio_min)
            {
                uteModuleNewFactoryTestData.u.vcxx.isPass = true;
            }
        }
...
void uteModuleNewFactoryTestSetCheckLightMode(uint8_t mode)
{
    ...
    uteModuleFactoryTestSetCheckLightMode(mode);
}
/**
*@brief  获取心率检测模式
*@details
