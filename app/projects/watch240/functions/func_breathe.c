...
compo_form_t *func_breathe_form_create(void)
{
    ...
    compo_cardbox_t *cardbox = compo_cardbox_create(frm,0,3,1,228,68);
...
compo_form_t *func_breathe_form_create(void)
{
    ...
    compo_cardbox_t *cardbox = compo_cardbox_create(frm,0,3,2,346,102);
    compo_cardbox_set_pos(cardbox,GUI_SCREEN_CENTER_X,80+51);
    compo_setid(cardbox,COMPO_ID_BTN_TIME);

    compo_cardbox_icon_set_pos(cardbox,0,0,102 / 2 - 1);
    compo_cardbox_icon_set(cardbox, 0, UI_BUF_I341001_19_BREATHING_TRAINING_LINE_BIN);
    ...
    compo_cardbox_text_set_location(cardbox,0,106-GUI_SCREEN_CENTER_X,0 + 10,130,36);
    widget_set_align_center(cardbox->text[0],false);
    uteModuleCharencodeReplaceSubString( i18n[STR_MIN_JOINT],txt_buf,"##",txt_num_buf);
    compo_cardbox_text_set(cardbox,0,txt_buf);

    compo_cardbox_text_set_location(cardbox,1,106-GUI_SCREEN_CENTER_X,0 - 36,130,36);
    widget_set_align_center(cardbox->text[1],false);
    compo_cardbox_text_set_forecolor(cardbox,1,make_color(0x14,0xaf,0xff));
    compo_cardbox_text_set(cardbox,1,i18n[STR_DURATION]);

    cardbox = compo_cardbox_create(frm,0,3,2,346,102);
    ...
    compo_cardbox_text_set_location(cardbox,1,106-GUI_SCREEN_CENTER_X,0 - 36,130,36);
    widget_set_align_center(cardbox->text[1],false);
    compo_cardbox_text_set_forecolor(cardbox,1,make_color(0x00,0xdd,0xd2));
    compo_cardbox_text_set(cardbox,1,i18n[STR_SPEED]);
...
//    compo_textbox_set_visible(textbox,false);
    ...
    compo_cardbox_t *cardbox = compo_cardbox_create(frm,0,3,1,300,90);
    compo_cardbox_set_pos(cardbox,GUI_SCREEN_CENTER_X,69+90/2);
    compo_setid(cardbox,COMPO_ID_BTN_TIME);

    compo_cardbox_icon_set_pos(cardbox,0,0,0);
    compo_cardbox_icon_set(cardbox, 0, UI_BUF_I343001_19_BREATHING_TRAINING_BG_BIN);

    compo_cardbox_icon_set_pos(cardbox,1,24+38/2-GUI_SCREEN_CENTER_X,0);
    compo_cardbox_icon_set(cardbox, 1, UI_BUF_I343001_19_BREATHING_TRAINING_TIME_BIN);

    compo_cardbox_icon_set_pos(cardbox,2,264+12/2-GUI_SCREEN_CENTER_X,0);
    compo_cardbox_icon_set(cardbox, 2, UI_BUF_I343001_20_ALARM_CLOCK_NEXT_BIN);

    compo_cardbox_text_set_location(cardbox,0,83-GUI_SCREEN_CENTER_X,0-15,130,36);
    widget_set_align_center(cardbox->text[0],false);
    uteModuleCharencodeReplaceSubString( i18n[STR_MIN_JOINT],txt_buf,"##",txt_num_buf);
    compo_cardbox_text_set(cardbox,0,txt_buf);

    cardbox = compo_cardbox_create(frm,0,3,1,300,90);
    compo_cardbox_set_pos(cardbox,GUI_SCREEN_CENTER_X,171+90/2);
    compo_setid(cardbox,COMPO_ID_BTN_MODE);
    ...
    compo_cardbox_icon_set_pos(cardbox,1,24+38/2-GUI_SCREEN_CENTER_X,0);
    compo_cardbox_icon_set(cardbox, 1, UI_BUF_I343001_19_BREATHING_TRAINING_SPEED_BIN);
    ...
    //创建按钮
    compo_button_t* btn = compo_button_create_by_image(frm, UI_BUF_I343001_19_BREATHING_TRAINING_BUTTON_START_BIN);
    compo_button_set_pos(btn, GUI_SCREEN_CENTER_X,299+70/2);
    compo_setid(btn, COMPO_ID_BTN_START);
...
compo_form_t *func_breathe_form_create(void)
{
    ...
    //新建按钮
    compo_button_t *btn;
    btn = compo_button_create_by_image(frm, UI_BUF_I338001_19_BREATHING_TRAINING_START_BIN);
    compo_setid(btn, COMPO_ID_BTN_START);
    compo_button_set_pos(btn, GUI_SCREEN_CENTER_X, 60+120);

    btn = compo_button_create_by_image(frm, UI_BUF_I338001_19_BREATHING_TRAINING_TIME_BIN);
    compo_setid(btn, COMPO_ID_BTN_TIME);
    compo_button_set_pos(btn, 40/2+45, 40/2+149);

    btn = compo_button_create_by_image(frm, UI_BUF_I338001_19_BREATHING_TRAINING_ICON_BIN);
    compo_setid(btn, COMPO_ID_BTN_MODE);
    compo_button_set_pos(btn, 40/2+274, 40/2+149);
    ...
    uteModuleCharencodeReplaceSubString( i18n[STR_MIN_JOINT],txt_buf,"##",txt_num_buf);
    compo_textbox_t *txt_time = compo_textbox_create(frm, strlen(txt_buf));
    compo_textbox_set_location(txt_time,40/2+45, 24/2+204,80,35);
    compo_textbox_set(txt_time, txt_buf);

    compo_textbox_t *txt_mode = compo_textbox_create(frm, strlen(i18n[str_id]));
    compo_textbox_set_location(txt_mode,40/2+274, 24/2+204,80,35);
    compo_textbox_set(txt_mode, i18n[str_id]);
...
compo_form_t *func_breathe_form_create(void)
{
    ...
    //新建按钮
    compo_button_t *btn;
    btn = compo_button_create_by_image(frm, UI_BUF_I340001_PUBLIC_RECTANGLE00_BIN);
    compo_setid(btn, COMPO_ID_BTN_START);
    compo_button_set_pos(btn, GUI_SCREEN_CENTER_X, 64/2+266);

    btn = compo_button_create_by_image(frm, UI_BUF_I340001_BREATHE_TIME_BIN);
    compo_setid(btn, COMPO_ID_BTN_TIME);
    compo_button_set_pos(btn, 54/2+18, 54/2+153);

    btn = compo_button_create_by_image(frm, UI_BUF_I340001_BREATHE_SPEED_BIN);
//    widget_set_size(btn->widget, gui_image_get_size(UI_BUF_BREATHE_TIME_BIN).wid, gui_image_get_size(UI_BUF_BREATHE_TIME_BIN).hei);
    compo_setid(btn, COMPO_ID_BTN_MODE);
    compo_button_set_pos(btn, 54/2+288, 54/2+153);
    ...
    //新建图像
    compo_picturebox_t *pic;
    pic = compo_picturebox_create(frm, UI_BUF_I340001_BREATHE_BG_BIN);
    compo_setid(pic, COMPO_ID_PIC_BREATHE);
    compo_picturebox_set_pos(pic, 186/2+87,186/2+79);
...
compo_form_t *func_breathe_form_create(void)
{
    ...
    //新建按钮
    compo_button_t *btn;
    btn = compo_button_create_by_image(frm, UI_BUF_I342001_19_BREATHING_TRAINING_START_BIN);
    compo_setid(btn, COMPO_ID_BTN_START);
    compo_button_set_pos(btn, GUI_SCREEN_CENTER_X, 80+80/2);

    btn = compo_button_create_by_image(frm, UI_BUF_I342001_19_BREATHING_TRAINING_TIME_BIN);
    compo_setid(btn, COMPO_ID_BTN_TIME);
    compo_button_set_pos(btn, 26/2+30, 26/2+99);

    btn = compo_button_create_by_image(frm, UI_BUF_I342001_19_BREATHING_TRAINING_ICON_BIN);
    compo_setid(btn, COMPO_ID_BTN_MODE);
    compo_button_set_pos(btn, 26/2+183, 26/2+99);


    //创建文本
    compo_textbox_t *txt_start = compo_textbox_create(frm, strlen(i18n[STR_START]));
    compo_textbox_set_pos(txt_start,GUI_SCREEN_CENTER_X, 28/2+184);
    compo_setid(txt_start, COMPO_ID_TXT_START);
    compo_textbox_set(txt_start, i18n[STR_START]);
    ...
    uteModuleCharencodeReplaceSubString( i18n[STR_MIN_JOINT],txt_buf,"##",txt_num_buf);
    compo_textbox_t *txt_time = compo_textbox_create(frm, strlen(txt_buf));
    compo_textbox_set_location(txt_time,26/2+30, 35/2+136,80,35);
    compo_textbox_set(txt_time, txt_buf);

    compo_textbox_t *txt_mode = compo_textbox_create(frm, strlen(i18n[str_id]));
    compo_textbox_set_location(txt_mode,26/2+183, 35/2+136,80,35);
    compo_textbox_set(txt_mode, i18n[str_id]);
...
static void func_breathe_enter(void)
{
    if (breatheState)
    {
        sys_cb.breathe_mode = BREATHE_MODE_MEDIUM;
        breatheState = false;
    }
...
#elif (GUI_SCREEN_SIZE_240X284RGB_I330001_SUPPORT || GUI_SCREEN_SIZE_360X360RGB_I332001_SUPPORT || GUI_SCREEN_SIZE_240X284RGB_I335001_SUPPORT || GUI_SCREEN_SIZE_360X360RGB_I340001_SUPPORT)
    f_breathe_t *f_breathe = (f_breathe_t *)func_cb.f_cb;
    compo_picturebox_t *pic_breathe = compo_getobj_byid(COMPO_ID_PIC_BREATHE);
