...
//    printf("bt_music = %d\n",uteModuleNotDisturbIsAllowLocalRingtone());
    if(!uteModuleNotDisturbIsAllowLocalRingtone())
    {
        return;
    }
    ...
    if (bt_is_ring() && (get_music_dec_sta() == MUSIC_STOP) && (bt_cb.disp_status == BT_STA_INCOMING) && (!bt_cb.ring_stop))
    {
...
#endif
        {
            ...
            if ((!f_bt_ring.play.inband_ring_sta) && tick_check_expire(f_bt_ring.play.ring_tick, UTE_MODULE_BT_LOCAL_RINGTONE_DURATION * 1000))
            {
                bsp_bt_ring_sync();
                ute_local_ringtone_res_info_t *res_info = uteModuleLocalGetRingtoneResInfo(UTE_MODULE_BT_RINGTONE_HF_CALL_IN_INDEX);
                if (*res_info->len == 0 || *res_info->ptr == NULL)
                {
                    ...
                    bsp_bt_ring_res_play(RES_BUF_RING_CALL_MP3, RES_LEN_RING_CALL_MP3);
#endif
#endif
                }
                else
                {
                    bsp_bt_ring_res_play(*res_info->ptr, *res_info->len);
                }
...
void bsp_bt_ring_enter(void)
{
    ...
    if (tick_get() > (UTE_MODULE_BT_LOCAL_RINGTONE_DURATION + 1) * 1000)
    {
        f_bt_ring.play.ring_tick = tick_get() - (UTE_MODULE_BT_LOCAL_RINGTONE_DURATION + 1) * 1000;
    }
    else
    {
        f_bt_ring.play.ring_tick = 0;
    }
    ...
    bt_audio_bypass();
    music_control(MUSIC_MSG_STOP);
    u8 volume = uteModuleLocalRingtoneGetVolume();
    bsp_change_volume(volume);
