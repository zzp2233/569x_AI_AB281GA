...
#define MAX_NOTIFY_LEN          UTE_BLE_MTU_MAX_SIZE     //max=247
...
*   ble rx buf set
*/
#define BLE_CMD_BUF_LEN     10
#define BLE_CMD_BUF_MASK    (BLE_CMD_BUF_LEN - 1)
#define BLE_RX_BUF_LEN      UTE_BLE_MTU_MAX_SIZE
...
#if UTE_LOG_PROTOCOL_LVL
    if ((ble_cmd_cb.cmd_wptr - ble_cmd_cb.cmd_rptr) >= BLE_CMD_BUF_LEN)
    {
        UTE_MODULE_LOG(UTE_LOG_PROTOCOL_LVL, "BLE RX buffer overflow, data lost!");
    }
...
#if UTE_MODULE_PROTOCOL_HANDLER_INTO_USER_TASK_SUPPORT
    uteModulePlatformSendMsgToAppTask(MSG_TYPE_PROTOCOL_HANDLER, 0);
#endif
...
#if UTE_MODULE_PROTOCOL_HANDLER_INTO_USER_TASK_SUPPORT
void uteModuleProfileBleProtocolHandler(void)
{
...
{
    uint8_t mtu = ble_get_gatt_mtu();
    if (length > mtu)
    {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"%s,error! length:%d > mtu:%d",__func__,length,mtu);
        length = mtu;
    }
...
bool uteModuleProfileBleSendToPhone(uint8_t *data,uint8_t size)
{
    ...
    if(!uteModuleAppBindingGetOurAppConnection())
    {
...
#if APP_MODULE_ORAIMO_BINDING_SUPPORT
            || (data[0] == CMD_SET_ANCS_PARAM)
#endif
           )
        {
            // binding
        ...
        else if (data[0] == CMD_DEBUG_TEST_DATA)
        {
            UTE_MODULE_LOG(UTE_LOG_PROTOCOL_LVL,"%s,send debug data",__func__);
        }
        else
        {
...
{
    ble_app_need_wakeup_flag = false;
    ble_cmd_cb.cmd_rptr = 0;
    ble_cmd_cb.cmd_wptr = 0;
