...
//    {
//        memcpy(f_bt->title_buf, str, BT_TITLE_BUF_LEN-1);
//        msg_enqueue(EVT_ID3_TITLE_UPDATE);
//    }
//    else if (BT_ID3_TAG_ARTIST == type)
//    {
//        memcpy(f_bt->artist_buf, str, BT_ARTIST_BUF_LEN-1);
...
#define BT_TITLE_BUF_LEN     UTE_MUSIC_TITLE_MAX_SIZE   //歌名buf长度
#define BT_ARTIST_BUF_LEN    UTE_MUSIC_ARTLIST_MAX_SIZE     //歌手/歌手buf长度
...
{
    uint32_t tick;
    char title_buf[BT_TITLE_BUF_LEN];
    char artist_buf[BT_ARTIST_BUF_LEN];
    char title_buf_old[BT_TITLE_BUF_LEN];
    char artist_buf_old[BT_ARTIST_BUF_LEN];
    bool refresh_data;
} f_bt_t;
/*****************************************************************************
 *          func_bt_music(UI)
...
compo_form_t *func_bt_form_create(void)
{
    ...
    if(!bt_a2dp_profile_completely_connected() && !ble_is_connect())
    {
        ...
        str_txt = i18n[STR_CONNECT_BLUETOOTH];
        str_leng = strlen(i18n[STR_CONNECT_BLUETOOTH]);
    }
    else
    {
    ...
    //歌名
    compo_textbox_t *lyric_txt = compo_textbox_create(frm, str_leng);
    compo_textbox_set_location(lyric_txt, GUI_SCREEN_CENTER_X,80,200, 30);
    compo_textbox_set_autoroll_mode(lyric_txt, TEXT_AUTOROLL_MODE_SROLL_CIRC);
    compo_setid(lyric_txt, COMPO_ID_TXT_MUSIC_LYRIC);
    compo_textbox_set(lyric_txt,str_txt);
    ...
    {
        f_bt_t *f_bt = (f_bt_t *)func_cb.f_cb;
        f_bt->refresh_data = (bt_a2dp_profile_completely_connected() || ble_is_connect());
    }
...
static void func_bt_music_refresh_disp(void)
{
    ...
    {
        memcpy(f_bt->title_buf_old, title_buf, BT_TITLE_BUF_LEN);
...
#ifndef BT_TITLE_BUF_LEN
#define BT_TITLE_BUF_LEN     UTE_MUSIC_TITLE_MAX_SIZE   //歌名buf长度
#endif
#ifndef BT_ARTIST_BUF_LEN
#define BT_ARTIST_BUF_LEN    UTE_MUSIC_ARTLIST_MAX_SIZE     //歌手/歌手buf长度
#endif
#define PROGRESS_BAR_LENGTH 146
#define BTN_REST_DISP_TIME 300   //按钮释放时间 ms 级别
...
compo_form_t *func_bt_form_create(void)
{
    char title_buf[BT_TITLE_BUF_LEN] = {0};
    char artist_buf[BT_ARTIST_BUF_LEN] = {0};
    uint16_t title_size_leng  = 0;
    uint16_t artist_size_leng = 0;
    uteModuleMusicGetPlayerTitle((uint8_t *)title_buf,&title_size_leng);
    uteModuleMusicGetPlayerArtistSize((uint8_t *)artist_buf,&artist_size_leng);
    ...
    {
        memset(title_buf,0,sizeof(title_buf));
        snprintf(title_buf,sizeof(title_buf),"%s",i18n[STR_VOICE_BT_NOT_CONNECT]);
        memset(artist_buf,0,sizeof(artist_buf));
    }
    else if(title_size_leng==0)
    {
        ...
        {
            memset(title_buf,0,sizeof(title_buf));
            snprintf(title_buf,sizeof(title_buf),"%s",i18n[STR_NOT_PLAYED]);
        }
        else
        {
            memset(title_buf,0,sizeof(title_buf));
            snprintf(title_buf,sizeof(title_buf),"%s",i18n[STR_PLAYING]);
        }
        memset(artist_buf,0,sizeof(artist_buf));
    }
...
//    compo_form_set_title(frm, i18n[STR_MUSIC]);
    ...
    //bg
    compo_picturebox_t *bg_pic = compo_picturebox_create(frm, (!bt_a2dp_profile_completely_connected() && !ble_is_connect()) ? UI_BUF_I341001_13_MUSIC_BG1_BIN : UI_BUF_I341001_13_MUSIC_BG_BIN);
    compo_setid(bg_pic, COMPO_ID_MUSIC_BG_FIG);
    compo_picturebox_set_pos(bg_pic, GUI_SCREEN_CENTER_X, GUI_SCREEN_CENTER_Y);

    //歌名
    compo_textbox_t *name_txt = compo_textbox_create(frm, BT_TITLE_BUF_LEN);
    compo_textbox_set_location(name_txt, GUI_SCREEN_CENTER_X, 56, 320, 48);
    compo_textbox_set_autoroll_mode(name_txt, TEXT_AUTOROLL_MODE_SROLL_CIRC);
    compo_setid(name_txt, COMPO_ID_TXT_MUSIC_NAME);
    compo_textbox_set(name_txt, title_buf);

    compo_textbox_t *lyric_txt = compo_textbox_create(frm, BT_ARTIST_BUF_LEN);
    compo_textbox_set_location(lyric_txt, GUI_SCREEN_CENTER_X, 108, 320, 48);
    compo_textbox_set_autoroll_mode(lyric_txt, TEXT_AUTOROLL_MODE_SROLL_CIRC);
    compo_setid(lyric_txt, COMPO_ID_TXT_MUSIC_LYRIC);
    compo_textbox_set(lyric_txt, artist_buf);
    compo_textbox_set_forecolor(lyric_txt, COLOR_GRAY);

    compo_button_t *btn = compo_button_create_by_image(frm, UI_BUF_I341001_13_MUSIC_PREVIOUS_SONG_BIN);///上一曲
    //if(!uteModuleCallBtIsConnected() && !ble_is_connect())
    compo_setid(btn, COMPO_ID_BTN_PREV);
    compo_button_set_pos(btn, 37+21, 214+23);
    if(!bt_a2dp_profile_completely_connected() && !ble_is_connect())
    {
        compo_button_set_alpha(btn, 120);
    }
    else
    {
    ...
    //if(!uteModuleCallBtIsConnected() && !ble_is_connect())
    btn = compo_button_create_by_image(frm, UI_BUF_I341001_13_MUSIC_NEXT_SONG_BIN);///下一曲
    compo_setid(btn, COMPO_ID_BTN_NEXT);
    compo_button_set_pos(btn, 290+21, 214+23);
    if(!bt_a2dp_profile_completely_connected() && !ble_is_connect())
    {
        compo_button_set_alpha(btn, 120);
    }
    ...
    if(!bt_a2dp_profile_completely_connected() && !ble_is_connect())
    {
        btn = compo_button_create_by_image(frm, UI_BUF_I341001_13_MUSIC_PLAY_NOT_CONNECTED_BIN);///灰色的暂停按钮
    }
    else
    {
        btn = compo_button_create_by_image(frm, !uteModuleMusicGetPlayerPaused() ? UI_BUF_I341001_13_MUSIC_PLAY_BIN : UI_BUF_I341001_13_MUSIC_PAUSE_BIN);///暂停 播放
    }
    compo_setid(btn, COMPO_ID_BTN_PLAY);
    compo_button_set_pos(btn, GUI_SCREEN_CENTER_X, 170+122/2);

    btn = compo_button_create_by_image(frm, UI_BUF_I341001_13_MUSIC_ICON_VOLUME_MIN_00_BIN);///音量减
    if(!bt_a2dp_profile_completely_connected() && !ble_is_connect())
    {
        compo_button_set_alpha(btn, 120);
//        btn = compo_button_create_by_image(frm, UI_BUF_I341001_13_MUSIC_ICON_VOLUME_MIN_01_BIN);//////音量减 灰
    }
    else
    {
        compo_button_set_alpha(btn, 255);
//        btn = compo_button_create_by_image(frm, UI_BUF_I341001_13_MUSIC_ICON_VOLUME_MIN_00_BIN);///音量减
    ...
    compo_setid(btn, COMPO_ID_BTN_VOL_DOWN);
    compo_button_set_pos(btn, 46+13,382+16);

    btn = compo_button_create_by_image(frm, UI_BUF_I341001_13_MUSIC_ICON_VOLUME_MAX_00_BIN);///音量加
    if(!bt_a2dp_profile_completely_connected() && !ble_is_connect())
    {
        compo_button_set_alpha(btn, 120);
//        btn = compo_button_create_by_image(frm, UI_BUF_I341001_13_MUSIC_ICON_VOLUME_MAX_01_BIN);///音量加 灰
    }
    else
    {
        compo_button_set_alpha(btn, 255);
//        btn = compo_button_create_by_image(frm, UI_BUF_I341001_13_MUSIC_ICON_VOLUME_MAX_00_BIN);///音量加
    ...
    compo_setid(btn, COMPO_ID_BTN_VOL_UP);
    compo_button_set_pos(btn, 304+13, 382+16);

    uint8_t vol = uteModuleMusicGetPlayerVolume() / 6;
    if(vol>16)vol=16;

    compo_picturebox_t *pic = compo_picturebox_create(frm, UI_BUF_I341001_13_MUSIC_SOUND_BIN);
    compo_picturebox_set_pos(pic, GUI_SCREEN_CENTER_X, 394 + 4);
    compo_picturebox_cut(pic, vol, 17);
    compo_picturebox_set_size(pic, 184, 8);
    compo_setid(pic, COMPO_ID_SHAPE_MUSIC_VOL);
    if(!bt_a2dp_profile_completely_connected() && !ble_is_connect())
    {
        compo_picturebox_set_alpha(pic, 120);
    }
    else
    {
        compo_picturebox_set_alpha(pic, 255);
    }
    ...
    if(uteModuleSportMoreSportGetStatus() != ALL_SPORT_STATUS_CLOSE)
    {
        ...
        shape = compo_shape_create(frm, COMPO_SHAPE_TYPE_RECTANGLE);
        ute_compo_shape_set(shape,GUI_SCREEN_CENTER_X,430 + 12 / 2, 12,12,12 / 2,0XFF,make_color(0x66,0x66,0x66));

        shape = compo_shape_create(frm, COMPO_SHAPE_TYPE_RECTANGLE);
        ute_compo_shape_set(shape,GUI_SCREEN_CENTER_X + 20,430 + 12 / 2, 12,12,12 / 2,0XFF,make_color(0xff,0xff,0xff));
    }
    return frm;
}
static void func_bt_music_refresh_disp(void)
{
    f_bt_t *f_bt = (f_bt_t *)func_cb.f_cb;
    compo_picturebox_t *picbg = compo_getobj_byid(COMPO_ID_MUSIC_BG_FIG);
    compo_picturebox_t *pic = compo_getobj_byid(COMPO_ID_SHAPE_MUSIC_VOL);
    compo_button_t *btn_play = compo_getobj_byid(COMPO_ID_BTN_PLAY);
    compo_button_t *btn_vol_up = compo_getobj_byid(COMPO_ID_BTN_VOL_UP);
    compo_button_t *btn_vol_down = compo_getobj_byid(COMPO_ID_BTN_VOL_DOWN);
    compo_button_t *btn_next = compo_getobj_byid(COMPO_ID_BTN_NEXT);
    compo_button_t *btn_pre = compo_getobj_byid(COMPO_ID_BTN_PREV);
    compo_textbox_t *tilte_txt = compo_getobj_byid(COMPO_ID_TXT_MUSIC_NAME);
    compo_textbox_t *tilte_art_txt = compo_getobj_byid(COMPO_ID_TXT_MUSIC_LYRIC);
    ...
    {
        compo_textbox_set(tilte_txt, i18n[STR_VOICE_BT_NOT_CONNECT]);
        compo_textbox_set(tilte_art_txt, i18n[STR_NULL]);
    }
    else
    {
        uteModuleMusicGetPlayerTitle((uint8_t *)title_buf, &title_size_leng);
        ...
        if (title_size_leng == 0)
        {
            if (uteModuleMusicGetPlayerPaused())
            {
                compo_textbox_set(tilte_txt, i18n[STR_NOT_PLAYED]);
            }
            else
            {
                compo_textbox_set(tilte_txt, i18n[STR_PLAYING]);
            }
            compo_textbox_set(tilte_art_txt, i18n[STR_NULL]);
        }
        else
        {
            if (strcmp(title_buf, f_bt->title_buf_old) != 0)
            {
                memcpy(f_bt->title_buf_old, title_buf, BT_TITLE_BUF_LEN);
                compo_textbox_set(tilte_txt, title_buf);
            }

            if (strcmp(artist_buf, f_bt->artist_buf_old) != 0 || artist_size_leng == 0)
            {
                memcpy(f_bt->artist_buf_old, artist_buf, BT_ARTIST_BUF_LEN);
                compo_textbox_set(tilte_art_txt, artist_buf);
            }
        }
    }
    ...
    if (!bt_a2dp_profile_completely_connected() && !ble_is_connect())
    {
        compo_button_set_bgimg(btn_play, UI_BUF_I341001_13_MUSIC_PLAY_NOT_CONNECTED_BIN);     /// 灰色的暂停按钮
//        compo_button_set_bgimg(btn_vol_up, UI_BUF_I341001_13_MUSIC_ICON_VOLUME_MAX_01_BIN);   /// 灰色的音量+
//        compo_button_set_bgimg(btn_vol_down, UI_BUF_I341001_13_MUSIC_ICON_VOLUME_MIN_01_BIN); /// 灰色的音量-
        compo_picturebox_set(picbg, UI_BUF_I341001_13_MUSIC_BG1_BIN);
        compo_button_set_alpha(btn_pre, 120);
        compo_button_set_alpha(btn_next, 120);
        compo_button_set_alpha(btn_vol_up, 120);
        compo_button_set_alpha(btn_vol_down, 120);
        compo_picturebox_set_alpha(pic, 120);
    }
    else
    {
        compo_button_set_bgimg(btn_play, !uteModuleMusicGetPlayerPaused() ? UI_BUF_I341001_13_MUSIC_PLAY_BIN : UI_BUF_I341001_13_MUSIC_PAUSE_BIN); /// 暂停 播放
//        compo_button_set_bgimg(btn_vol_up, UI_BUF_I341001_13_MUSIC_ICON_VOLUME_MAX_00_BIN);                                                        /// 音量+
//        compo_button_set_bgimg(btn_vol_down, UI_BUF_I341001_13_MUSIC_ICON_VOLUME_MIN_00_BIN);                                                      /// 音量-
        compo_picturebox_set(picbg, UI_BUF_I341001_13_MUSIC_BG_BIN);
        compo_button_set_alpha(btn_pre, 255);
        compo_button_set_alpha(btn_next, 255);
        compo_button_set_alpha(btn_vol_up, 255);
        compo_button_set_alpha(btn_vol_down, 255);
        compo_picturebox_set_alpha(pic, 255);
    }
...
compo_form_t *func_bt_form_create(void)
{
    ...
    //bg
    compo_picturebox_t *bg_pic = compo_picturebox_create(frm, UI_BUF_I343001_13_MUSIC_BG1_BIN);
    compo_setid(bg_pic, COMPO_ID_MUSIC_BG_FIG);
    compo_picturebox_set_pos(bg_pic, GUI_SCREEN_CENTER_X, GUI_SCREEN_CENTER_Y);
    //compo_picturebox_cut(bg_pic,0,2);

    //歌名
    compo_textbox_t *lyric_txt = compo_textbox_create(frm, BT_TITLE_BUF_LEN);
    compo_textbox_set_location(lyric_txt, GUI_SCREEN_CENTER_X, 76+30/2, 200, 30);
    compo_textbox_set_autoroll_mode(lyric_txt, TEXT_AUTOROLL_MODE_SROLL_CIRC);
    compo_setid(lyric_txt, COMPO_ID_TXT_MUSIC_LYRIC);
    compo_textbox_set(lyric_txt,title_buf);

    compo_button_t *btn = compo_button_create_by_image(frm, UI_BUF_I343001_13_MUSIC_PREVIOUSSONG01_BIN);///上一曲
    //if(!uteModuleCallBtIsConnected() && !ble_is_connect())
    compo_setid(btn, COMPO_ID_BTN_PREV);
    compo_button_set_pos(btn, 32+36/2, 186+40/2);

    //if(!uteModuleCallBtIsConnected() && !ble_is_connect())
    btn = compo_button_create_by_image(frm, UI_BUF_I343001_13_MUSIC_NEXTSONG01_BIN);///下一曲
    compo_setid(btn, COMPO_ID_BTN_NEXT);
    compo_button_set_pos(btn, 252+36/2, 186+40/2);
    ...
    {
        // compo_button_set_bgimg(btn,UI_BUF_I335001_MUSIC_2_NOT_CONNECTED_ICON_96X96_X72_Y104_BIN);
        btn = compo_button_create_by_image(frm,UI_BUF_I343001_13_MUSIC_PLAY02_BIN);///灰色的暂停按钮
    }
    else
    {
        btn = compo_button_create_by_image(frm,bt_cb.music_playing ? UI_BUF_I343001_13_MUSIC_PAUSED_BIN:UI_BUF_I343001_13_MUSIC_PLAY01_BIN);///暂停 播放
    }
    compo_setid(btn, COMPO_ID_BTN_PLAY);
    compo_button_set_pos(btn, GUI_SCREEN_CENTER_X, 136+128/2);

    btn = compo_button_create_by_image(frm, UI_BUF_I343001_13_MUSIC_VOLUME00_BIN);///音量减
    compo_setid(btn, COMPO_ID_BTN_VOL_DOWN);
    compo_button_set_pos(btn, 32+32/2,332+28/2);

    btn = compo_button_create_by_image(frm, UI_BUF_I343001_13_MUSIC_VOLUME01_BIN);///音量加
    compo_setid(btn, COMPO_ID_BTN_VOL_UP);
    compo_button_set_pos(btn, 256+32/2,332+28/2);

    uint8_t vol = uteModuleMusicGetPlayerVolume() / 6;
    if(vol>16)vol=16;

    compo_picturebox_t *pic = compo_picturebox_create(frm,UI_BUF_I343001_13_MUSIC_VOLUME_160X6_BIN);
    compo_picturebox_set_pos(pic,GUI_SCREEN_CENTER_X,344+6/2);
    compo_picturebox_cut(pic,vol,17);
    compo_picturebox_set_size(pic, 160, 6);
    compo_setid(pic,COMPO_ID_SHAPE_MUSIC_VOL);
...
static void func_bt_music_refresh_disp(void)
{
    ...
    {
        btn_play   = compo_getobj_byid(COMPO_ID_BTN_PLAY);
        compo_button_set_bgimg(btn_play,UI_BUF_I343001_13_MUSIC_PLAY02_BIN);///灰色的暂停按钮
    }
    else
    {
        btn_play   = compo_getobj_byid(COMPO_ID_BTN_PLAY);
        compo_button_set_bgimg(btn_play,bt_cb.music_playing ? UI_BUF_I343001_13_MUSIC_PAUSED_BIN:UI_BUF_I343001_13_MUSIC_PLAY01_BIN);///暂停 播放
    }
    uint8_t vol = uteModuleMusicGetPlayerVolume() / 6;
    if(vol>16)vol=16;
    compo_picturebox_cut(pic,vol,17);
...
compo_form_t *func_bt_form_create(void)
{
    ...
    if(bt_a2dp_profile_completely_connected() || ble_is_connect())
    {
        ...
        compo_picturebox_t* pic = compo_picturebox_create(frm, UI_BUF_I332001_MUSIC_VOLUME02_BIN);//音量减
        compo_picturebox_set_pos(pic, 90,302);
        compo_setid(pic, COMPO_ID_PIC_DOWN);
        compo_picturebox_set_visible(pic, true);

        compo_button_t* but =compo_button_create(frm);
        compo_setid(but, COMPO_ID_BTN_VOL_DOWN);
        compo_button_set_location(but,90,302,42+40,42+40);

        // btn = compo_button_create_by_image(frm, UI_BUF_I332001_MUSIC_VOLUME03_BIN);///音量加
        // compo_setid(btn, COMPO_ID_BTN_VOL_UP);
        // compo_button_set_pos(btn, 270, 302);

        pic = compo_picturebox_create(frm, UI_BUF_I332001_MUSIC_VOLUME03_BIN);//音量加
        compo_picturebox_set_pos(pic, 270, 302);
        compo_setid(pic, COMPO_ID_PIC_UP);
        compo_picturebox_set_visible(pic, true);

        but =compo_button_create(frm);
        compo_setid(but, COMPO_ID_BTN_VOL_UP);
        compo_button_set_location(but,270, 302,42+40,42+40);
    ...
    else
    {
        ...
        // compo_setid(btn, COMPO_ID_BTN_VOL_DOWN);
        // compo_button_set_pos(btn, 90,302);
        // compo_button_set_location(btn,180/2,302,180,42+40);
        ...
        // compo_setid(btn, COMPO_ID_BTN_VOL_UP);
        // compo_button_set_pos(btn, 270, 302);
        // compo_button_set_location(btn,180+180/2, 302,180,42+40);

        pic = compo_picturebox_create(frm, UI_BUF_I332001_MUSIC_VOLUME04_BIN);//音量加
        compo_picturebox_set_pos(pic,  270, 302);
        compo_setid(pic, COMPO_ID_PIC_UP);
        compo_picturebox_set_visible(pic, true);
...
static void func_bt_music_refresh_disp(void)
{
    ...
    compo_button_t *btn_next = compo_getobj_byid(COMPO_ID_BTN_NEXT);
    compo_button_t *btn_play = compo_getobj_byid(COMPO_ID_BTN_PLAY);
    compo_shape_t *btn_up   = compo_getobj_byid(COMPO_ID_PIC_UP);
    compo_shape_t *btn_down = compo_getobj_byid(COMPO_ID_PIC_DOWN);
    compo_textbox_t *tilte_txt = compo_getobj_byid(COMPO_ID_TXT_MUSIC_LYRIC);
    compo_textbox_t *tilte_art_txt = compo_getobj_byid(COMPO_ID_TXT_MUSIC_NAME);
    ...
    if(!bt_a2dp_profile_completely_connected() && !ble_is_connect())
    {
        ...
        compo_picturebox_set(btn_up,   UI_BUF_I332001_MUSIC_VOLUME04_BIN);
        compo_picturebox_set(btn_down, UI_BUF_I332001_MUSIC_VOLUME02_BIN);
        memset(f_bt->title_buf,1,sizeof(f_bt->title_buf_old));
        memset(f_bt->artist_buf,1,sizeof(f_bt->artist_buf_old));
        f_bt->refresh_data = true;
        return;
...
static void func_bt_button_release_handle()
{
    ...
    if(f_bt->tick <= tick_get()-BTN_REST_DISP_TIME)
    {
        ...
        compo_picturebox_set(btn_up,UI_BUF_I332001_MUSIC_VOLUME03_BIN );
        compo_picturebox_set(btn_down,UI_BUF_I332001_MUSIC_VOLUME01_BIN );
    }
...
compo_form_t *func_bt_form_create(void)
{
    ...
    compo_picturebox_t *picbox = compo_picturebox_create(frm,(bt_a2dp_profile_completely_connected() || ble_is_connect()) ? UI_BUF_I338001_13_MUSIC_BG1_BIN : UI_BUF_I338001_13_MUSIC_BG_BIN);
    compo_picturebox_set_pos(picbox,GUI_SCREEN_CENTER_X,GUI_SCREEN_CENTER_Y);
    compo_setid(picbox,COMPO_ID_PIC_BG);

    //歌名
    compo_textbox_t *lyric_txt = compo_textbox_create(frm, str_leng);
    compo_textbox_set_location(lyric_txt, GUI_SCREEN_CENTER_X, 34/2+63, 200, 50);
    compo_textbox_set_autoroll_mode(lyric_txt, TEXT_AUTOROLL_MODE_SROLL_CIRC);
    compo_setid(lyric_txt, COMPO_ID_TXT_MUSIC_LYRIC);
    compo_textbox_set(lyric_txt,str_txt);

    compo_button_t *btn = compo_button_create_by_image(frm, UI_BUF_I338001_13_MUSIC_PREVIOUSSONG01_BIN);///上一曲
    compo_setid(btn, COMPO_ID_BTN_PREV);
    compo_button_set_pos(btn, GUI_SCREEN_CENTER_X-GUI_SCREEN_CENTER_X/1.5, GUI_SCREEN_CENTER_Y);

    btn = compo_button_create_by_image(frm, UI_BUF_I338001_13_MUSIC_NEXTSONG01_BIN);///下一曲
    compo_setid(btn, COMPO_ID_BTN_NEXT);
    compo_button_set_pos(btn, GUI_SCREEN_CENTER_X+GUI_SCREEN_CENTER_X/1.5, GUI_SCREEN_CENTER_Y);
    ...
    btn = compo_button_create_image_in_btn(frm, UI_BUF_I338001_13_MUSIC_VOLUME01_BIN, 90,302,50,50);///音量减
    compo_setid(btn, COMPO_ID_BTN_VOL_DOWN);

    btn = compo_button_create_image_in_btn(frm, UI_BUF_I338001_13_MUSIC_VOLUME03_BIN,270, 302,50,50);///音量加
    compo_setid(btn, COMPO_ID_BTN_VOL_UP);
    ...
    compo_shape_set_location(shape,  (GUI_SCREEN_WIDTH-128)/2+vol*(128/16)/2,302,vol*(128/16), 4 );
    compo_shape_set_radius(shape, 1);
    compo_shape_set_visible(shape, (bt_a2dp_profile_completely_connected() || ble_is_connect()));
...
compo_form_t *func_bt_form_create(void)
{
    ...
    compo_form_set_mode(frm, COMPO_FORM_MODE_SHOW_TITLE | COMPO_FORM_MODE_SHOW_TIME);
    compo_form_set_title(frm, i18n[STR_MUSIC]);
    compo_form_add_image(frm, UI_BUF_I340001_MUSIC_FRAME_2313_BIN, GUI_SCREEN_CENTER_X,GUI_SCREEN_CENTER_Y );
    ...
    if(bt_a2dp_profile_completely_connected() || ble_is_connect())
    {
        ...
        compo_button_t *btn = compo_button_create_by_image(frm, UI_BUF_I340001_MUSIC_PREVIOUSSONG01_BIN);///上一曲
        compo_setid(btn, COMPO_ID_BTN_PREV);
        compo_button_set_pos(btn, GUI_SCREEN_CENTER_X-GUI_SCREEN_CENTER_X/1.5, GUI_SCREEN_CENTER_Y);

        btn = compo_button_create_by_image(frm, UI_BUF_I340001_MUSIC_NEXTSONG01_BIN);///下一曲
        compo_setid(btn, COMPO_ID_BTN_NEXT);
        compo_button_set_pos(btn, GUI_SCREEN_CENTER_X+GUI_SCREEN_CENTER_X/1.5, GUI_SCREEN_CENTER_Y);
        ...
        btn = compo_button_create_by_image(frm,bt_cb.music_playing ? UI_BUF_I340001_MUSIC_PAUSED_BIN:UI_BUF_I340001_MUSIC_PLAY01_BIN);///暂停 播放
        compo_setid(btn, COMPO_ID_BTN_PLAY);
        compo_button_set_pos(btn, GUI_SCREEN_CENTER_X, GUI_SCREEN_CENTER_Y);

        // btn = compo_button_create_by_image(frm, UI_BUF_I340001_MUSIC_VOLUME01_BIN);///音量减
        // compo_setid(btn, COMPO_ID_BTN_VOL_DOWN);
        // compo_button_set_pos(btn, 90,302);

        compo_picturebox_t* pic = compo_picturebox_create(frm, UI_BUF_I340001_MUSIC_VOLUME02_BIN);//音量减
        compo_picturebox_set_pos(pic, 90,302);
        compo_setid(pic, COMPO_ID_PIC_DOWN);
        compo_picturebox_set_visible(pic, true);
        ...
        // btn = compo_button_create_by_image(frm, UI_BUF_I340001_MUSIC_VOLUME03_BIN);///音量加
        // compo_setid(btn, COMPO_ID_BTN_VOL_UP);
        // compo_button_set_pos(btn, 270, 302);

        pic = compo_picturebox_create(frm, UI_BUF_I340001_MUSIC_VOLUME03_BIN);//音量加
        compo_picturebox_set_pos(pic, 270, 302);
        compo_setid(pic, COMPO_ID_PIC_UP);
        compo_picturebox_set_visible(pic, true);
    ...
    else
    {
        ...
        compo_button_t *btn = compo_button_create_by_image(frm, UI_BUF_I340001_MUSIC_PREVIOUSSONG02_BIN);///上一曲
        compo_setid(btn, COMPO_ID_BTN_PREV);
        compo_button_set_pos(btn, GUI_SCREEN_CENTER_X-GUI_SCREEN_CENTER_X/1.5, GUI_SCREEN_CENTER_Y);

        btn = compo_button_create_by_image(frm, UI_BUF_I340001_MUSIC_NEXTSONG02_BIN);///下一曲
        compo_setid(btn, COMPO_ID_BTN_NEXT);
        compo_button_set_pos(btn, GUI_SCREEN_CENTER_X+GUI_SCREEN_CENTER_X/1.5, GUI_SCREEN_CENTER_Y);
        ...
        btn = compo_button_create_by_image(frm,UI_BUF_I340001_MUSIC_PLAY02_BIN);///暂停 播放
        compo_setid(btn, COMPO_ID_BTN_PLAY);
        compo_button_set_pos(btn, GUI_SCREEN_CENTER_X, GUI_SCREEN_CENTER_Y);

        // btn = compo_button_create_by_image(frm, UI_BUF_I340001_MUSIC_VOLUME02_BIN);///音量减
        // compo_setid(btn, COMPO_ID_BTN_VOL_DOWN);
        // compo_button_set_pos(btn, 90,302);
        // compo_button_set_location(btn,180/2,302,180,42+40);
        ...
        // btn = compo_button_create_by_image(frm, UI_BUF_I340001_MUSIC_VOLUME04_BIN);///音量加
        // compo_setid(btn, COMPO_ID_BTN_VOL_UP);
        // compo_button_set_pos(btn, 270, 302);
        // compo_button_set_location(btn,180+180/2, 302,180,42+40);

        pic = compo_picturebox_create(frm, UI_BUF_I340001_MUSIC_VOLUME04_BIN);//音量加
        compo_picturebox_set_pos(pic,  270, 302);
        compo_setid(pic, COMPO_ID_PIC_UP);
        compo_picturebox_set_visible(pic, true);
...
static void func_bt_music_refresh_disp(void)
{
    ...
    if(!bt_a2dp_profile_completely_connected() && !ble_is_connect())
    {
        ...
        compo_setid(tilte_txt, COMPO_ID_TXT_MUSIC_LYRIC);
        compo_shape_set_visible(shape,false);
        compo_button_set_bgimg(btn_play, UI_BUF_I340001_MUSIC_PLAY02_BIN);
        compo_button_set_bgimg(btn_next, UI_BUF_I340001_MUSIC_NEXTSONG02_BIN);
        compo_button_set_bgimg(btn_prev, UI_BUF_I340001_MUSIC_PREVIOUSSONG02_BIN);
        compo_picturebox_set(btn_up,   UI_BUF_I340001_MUSIC_VOLUME04_BIN);
        compo_picturebox_set(btn_down, UI_BUF_I340001_MUSIC_VOLUME02_BIN);
        memset(f_bt->title_buf,1,sizeof(f_bt->title_buf_old));
        memset(f_bt->artist_buf,1,sizeof(f_bt->artist_buf_old));
        f_bt->refresh_data = true;
        return;
    ...
    compo_button_set_bgimg(btn_play, bt_cb.music_playing ? UI_BUF_I340001_MUSIC_PAUSED_BIN:UI_BUF_I340001_MUSIC_PLAY01_BIN);
...
static void func_bt_button_release_handle()
{
    ...
    if(f_bt->tick <= tick_get()-BTN_REST_DISP_TIME)
    {
        compo_button_set_bgimg(btn_prev,UI_BUF_I340001_MUSIC_PREVIOUSSONG01_BIN );
        compo_button_set_bgimg(btn_next,UI_BUF_I340001_MUSIC_NEXTSONG01_BIN );
        compo_picturebox_set(btn_up,UI_BUF_I340001_MUSIC_VOLUME03_BIN );
        compo_picturebox_set(btn_down,UI_BUF_I340001_MUSIC_VOLUME01_BIN );
    }
...
compo_form_t *func_bt_form_create(void)
{
    ...
    compo_picturebox_t *picbox = compo_picturebox_create(frm,(bt_a2dp_profile_completely_connected() || ble_is_connect()) ? UI_BUF_I342001_13_MUSIC_BG1_BIN : UI_BUF_I342001_13_MUSIC_BG0_BIN);
    compo_picturebox_set_pos(picbox,GUI_SCREEN_CENTER_X,GUI_SCREEN_CENTER_Y);
    compo_setid(picbox,COMPO_ID_PIC_BG);

    //歌名
    compo_textbox_t *lyric_txt = compo_textbox_create(frm, 50);
    compo_textbox_set_location(lyric_txt, GUI_SCREEN_CENTER_X, 50/2+35, 200, 50);
    compo_textbox_set_autoroll_mode(lyric_txt, TEXT_AUTOROLL_MODE_SROLL_CIRC);
    compo_setid(lyric_txt, COMPO_ID_TXT_MUSIC_LYRIC);
    compo_textbox_set(lyric_txt,title_buf);

    compo_button_t *btn = compo_button_create_by_image(frm, UI_BUF_I342001_13_MUSIC_PREVIOUSSONG01_BIN);///上一曲
    compo_setid(btn, COMPO_ID_BTN_PREV);
    compo_button_set_pos(btn, GUI_SCREEN_CENTER_X-GUI_SCREEN_CENTER_X/1.5, GUI_SCREEN_CENTER_Y);

    btn = compo_button_create_by_image(frm, UI_BUF_I342001_13_MUSIC_NEXTSONG01_BIN);///下一曲
    compo_setid(btn, COMPO_ID_BTN_NEXT);
    compo_button_set_pos(btn, GUI_SCREEN_CENTER_X+GUI_SCREEN_CENTER_X/1.5, GUI_SCREEN_CENTER_Y);
    ...
    btn = compo_button_create_by_image(frm,(bt_a2dp_profile_completely_connected() || ble_is_connect()) ? bt_cb.music_playing ? UI_BUF_I342001_13_MUSIC_PAUSED_BIN : UI_BUF_I342001_13_MUSIC_PLAY01_BIN : UI_BUF_I342001_13_MUSIC_PLAT02_BIN);///暂停 播放
    compo_setid(btn, COMPO_ID_BTN_PLAY);
    compo_button_set_pos(btn, GUI_SCREEN_CENTER_X, GUI_SCREEN_CENTER_Y);

    btn = compo_button_create_by_image(frm, UI_BUF_I342001_13_MUSIC_VOLUME00_BIN);///音量减
    compo_setid(btn, COMPO_ID_BTN_VOL_DOWN);
    compo_button_set_pos(btn, 36+19/2,178+19/2);

    btn = compo_button_create_by_image(frm, UI_BUF_I342001_13_MUSIC_VOLUME01_BIN);///音量加
    compo_setid(btn, COMPO_ID_BTN_VOL_UP);
    compo_button_set_pos(btn, 190+19/2, 178+19/2);

    compo_shape_t *shape = compo_shape_create(frm,COMPO_SHAPE_TYPE_RECTANGLE);///音量进度条背景
    compo_shape_set_location(shape, GUI_SCREEN_CENTER_X, 178+19/2, 106, 4 );
    compo_shape_set_radius(shape, 3);
    compo_shape_set_color(shape,make_color(0x33,0x33,0x33) );
    ...
    shape = compo_shape_create(frm,COMPO_SHAPE_TYPE_RECTANGLE);///音量进度条
    compo_setid(shape, COMPO_ID_SHAPE_MUSIC_VOL);
    compo_shape_set_location(shape,  (GUI_SCREEN_WIDTH-106)/2+vol*(106/16)/2,178+19/2,vol*(106/16), 4 );
    compo_shape_set_radius(shape, 1);
...
static void func_bt_music_refresh_disp(void)
{
    ...
    compo_textbox_t *tilte_txt = compo_getobj_byid(COMPO_ID_TXT_MUSIC_LYRIC);
    compo_shape_t *shape = compo_getobj_byid(COMPO_ID_SHAPE_MUSIC_VOL);
    compo_picturebox_t *pic_bg = compo_getobj_byid(COMPO_ID_PIC_BG);

    if(!bt_a2dp_profile_completely_connected() && !ble_is_connect())
    {
        compo_button_set_bgimg(btn_play, UI_BUF_I342001_13_MUSIC_PLAT02_BIN);
        compo_textbox_set(tilte_txt, i18n[STR_UNKNOWN]);
        compo_shape_set_visible(shape,false);
        compo_picturebox_set(pic_bg,UI_BUF_I342001_13_MUSIC_BG0_BIN);
        return;
    }
    uint8_t vol = uteModuleMusicGetPlayerVolume() / 6;
    uint16_t title_size_leng  = 0;
    ...
    compo_picturebox_set(pic_bg,UI_BUF_I342001_13_MUSIC_BG1_BIN);
    ...
    compo_button_set_bgimg(btn_play, bt_cb.music_playing ? UI_BUF_I342001_13_MUSIC_PAUSED_BIN : UI_BUF_I342001_13_MUSIC_PLAY01_BIN);
...
static void func_bt_message_do(size_msg_t msg)
{
    ...
    switch (msg)
    {
        ...
        case MSG_CTP_SHORT_RIGHT:
            if(uteModuleSportMoreSportGetStatus() != ALL_SPORT_STATUS_CLOSE)
            {
                func_switch_to(FUNC_SPORT_SUB_RUN, FUNC_SWITCH_LR_RIGHT);
                task_stack_pop();
                return;
            }
            else
