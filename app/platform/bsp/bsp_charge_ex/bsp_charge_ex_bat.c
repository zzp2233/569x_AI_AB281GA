...
const int vbat_corrected = 30;                                 //电压采集修正(工作放电), 便于整体加减放电表测试
const int vbat_corrected_charge = 30;                          //电压采集修正(工作充电)
const u8 vbat_corrected_idle = 30;                             //idle模式修正
//const int tbl_vbat_corrected_bright = -30;                   //屏亮度修正
const int vbat_corrected_earphone_charge[EARPHONE_MAX] =       //电压采集修正(耳机充电)
{
    -15, -15
    };
...
#else
const u16 tbl_box_percent[1] = {3100};
u8 bsp_charge_ex_percent_show_get(void)
{
    return 0;
}
