...
#include "spt5113c_firmware_YCY_5758B01_04.h"
#elif UTE_DRV_TFT_S360X360_GC9B71_HY139074A_QSPI_SUPPORT
#include "spt5113c_firmware_YCY_JY139012AV0_01.h"
#elif UTE_DRV_AMOLED_S368X448_CH13620_193WY01012_QSPI_SUPPORT || UTE_DRV_AMOLED_S368X448_GC3A71_193WG2014V1_QSPI_SUPPORT
#include "spt5113c_firmware_YCY_AZ192012A1_03.h"
...
#include "spt5113c_firmware_YCY_5826B01_04.h"
#else
#include "spt5113c_firmware_YCY_5758B01_A0_01.h"
#endif

// 动态申请内存升级TP固件 1: malloc, 0: static
#define SPT5113C_FIRMWARE_MALLOC 1

//噪声检测及跳频
#define APP_FUNC_FREQ_HOPPING 0 //=1=使能噪声检测及跳频功能，=0=不使能

static bool spt5113c_firmware_update(void);
static bool spt5113c_is_init = false;
static u16 spt5113c_frequency = 0;
typedef enum
{
    EXEC_OK = 0,
    CHIP_CODE_ERR,
...
#if (SPT5113C_FIRMWARE_MALLOC)
static uint8_t *ctp_fw_temp_buffer = NULL;
#else
...
{
#if (SPT5113C_FIRMWARE_MALLOC)
    if (ctp_fw_temp_buffer == NULL)
    {
        ctp_fw_temp_buffer = ab_malloc(write_length);
    }
#endif

    if ((write_buffer == NULL) || (write_length == 0) || ((write_addr + write_length) > 0x2800) || ctp_fw_temp_buffer == NULL)
    {

#if (SPT5113C_FIRMWARE_MALLOC)
        if (ctp_fw_temp_buffer != NULL)
        {
            ab_free(ctp_fw_temp_buffer);
            ctp_fw_temp_buffer = NULL;
        }
#endif
...
void spt5113c_start_work_with_scan(void)
{
    ...
    reg_temp = (APP_DEV_ADDR & 0x7F) << 1;
    spt5113c_i2c_write_16bit(ASU_DEV_ADDR, 0xC10E, &reg_temp, 1);

    reg_temp = 0x2F;
    spt5113c_i2c_write_16bit(ASU_DEV_ADDR, 0xC10F, &reg_temp, 1);

    spt5113c_i2c_read_16bit(ASU_DEV_ADDR, 0xC10D, &reg_temp, 1);
    reg_temp |= 0x01;
    spt5113c_i2c_write_16bit(ASU_DEV_ADDR, 0xC10D, &reg_temp, 1);
...
void spt5113c_frequency_set(uint16_t freq)
{
    spt5113c_frequency = freq;
}
...
static bool spt5113c_firmware_update(void)
{
    ...
    //reset
#if !UTE_DRV_CTP_SPT5113C_REUSE_SCREEN_RESET_SUPPORT
    spt5113c_reset();
#endif
...
#if APP_FUNC_FREQ_HOPPING
        spt5113c_start_work_with_freqset(spt5113c_frequency);
#else
