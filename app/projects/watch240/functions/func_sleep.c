...
//    sleep_data->fallAsSleepTime.hour =0;
//    sleep_data->fallAsSleepTime.min =0;
//    sleep_data->getUpSleepTime.hour =0;
//    sleep_data->getUpSleepTime.min = 0;
//
//    sleep_data->lightSleepMin= 480;
//    sleep_data->deepSleepMin= 480;
...
//    sleep_data->sleep_record[4].period = 20;
    ...
    if(sleep_data->totalSleepMin) ///是否有睡眠时长
    {
        ...
        compo_textbox_set_pos(txt, 75+32, 164+32);
        compo_textbox_set(txt, buf);
    }
    else
    {
        //snprintf(buf, sizeof(buf), "--");///* 总睡眠小时*/
        pic = compo_picturebox_create(frm, UI_BUF_I341001_9_SLEEP_NO_DATE00_BIN);
        compo_picturebox_set_pos(pic, 75+32, 205);
    }
    txt = compo_textbox_create(frm,strlen(i18n[STR_HOUR]));
    compo_textbox_set_align_center(txt, false);
    compo_textbox_set_location(txt, 141+12, 177+28/2, 48, 50);
    compo_textbox_set_autoroll_mode(txt, 0);
    compo_textbox_set(txt, i18n[STR_HOUR]);
    ...
    if(sleep_data->totalSleepMin)  ///是否有睡眠时长
    {
        ...
        txt = compo_textbox_create(frm,strlen(buf));
        compo_textbox_set_font(txt, UI_BUF_0FONT_FONT_NUM_64_BIN);
        compo_textbox_set_pos(txt, 203+32, 164+32);
        compo_textbox_set(txt, buf);
    }
    else
    {
        //snprintf(buf, sizeof(buf), "--");///* 总睡眠分钟*/
        pic = compo_picturebox_create(frm, UI_BUF_I341001_9_SLEEP_NO_DATE00_BIN);
        compo_picturebox_set_pos(pic, 203+38, 205);
    }
    txt = compo_textbox_create(frm,strlen(i18n[STR_MIN]));
    compo_textbox_set_align_center(txt, false);
    ...
    compo_textbox_set(txt, i18n[STR_MIN]);
//////////////////////////////////////////////////////////////////////
    pic = compo_picturebox_create(frm, UI_BUF_I341001_9_SLEEP_DATE_BG1_BIN);///* 画图背景*/
    compo_picturebox_set_pos(pic, GUI_SCREEN_CENTER_X, 311);
    ...
    {
        txt = compo_textbox_create(frm, strlen(i18n[STR_SLEEP_NOTIFY]));///* 无数据*/
        compo_textbox_set_location(txt, GUI_SCREEN_CENTER_X, 326, 300, 106);
        ...
        compo_textbox_set(txt, i18n[STR_SLEEP_NOTIFY]);
        if (widget_text_get_area(txt->txt).hei > 106)
        {
            compo_textbox_set_location(txt, GUI_SCREEN_CENTER_X, 310, 300, 108);
        }
...
#define first_y 451
    ...
    txt = compo_textbox_create(frm, strlen(i18n[STR_DEEP_SLEEP]));///* 深睡*/
    compo_textbox_set_align_center(txt, false);
    compo_textbox_set_location(txt, 35, first_y, 160, 40);
    compo_textbox_set(txt, i18n[STR_DEEP_SLEEP]);
    compo_textbox_set_forecolor(txt, make_color(120, 53, 252));
    pic = compo_picturebox_create(frm, UI_BUF_I341001_9_SLEEP_ICON_SLEEP_DEEP_SLEEP_BIN);
    compo_picturebox_set_pos(pic, 20, first_y + 16);

    txt = compo_textbox_create(frm, strlen(i18n[STR_LIGHT_SLEEP]));///* 浅睡*
    compo_textbox_set_align_center(txt, false);
    compo_textbox_set_location(txt, 35, first_y + height_spacing, 160, 40);
    compo_textbox_set(txt, i18n[STR_LIGHT_SLEEP]);
    compo_textbox_set_forecolor(txt, make_color(173, 134, 252));
    pic = compo_picturebox_create(frm, UI_BUF_I341001_9_SLEEP_ICON_SLEEP_LIGHT_SLEEP_BIN);
    compo_picturebox_set_pos(pic, 20, first_y + height_spacing + 16);

    txt = compo_textbox_create(frm, strlen(i18n[STR_EYE_MOVE_SLEEP]));///* 快速眼动*/
    compo_textbox_set_align_center(txt, false);
    compo_textbox_set_location(txt, 35, first_y + height_spacing*2, 160, 40);
    compo_textbox_set(txt, i18n[STR_EYE_MOVE_SLEEP]);
    compo_textbox_set_forecolor(txt, make_color(0, 183, 255));
    pic = compo_picturebox_create(frm, UI_BUF_I341001_9_SLEEP_ICON_SLEEP_REM_BIN);
    compo_picturebox_set_pos(pic, 20, first_y + height_spacing*2 + 16);

    txt = compo_textbox_create(frm, strlen(i18n[STR_ALWAKE_SLEEP]));///* 清醒*/
    compo_textbox_set_align_center(txt, false);
    compo_textbox_set_location(txt, 35, first_y + height_spacing*3, 160, 40);
    compo_textbox_set(txt, i18n[STR_ALWAKE_SLEEP]);
    compo_textbox_set_forecolor(txt, make_color(255, 135, 15));
    pic = compo_picturebox_create(frm, UI_BUF_I341001_9_SLEEP_ICON_SLEEP_SOBER_BIN);
    compo_picturebox_set_pos(pic, 20, first_y + height_spacing*3 + 16);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    compo_textbox_set(txt, buf);
//////////////////////////////////////////////////////////////////////////////////////////////////////
    uint16_t width = (318);    // 总宽度
    uint8_t fillRect_h = 30; // 绘制单个高度
    uint16_t startX = ((GUI_SCREEN_WIDTH - width) / 2);     // 相对坐标
    uint16_t fillRect_y = 312;  //先从浅睡眠中心点开始绘图
    ...
    if (sleep_data->totalSleepMin) //是否有睡眠时长
    {
        ...
        for (uint16_t i = 0; i < sleep_data->recordCnt; i++)
        {
            ...
            if (sleep_data->sleep_record[i].state == DEEP_SLEEP)
            {
                compo_shape_set_location(shape, startX + fillRect_x + (fillRect_w / 2), fillRect_y + 3*fillRect_h/2 + 2, fillRect_w, fillRect_h);
                compo_shape_set_color(shape, make_color(120, 53, 252));
            }
            else if (sleep_data->sleep_record[i].state == LIGHT_SLEEP)
            {
                compo_shape_set_location(shape, startX + fillRect_x + (fillRect_w / 2), fillRect_y + fillRect_h/2,   fillRect_w, fillRect_h);
                compo_shape_set_color(shape, make_color(173, 134, 252));
            }
            else if (sleep_data->sleep_record[i].state == AWAKE_SLEEP)
            {
                compo_shape_set_location(shape, startX + fillRect_x + (fillRect_w / 2), fillRect_y - 3*fillRect_h/2 - 5, fillRect_w, fillRect_h);
                compo_shape_set_color(shape, make_color(255, 135, 15));
            }
            else if (sleep_data->sleep_record[i].state == REM_SLEEP)
            {
                compo_shape_set_location(shape, startX + fillRect_x + (fillRect_w / 2), fillRect_y - fillRect_h/2 - 2,   fillRect_w, fillRect_h);
                compo_shape_set_color(shape, make_color(0, 183, 255));
            }
...
static void func_sleep_process(void)
{
    ...
    s32 dx, dy;
    s16 value = abs_s(f_sleep->ptm->move_offset) * 1000 / abs_s(f_sleep->ptm->down_spring_offset);
    compo_scroll_t* scroll = compo_getobj_byid(COMPO_ID_SCROLLBAR);
    compo_scroll_set_pos(scroll, 352, 78 - f_sleep->ptm->move_offset);
    compo_scroll_set_value(scroll, value);

    if (f_sleep->touch_flag)
    {
        f_sleep->tick = tick_get();
        widget_set_visible(scroll->scroll_bg, true);
        widget_set_visible(scroll->scroll_fg, true);
        f_sleep->touch_flag = false;
    }
    f_sleep->touch_flag = ctp_get_dxy(&dx, &dy);
    if(tick_check_expire(f_sleep->tick, 3000))
    {
