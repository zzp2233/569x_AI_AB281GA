...
#define CTP_CNT_LONG                     100                        //CTP长按时间阈值(CTP INT时钟个数)
...
static struct ctp_cb_t
{
    ...
    u8 sta;
    bool flag_last;
#if UTE_DRV_TOUCH_MULTI_CLICK_SUPPORT
    uint8_t click_count;              // 点击计数
    uint32_t click_time_cnt;          // 点击时间
    bool multi_click_pending;         // 多击待处理标志
#endif
} ctp_cb;
...
void ctp_msg_deal(bool press)
{
...
#if UTE_DRV_TOUCH_MULTI_CLICK_SUPPORT
                        // 取消多击待处理状态
                        if (ctp_cb.multi_click_pending)
                        {
                            ctp_cb.multi_click_pending = false;
                            ctp_cb.click_count = 0;
                        }
#endif
    ...
    else
    {
        if (ctp_cb.sta == CTP_STA_PRESS)
        {
            ...
            if (uteModuleGuiCommonIsAllowTouchMultiClickScreen())
            {
                ctp_cb.click_count ++;
                ctp_cb.multi_click_pending = true;
                ctp_cb.click_time_cnt = 0;
            }
            else
#endif
            {
...
#if UTE_DRV_TOUCH_MULTI_CLICK_SUPPORT
AT(.com_text.ctp)
void multi_click_5ms_handler(void)
{
    if (ctp_cb.multi_click_pending)
    {
        if (ctp_cb.click_time_cnt >= (UTE_DRV_TOUCH_MULTI_CLICK_TIMEOUT_MS / 5))
        {
            if (ctp_cb.click_count == 2)
            {
...
#elif (CTP_SELECT == CTP_SPT5113C)
    CTP_IIC->sfr->IICxCON0 = BIT(10) | (5 << 4) | BIT(0);
#else
