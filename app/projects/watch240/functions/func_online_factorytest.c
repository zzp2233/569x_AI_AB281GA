...
static compo_form_t *func_online_factory_test_check_light_create(void)
{
    ...
    compo_textbox_set(textbox, "Leakage test: Green light");
...
static void func_online_factory_test_check_light_process(void)
{
    ...
    {
        compo_textbox_set(txt_vcxx_title, "Leakage test: red light");
    }
    else if (uteModuleFactoryTestGetCheckLightMode() == FACTORY_VCXX_TEST_MODE_INFRARED)
    {
        compo_textbox_set(txt_vcxx_title, "Leakage test: infrared");
    }
    else
    {
        compo_textbox_set(txt_vcxx_title, "Leakage test: Green light");
    }
...
static compo_form_t *func_online_factory_test_battery_create(void)
{
    ...
    memset(str_buff, 0, sizeof(str_buff));
    snprintf(str_buff, sizeof(str_buff),"Start Time:%04d-%02d-%02d %02d:%02d:%02d",testData->u.battery.startTime.year,testData->u.battery.startTime.month,testData->u.battery.startTime.day,testData->u.battery.startTime.hour,testData->u.battery.startTime.min,testData->u.battery.startTime.sec);
    textbox = compo_textbox_create(frm, 40);
    ...
    memset(str_buff, 0, sizeof(str_buff));
    snprintf(str_buff, sizeof(str_buff),"Current time:%04d-%02d-%02d %02d:%02d:%02d",time.year,time.month,time.day,time.hour,time.min,time.sec);
    textbox = compo_textbox_create(frm, 40);
    compo_textbox_set(textbox, str_buff);
    if(UTE_DRV_SCREEN_SHAPE)
    {
    ...
    memset(str_buff, 0, sizeof(str_buff));
    snprintf(str_buff, sizeof(str_buff),"Already running %02d:%02d:%02d",(uint16_t)(testData->u.battery.runningTimeSecond / 3600), (uint16_t)((testData->u.battery.runningTimeSecond % 3600) / 60), (uint16_t)(testData->u.battery.runningTimeSecond % 60));
    textbox = compo_textbox_create(frm, 40);
    if(UTE_DRV_SCREEN_SHAPE)
    {
    ...
    memset(str_buff, 0, sizeof(str_buff));
    snprintf(str_buff, sizeof(str_buff),"Start voltage:%dmV",testData->u.battery.startBatteryVoltage);
    textbox = compo_textbox_create(frm, 40);
    if(UTE_DRV_SCREEN_SHAPE)
    {
    ...
    memset(str_buff, 0, sizeof(str_buff));
    snprintf(str_buff, sizeof(str_buff),"Current Voltage:%dmV",testData->u.battery.currBatteryVoltage);
    textbox = compo_textbox_create(frm, 40);
    if(UTE_DRV_SCREEN_SHAPE)
    {
    ...
    memset(str_buff, 0, sizeof(str_buff));
    snprintf(str_buff, sizeof(str_buff),"Start battery level:%d%%",testData->u.battery.startBatteryLvl);
    textbox = compo_textbox_create(frm, 40);
    if(UTE_DRV_SCREEN_SHAPE)
    {
    ...
    memset(str_buff, 0, sizeof(str_buff));
    snprintf(str_buff, sizeof(str_buff),"Current battery level:%d%%",testData->u.battery.currBatteryLvl);
    textbox = compo_textbox_create(frm, 40);
    if(UTE_DRV_SCREEN_SHAPE)
    {
...
static compo_form_t * func_online_factory_test_ageing_create(void)
{
    ...
    memset(str_buff, 0, sizeof(str_buff));
    snprintf(str_buff, sizeof(str_buff),"Battery:%s %d%%",uteDrvBatteryCommonGetChargerStatus() == BAT_STATUS_NO_CHARGE ? "Uncharged" : "Charging", uteDrvBatteryCommonGetLvl());
    textbox = compo_textbox_create(frm, 40);
    compo_textbox_set(textbox, str_buff);
    compo_textbox_set_pos(textbox, GUI_SCREEN_CENTER_X, GUI_SCREEN_HEIGHT * 1 / 6);
    ...
    memset(str_buff, 0, sizeof(str_buff));
    snprintf(str_buff, sizeof(str_buff),"Heart:%s %d bpm",uteModuleHeartIsWear() ? "Already worn" : "Not worn", testData->u.aging.heart);
    textbox = compo_textbox_create(frm, 40);
    compo_textbox_set(textbox, str_buff);
    compo_textbox_set_pos(textbox, GUI_SCREEN_CENTER_X, GUI_SCREEN_HEIGHT * 2 / 6);
    ...
    textbox = compo_textbox_create(frm, 40);
    memset(str_buff, 0, sizeof(str_buff));
    snprintf(str_buff, sizeof(str_buff),"Already running %02d:%02d:%02d",(uint16_t)(testData->u.aging.runningTimeSecond / 3600), (uint16_t)((testData->u.aging.runningTimeSecond % 3600) / 60), (uint16_t)(testData->u.aging.runningTimeSecond % 60));
    compo_textbox_set_pos(textbox, GUI_SCREEN_CENTER_X, GUI_SCREEN_HEIGHT * 5 / 6);
    compo_textbox_set(textbox, str_buff);
    compo_setid(textbox, COMPO_ID_TXT_RUNNING_TIME);
...
{
    compo_form_t *frm = compo_form_create(true);
    compo_textbox_t *textbox = compo_textbox_create(frm, strlen("Microphone and speaker testing"));
    compo_textbox_set(textbox, "Microphone and speaker testing");
    ...
    compo_shape_set_location(shape, GUI_SCREEN_CENTER_X, GUI_SCREEN_HEIGHT * 2 / 5, GUI_SCREEN_WIDTH / 2+20, GUI_SCREEN_HEIGHT / 6);
    ...
    compo_shape_set_location(shape, GUI_SCREEN_CENTER_X, GUI_SCREEN_HEIGHT * 3 / 5, GUI_SCREEN_WIDTH / 2+20, GUI_SCREEN_HEIGHT / 6);
    compo_shape_set_radius(shape, 6);
...
compo_form_t *func_online_factory_test_form_create(void)
{
    ...
    compo_textbox_set(textbox, "This test is not supported!");
    compo_textbox_set_pos(textbox, GUI_SCREEN_CENTER_X, GUI_SCREEN_HEIGHT * 1 / 6);
    compo_setid(textbox, COMPO_ID_TXT_TITLE);
    return frm;
}
