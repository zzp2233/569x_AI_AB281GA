...
#if LE_ADV0_EN
#define CMD_TX_PR                   200         //命令最小发送间隔
#define CMD_TIMEOUT_TICK            200         //命令超时时间(ms)
...
#define BREATH_TIMEOUT_CNT          3           //心跳包超时阈值次数
#else
#define BREATH_TIMEOUT_CNT          2           //心跳包超时阈值次数
#endif
#endif
...
{
    while (!tick_check_expire(uart_cb.ticks, 5))    //接收数据后等待一段时间再发送
    {
        WDT_CLR();
    }
