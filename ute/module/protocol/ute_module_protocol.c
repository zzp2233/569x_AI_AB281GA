...
void uteAppCmdVerificationBindingNotify()
{
    ...
    if (!uteModuleAppBindingGetBindingNotify())
    {
        ...
        uteTaskGuiStartScreen(FUNC_BINDING_NOTIFY, 0, __func__);
    ...
    if (length>=8)
    {
        if(receive[1]==0x00)/*!震动控制 zn.zeng, 2021-08-21  */
        {
            if ((receive[4] == 0x01) && (receive[5] == 0x02) && (receive[6] == 0x07) && (receive[7] == 0x01))
            {
                ...
                if(0)
                {}
#if 0//UTE_MODULE_SCREENS_SCREEN_SAVER_SUPPORT
                else if(uteModuleGuiCommonIsInScreenSaver())
                {
                    uteTaskGuiStartScreen(uteModuleGuiCommonGetTempScreenId(), 0, __func__);
                }
#endif
                else
                {
...
#endif
#ifdef SCREEN_TITLE_MULTIPLE_FARSI_LANGUAGE_SUPPORT //波斯语（伊朗）
        response[15] |= SCREEN_TITLE_MULTIPLE_FARSI_LANGUAGE_SUPPORT << 7;
#endif
...
static uint8_t weathercrcvalue = 0;
static uint16_t weatherReceiveLen = 0;
#if !APP_MODULE_RECEIVE_SAVE_TO_FILESYS_SUPPORT
static uint8_t weatherReceiveBuf[9+120+48];//24小时+最多6天+当天
#endif
#endif
...
void uteModuleProtocolSetWeatherData(uint8_t*receive,uint8_t length)
{
...
#if APP_MODULE_WEATHER_HOURS_INFO_SUPPORT
    if (receive[1] >= 0x01 && receive[1] < 0xF0)
    {
        if (receive[1] == 0x01)
        {
            weathercrcvalue = 0;
...
#if APP_MODULE_RECEIVE_SAVE_TO_FILESYS_SUPPORT
        if (receive[1] == 0x01)
            uteModuleSystemTimeSaveReceiveData(&receive[2], length - 2, 0x03, true);
        else
            uteModuleSystemTimeSaveReceiveData(&receive[2], length - 2, 0x03, false);
#else
        if (receive[1] == 0x01)
            memset(&weatherReceiveBuf[0], 0, sizeof(weatherReceiveBuf));
        memcpy(&weatherReceiveBuf[weatherReceiveLen], &receive[2], (length - 2));
#endif
        uteModuleCrc8Bit(&weathercrcvalue, &receive[1], (length - 1));
        weatherReceiveLen += (length - 2);
    }
    else if (receive[1] == 0xFD)
    {
        uint8_t response[10];
        ...
        response[0] = receive[0];
        if (receive[2] == weathercrcvalue)
        {
            response[1] = 0xFD;
#if APP_MODULE_RECEIVE_SAVE_TO_FILESYS_SUPPORT
            {
                uint8_t path[30];
                uint8_t *readBuff = uteModulePlatformMemoryAlloc(200);
                void *file;
                int32_t fileSize = 0;

                memset(&path[0], 0, 30);
                sprintf((char *)&path[0], "%s/%02d", UTE_MODULE_FILESYSTEM_PROTOCAL_RECEIVE_DIR, 0x03);
                if (uteModuleFilesystemOpenFile(&path[0], &file, FS_O_RDONLY))
                {
                    ...
                    UTE_MODULE_LOG(UTE_LOG_PROTOCOL_LVL, "%s,fileSize=%d %d", __func__, fileSize, weatherReceiveLen);
                    uteModuleFilesystemSeek(file, 0, FS_SEEK_SET);
                    uteModuleFilesystemReadData(file, &readBuff[0], fileSize);
                    uteModuleFilesystemCloseFile(file);
                    if (weatherReceiveLen == fileSize && weatherReceiveLen >= 169)
                        uteModuleWeatherSaveAllWeatherInfo(readBuff, weatherReceiveLen);
                }
                uteModulePlatformMemoryFree(readBuff);
            }
#else
            uteModuleWeatherSaveAllWeatherInfo(weatherReceiveBuf, weatherReceiveLen);
#endif
        }
        else
        {
    ...
    else if (receive[1] == 0x02 || receive[1] == 0x03 || receive[1] == 0x04 || receive[1] == 0x05
            || receive[1] == 0x06 || receive[1] == 0x07 || receive[1] == 0x08 || receive[1] == 0x09)
    {
        uint8_t i = 0;
        uint8_t index = 0;
        ...
        for (i = 0; i < 3 ; i ++)
        {
            if (receive[i * 5 + 2] & 0x80)
                data.hourTemperature[index * 3 + i] = -(receive[i * 5 + 2] & (~0x80));
            else
                data.hourTemperature[index * 3 + i] = receive[i * 5 + 2];
            data.hourHumidity[index * 3 + i] = receive[i * 5 + 3];
            data.hourUltraviolet[index * 3 + i] = receive[i * 5 + 4];
            data.hourWeather[index * 3 + i] = (receive[i * 5 + 5] << 8) | receive[i * 5 + 6];
            if (index * 3 + i == 0)
            {
                data.dayHumidity[0] = receive[i * 5 + 3];
                data.nightHumidity[0] = receive[i * 5 + 3];
                data.dayUltraviolet[0] = receive[i * 5 + 4];
                data.nightUltraviolet[0] = receive[i * 5 + 4];
            }
        }
        uteModuleSystemtimeGetTime(&data.updateTime);
        data.currHour = data.updateTime.hour;
    }
    else if (receive[1] == 0x0A || receive[1] == 0x0B)
    {
        uint8_t j = 0;
        uint8_t index0 = 0;
        ...
        for (j = 0; j < 2; j ++)
        {
            data.DayWeather[index0 * 2 + j + 1] = (receive[2 + 8 * j] << 8) | receive[2 + 8 * j + 1];
            if (receive[2 + 8 * j + 2] & 0x80)
            {
                data.dayTemperatureMax[index0 * 2 + j + 1] = -(receive[2 + 8 * j + 2] & (~0x80));
            }
            else
            {
                data.dayTemperatureMax[index0 * 2 + j + 1] = receive[2 + 8 * j + 2];
            }
            if (receive[2 + 8 * j + 3] & 0x80)
            {
                data.dayTemperatureMin[index0 * 2 + j + 1] = -(receive[2 + 8 * j + 3] & (~0x80));
            }
            else
            {
                data.dayTemperatureMin[index0 * 2 + j + 1] = receive[2 + 8 * j + 3];
            }
            data.dayHumidity[index0 * 2 + j + 1] = receive[2 + 8 * j + 4];
            data.nightHumidity[index0 * 2 + j + 1] = receive[2 + 8 * j + 5];
            data.dayUltraviolet[index0 * 2 + j + 1] = receive[2 + 8 * j + 6];
            data.nightUltraviolet[index0 * 2 + j + 1] = receive[2 + 8 * j + 7];
        }
    }
    else if (receive[1] == 0x0C)
    {
        data.DayWeather[5] = (receive[2] << 8) | receive[3];
        if (receive[4] & 0x80)
        {
            data.dayTemperatureMax[5] = -(receive[4] & (~0x80));
        }
        else
        {
            data.dayTemperatureMax[5] = receive[4];
        }
        if (receive[5] & 0x80)
        {
            data.dayTemperatureMin[5] = -(receive[5] & (~0x80));
        }
        else
        {
            data.dayTemperatureMin[5] = receive[5];
        }
        data.dayHumidity[5] = receive[6];
        data.nightHumidity[5] = receive[7];
        data.dayUltraviolet[5] = receive[8];
        data.nightUltraviolet[5] = receive[9];
        uteModuleSystemtimeGetTime(&data.updateTime);
...
*@date       2022-03-30
*/
void uteModuleProtocolSetDrinkWater(uint8_t*receive,uint8_t length)
{
#if UTE_MODULE_DRINK_WATER_NOTIFY_SCREEN_SUPPORT
    ute_module_drink_water_data_t param;
    memset(&param,0,sizeof(ute_module_drink_water_data_t));
    if(receive[1]==0x01)
    {
        param.isOpen = true;
        param.min = receive[2];
        if(length >7)
        {
            ...
            param.startNoonSleepHour= receive[12];
            param.startNoonSleepMin= receive[13];
            param.endNoonSleepHour= receive[14];
            param.endNoonSleepMin = receive[15];
        }
    }
    else
    {
    ...
    response[1] = receive[1];
    uteModuleProfileBleSendToPhone(&response[0],2);
    uteModuleDrinkWaterSaveParam(&param);
#endif
}
...
//            msgbox((char*)i18n[STR_LOW_BATTERY], NULL, NULL, MSGBOX_MODE_BTN_NONE, MSGBOX_MSG_TYPE_LOW_BATTERY);
    ...
    else if (receive[1] == 0x01)
    {
        ...
        else if (receive[2] == 0x01)
        {
            ...
            uteModulePlatformSendMsgToUteApplicationTask(MSG_TYPE_MODULE_SPORT_GPS_LOCATION_STATE, 1);
            uteModuleProfileBleSendToPhone(&receive[0], 3);
        }
        else
        {
            //gps location fail
            uteModulePlatformSendMsgToUteApplicationTask(MSG_TYPE_MODULE_SPORT_GPS_LOCATION_STATE, 0);
            uteModuleProfileBleSendToPhone(&receive[0], 3);
        }
    }
    else if (receive[1] == 0x02)
    {
        ...
        latitude = (receive[2] << 24) | (receive[3] << 16) | (receive[4] << 8) | receive[5];
        longitude = (receive[6] << 24) | (receive[7] << 16) | (receive[8] << 8) | receive[9];
        speed = (receive[10] << 8) | receive[11];
        curDistance = (receive[12] << 24) | (receive[13] << 16) | (receive[14] << 8) | receive[15];
        totalDistance = (receive[16] << 24) | (receive[17] << 16) | (receive[18] << 8) | receive[19];
        response[0] = receive[0];
        response[1] = receive[1];
        uteModuleSportMoreSportsGPSDataHandler(0, speed, latitude, longitude, curDistance, totalDistance);
        uteModulePlatformSendMsgToUteApplicationTask(MSG_TYPE_MODULE_SPORT_GPS_LOCATION_STATE, 2);
        uteModuleProfileBleSendToPhone(&response[0], 2);
    }
    /*else if (receive[1] == 0x03)
    {
        ...
        curDistance = (receive[2] << 24) | (receive[3] << 16) | (receive[4] << 8) | receive[5];
        totalDistance = (receive[6] << 24) | (receive[7] << 16) | (receive[8] << 8) | receive[9];
        speed = (receive[10] << 8) | receive[11];
        response[0] = receive[0];
        response[1] = receive[1];
        uteModuleSportMoreSportsGPSDataHandler(1, speed, curDistance, totalDistance);
        uteModulePlatformSendMsgToUteApplicationTask(MSG_TYPE_MODULE_SPORT_GPS_LOCATION_STATE, 3);
        uteModuleProfileBleSendToPhone(&response[0], 2);
    }*/
#endif
}
...
void uteModuleProtocolTodayTargetCtrl(uint8_t*receive,uint8_t length)
{
    ...
    else if (receive[1] == 0x06)//活动时长目标
    {
        if (receive[2] == 0x00)
        {
            targetNotifyData.isExerciseTimeTargetNotifyOpen = false;
            targetNotifyData.isExerciseTimeTargetHasNotify = false;
        }
        else if (receive[2] == 0x01)
        {
            targetNotifyData.isExerciseTimeTargetNotifyOpen = true;
        }
        if ((uteModuleSportGetCurrDayExerciseData() / 60) < ((receive[3] << 8) | receive[4]))
        {
            //当天已经提醒，那么需要设置比之前大才要提醒
            targetNotifyData.isExerciseTimeTargetHasNotify = false;
            targetNotifyData.isTodayAllTargetHasNotify = false;
        }
        targetNotifyData.exerciseTimeTargetSetMin = (receive[3] << 8) | receive[4];
        uteModuleProfileBleSendToPhone(&receive[0],length);
    }
#endif
...
void uteModuleProtocolSyncAddressBook(uint8_t*receive,uint8_t length)
{
    ...
    else if(receive[1] == 0xBA)
    {
        uteModuleCallDeleteAllBookData();
        uteModuleProfileBleSendToPhone(&receive[0],2);
    }
    ...
    else if (receive[1] == 0xAF)
    {
#if APP_MODULE_CALL_SOS_SWITCH_SUPPORT
        ute_custom_setting_info_t cutsomSetting;

        uteApplicationCommonGetCustomSettingInfo(&cutsomSetting);
        cutsomSetting.isSOSDialEnable = receive[2];
        uteApplicationCommonSetCustomSettingInfo(&cutsomSetting);
        uteModuleProfileBleSendToPhone(&receive[0], 3);
#endif
    }
#endif
...
void uteModuleProtocolWomenMenstrualCycle(uint8_t*receive,uint8_t length)
{
    ...
    uteModuleMenstrualCycleGetParam(&param);
    switch(receive[1])
    {
        case 0x01: //open
        {
            param.reminderSwitch = true;
            uint16_t year = receive[2]<<8|receive[3];
            if(year >= DEFAULT_SYSTEM_TIME_YEAR)
            {
        ...
        case 0xac:
        {
            param.isMenstrualRemindOpen = receive[2];
            param.isMenstrualEndRemindOpen = receive[3];
            param.isOvulationRemindOpen = receive[4];
            param.isOvulationEndRemindOpen = receive[5];
        }
        break;
        default :
            break;
...
void uteModuleProtocolSportsTargetSelect(uint8_t*receive,uint8_t length)
{
    ...
    else if (receive[1] == 0x03)//多运动目标时长
    {
        ...
        {
            uteModuleSportTargetSetTimeGoal(sportTimeTargetSec);
            uteModuleSportTargetSetIsNoTarget(false);
        }
    }
    else if (receive[1] == 0x04)//多运动目标kcal
    {
        ...
        {
            uteModuleSportTargetSetKcalGoal(sportKcalTarget);
            uteModuleSportTargetSetIsNoTarget(false);
        }
    }
    else if (receive[1] == 0x05)//最大心率值设置
    {
#if APP_MODULE_HEART_MAX_HEARTRATE_SET_SUPPORT
        if (receive[2] > 0)
        {
            uteModuleHeartSetMaxValue(receive[2]);
        }
#endif
    }
    response[0] = receive[0];
...
void uteModuleProtocolAppBindingCtrl(uint8_t *receive, uint8_t length)
{
    ...
    {
#if APP_RANDCODE_USERID_SUPPORT
        uint32_t randCode = 0;
        newBindingUserId = (receive[2] << 24) | (receive[3] << 16) | (receive[4] << 8) | receive[5];
        if (length >= 10)
            randCode = (receive[6] << 24) | (receive[7] << 16) | (receive[8] << 8) | receive[9];
        uteModuleAppBindingSetNewUserId(newBindingUserId);
        uteModulePlatformFlashNorRead((uint8_t *)&snData, UTE_USER_PARAM_ADDRESS, size);
        UTE_MODULE_LOG(UTE_LOG_PROTOCOL_LVL, "%s,newBindingUserId=%d,userId=0x%08x randCode=%d QR=%d", __func__, newBindingUserId, snData.userId, randCode, uteModuleAppBindingGetQRcodeRandom());
#if UTE_QRCODE_BINDING_NO_CONFIRM_SUPPORT // 扫码绑定不需要确认
        if ((newBindingUserId == snData.userId) || (randCode == uteModuleAppBindingGetQRcodeRandom() && randCode > 0 && uteModuleAppBindingGetQRcodeRandom() > 0))
#else
        if (newBindingUserId == snData.userId)
#endif
        {
#if UTE_QRCODE_BINDING_NO_CONFIRM_SUPPORT
            if (randCode == uteModuleAppBindingGetQRcodeRandom() && randCode > 0 && uteModuleAppBindingGetQRcodeRandom() > 0)
            {
                uteModulePlatformFlashNorRead((uint8_t *)&snData, UTE_USER_PARAM_ADDRESS, size);
                uteModulePlatformFlashNorErase(UTE_USER_PARAM_ADDRESS);
        ...
        else
        {
            ...
            uteModuleProfileBleSendToPhone(&response[0], 3);
#if APP_MODULE_ORAIMO_BINDING_SUPPORT // 一台手机绑定以后，回复出厂设置才可以绑定其他手机（传音逻辑）receive[10]：00表示绑定，01代表回连
            if ((snData.userId == 0xffffffff) && (receive[10] == 0x00))
#endif
            {
                ...
                uint8_t ret = uteModuleGuiCommonStartMsgbox(i18n[STR_WETH_BINDING], NULL, NULL, MSGBOX_MODE_BTN_YESNO, MSGBOX_MSG_TYPE_BINDING, 0, false);
                if (ret == MSGBOX_RES_OK)
                {
                    uteModuleAppBindingStrat();
                }
                else
                {
                    ...
                    delay_5ms(100);
                    uteModulePlatformBleDisconnect(connectStatus.connId);
                    uteTaskGuiStartScreen(FUNC_BINDING_NOTIFY, 0, __func__);
                }
#endif
            }
...
*@date       2022-03-24
*/
void uteModuleProtocolAlarmCtrl(uint8_t *receive, uint8_t length)
{
#if UTE_MODULE_LOCAL_ALARM_SUPPORT
    uteModuleSystemtimeAlarmCmd(receive, length);
#endif
}
...
void uteModuleProtocolCustomRemindSetting(uint8_t *receive, uint8_t length)
{
    uint8_t response[20];
    uint16_t receiveLenth = 0;
    static uint8_t testReceivecrc = 0;//for test
    ...
    if (receive[1] == 0xAA)
    {
        if (uteModuleSystemtimeGetRemindTotalCnt() == 0)
        {
            response[2] = 0xFF;
            uteModuleProfileBle50SendToPhone((uint8_t *)&response[0], 3);
        }
        else
        {
            uteModuleSystemtimeSetSendCustomRemindType(receive[1]);
            uteModuleSystemtimeSendAllCustomRemindData();
        }
    }
    else if (receive[1] == 0xAC)
    {
        if (uteModuleSystemtimeGetSupportRemindSetting() == true)
            response[2] = 0x01;
        else
            response[2] = 0x00;
        uteModuleProfileBle50SendToPhone((uint8_t *)&response[0], 3);
    ...
    else if (receive[1] == 0xFA)
    {
        if (receive[2] == 0x01)
        {
            uteModuleSystemtimeSetSupportRemindSetting(true);
        }
        else
        {
            uteModuleSystemtimeSetSupportRemindSetting(false);
        }
        response[2] = receive[2];
        uteModuleProfileBle50SendToPhone((uint8_t *)&response[0], 3);
    }
    else if (receive[1] == 0xFB)
    {
        if (receive[2] == 0xFD)
        {
            if (receive[3] == testReceivecrc)
            {
                response[2] = 0xFD;
                response[3] = testReceivecrc;
#if APP_MODULE_RECEIVE_SAVE_TO_FILESYS_SUPPORT
                uteModuleSystemtimeProcessRemindData();
#else
                //todo
#endif
                uteModuleProfileBle50SendToPhone((uint8_t *)&response[0], 4);
            }
            else
            {
                response[1] = 0xFF;
                uteModuleProfileBle50SendToPhone((uint8_t *)&response[0], 2);
            }
        }
        else
        {
            ...
            {
                testReceivecrc = 0;
                uteModuleSystemtimeSetCustomRemindLenth(length - 3);
#if APP_MODULE_RECEIVE_SAVE_TO_FILESYS_SUPPORT
                uteModuleSystemTimeSaveReceiveData(&receive[3], length - 3, 0x04, true);
#else
                //todo
#endif
            }
            ...
            {
                receiveLenth = uteModuleSystemtimeGetCustomRemindLenth();
                receiveLenth += length - 3;
                uteModuleSystemtimeSetCustomRemindLenth(receiveLenth);
#if APP_MODULE_RECEIVE_SAVE_TO_FILESYS_SUPPORT
                uteModuleSystemTimeSaveReceiveData(&receive[3], length - 3, 0x04, false);
#else
                //todo
#endif
            }
            ...
            {
                testReceivecrc ^= receive[i + 2];
                UTE_MODULE_LOG(UTE_LOG_PROTOCOL_LVL, "%s,receiveCrc = %02x,receive[%d]=%02x", __func__,testReceivecrc,i+2,receive[i+2]);
            }
            response[2] = receive[2];
            uteModuleProfileBle50SendToPhone((uint8_t *)&response[0], 3);
        }
    ...
    {
        uteModuleSystemTimeResetRemindData();
        uteModuleProfileBle50SendToPhone((uint8_t *)&response[0], 2);
    }
}
#endif
...
*@date       2022-11-18
*/
void uteModuleProtocolGetExerciseData(uint8_t *receive, uint8_t length)
{
#if APP_MODULE_SPORT_EXERCISE_TIME_SUPPORT
    ute_module_systemtime_time_t time;
    ...
    uteModuleSportStartSendExerciseTimeHistoryData(time);
#endif
}
...
*@date       2022-11-18
*/
void uteModuleProtocolSetBTDisconnectNotify(uint8_t *receive, uint8_t length)
{
#if APP_MODULE_BT_DISCONNECT_SWITCH_SUPPORT
    uint8_t response[20];
    ute_custom_setting_info_t cutsomSetting;
    ...
    {
        response[1] = receive[1];
        if (cutsomSetting.isBTDisconnectNotify)
            response[2] = 0x01;
        else
            response[2] = 0x00;
        uteModuleProfileBleSendToPhone((uint8_t *)&response[0], 3);
    ...
    {
        response[1] = receive[1];
        cutsomSetting.isBTDisconnectNotify = receive[1];
        uteApplicationCommonSetCustomSettingInfo(&cutsomSetting);
        uteModuleProfileBleSendToPhone((uint8_t *)&response[0], 2);
    }
#endif
...
*@date       2022-11-18
*/
void uteModuleProtocolCustomSetting(uint8_t *receive, uint8_t length)
{
#if APP_MODULE_FEMALE_SET_SUPPORT
    uint8_t response[20];
    ute_custom_setting_info_t cutsomSetting;
    ...
    {
        response[2] = receive[2];
        if (receive[2] == 0x01)//女性健康
        {
            if (cutsomSetting.femaleSet)
                response[3] = 0x01;
            else
                response[3] = 0x00;
            uteModuleProfileBleSendToPhone((uint8_t *)&response[0], 4);
    ...
    else if (receive[1] == 0x01)//女性健康
    {
        response[2] = receive[2];
        cutsomSetting.femaleSet = receive[2];
        uteApplicationCommonSetCustomSettingInfo(&cutsomSetting);
        uteModuleProfileBleSendToPhone((uint8_t *)&response[0], 3);
    }
#endif
...
*@date       2022-09-28
*/
void uteModuleProtocolWatchSetCtrl(uint8_t *receive, uint8_t length)
{
#if APP_MODULE_DIAL_SET_WATCH_SUPPORT
    uint8_t response[20];
    uint8_t set_watch_index = 0;
    ...
    if (receive[1] == 0xFA)
    {
        uteModuleWatchOnlineSetInfo(&response[2]);
        uteModuleProfileBleSendToPhone((uint8_t *)&response[0], 20);
    }
    else if (receive[1] == 0xFB)
    {
        set_watch_index = receive[2];
        response[2] = receive[2];
        if (set_watch_index > UTE_MODULE_SCREENS_WATCH_CNT_MAX)
        {
            if (uteModuleWatchOnlineGetVailWatchCnt())
            {
                uteModuleGuiCommonSetCurrWatchIndex(set_watch_index);
            }
            else
            {
                uteModuleGuiCommonSetCurrWatchIndex(DEFAULT_WATCH_INDEX);
            }
            // 如果当前在表盘界面并且处于亮屏状态，需要更新屏幕
            if (uteModuleGuiCommonGetCurrentScreenId() == UTE_MOUDLE_SCREENS_WATCHMAIN_ID, 0, __func__)
            {
...
const ute_module_protocol_cmd_list_t uteModuleProtocolCmdList[]=
{
    ...
    {.privateCmd = CMD_ALARM_DATA,.publicCmd=CMD_ALARM_DATA,.function=uteModuleProtocolAlarmCtrl},
...
#endif
#if APP_CUSTOM_REMIND_SETTING_SUPPORT
    {.privateCmd = CMD_CUSTOM_REIMIND_SETTING,.publicCmd=CMD_CUSTOM_REIMIND_SETTING,.function=uteModuleProtocolCustomRemindSetting},
#endif
    {.privateCmd = CMD_GET_EXERCISE_DATA,.publicCmd = CMD_GET_EXERCISE_DATA,.function = uteModuleProtocolGetExerciseData},
    {.privateCmd = CMD_SET_BT_NOTIFY,.publicCmd = CMD_SET_BT_NOTIFY,.function = uteModuleProtocolSetBTDisconnectNotify},
    {.privateCmd = CMD_CUSTOM_SETTING,.publicCmd=CMD_CUSTOM_SETTING,.function=uteModuleProtocolCustomSetting},
    {.privateCmd = CMD_SET_WATCH,.publicCmd = CMD_SET_WATCH,.function = uteModuleProtocolWatchSetCtrl},
};
...
void uteModuleProtocolFromPhone(uint8_t *receive,uint8_t length,bool isPublic)
{
    ...
    if(uteApplicationCommonGetMtuSize == 20)
    {
        msg_enqueue(EVT_READY_EXCHANGE_MTU);
    }

#if UTE_USER_ID_FOR_BINDING_SUPPORT && !DUG_VCXX_HEART_SUPPORT
    ...
    else
    {
        ...
        else if (receive[0] == CMD_DEBUG_TEST_DATA)
        {
            UTE_MODULE_LOG(UTE_LOG_PROTOCOL_LVL,"%s,receice debug data",__func__);
        }
        else if (receive[0] == CMD_SET_ANCS_PARAM)
        {
            UTE_MODULE_LOG(UTE_LOG_PROTOCOL_LVL,"%s,receice ANCS data",__func__);
        }
        else
        {
...
void uteModuleProtocolReadExpandFunctionSupport(uint8_t *data,uint8_t size)
{
    ...
    data[1] = mtuSize&0xff;
    /*------------前两字节不允许变动----------------*/
#if UTE_MODULE_WORLD_CLOCK_QUARTER_TIME_SUPPORT
    data[19] |= 0x01;
#endif
#if APP_MODULE_HEART_RESTING_HEARTRATE_SUPPORT
    data[19] |= 0x02;
    ...
    data[19] |= 0x04;
#endif
#if APP_MODULE_SCREENS_DOWNKEY_SET_SUPPORT
    data[19] |= 0x08;
#endif
#if APP_MODULE_DIAL_SET_WATCH_SUPPORT
    data[19] |= 0x10;
    ...
    data[18] |= 0x20;
#endif
#if APP_MODULE_WEATHER_TYPE_EXTEND_SUPPORT
    data[18] |= 0x40;
#endif
    ...
    data[17] |= 0x80;
#endif
#if UTE_APP_MUSIC_CONTROL_SWITCH_SUPPORT
    data[16] |= 0x02;
#endif
