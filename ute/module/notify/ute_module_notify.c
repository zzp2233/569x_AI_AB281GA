...
void uteModuleNotifySaveNotifycationData(void)
{
    ...
    {
        ute_module_systemtime_time_t fileTime;
        memset(&fileTime, 0, sizeof(ute_module_systemtime_time_t));
        char *fileName = (char *)&dirInfo->filesName[i][0];
        if (strlen(fileName) >= 14)
        {
            fileTime.year = (fileName[0] - '0') * 1000 +
                            (fileName[1] - '0') * 100 +
                            (fileName[2] - '0') * 10 +
                            (fileName[3] - '0');

            fileTime.month = (fileName[4] - '0') * 10 +
                             (fileName[5] - '0');

            fileTime.day = (fileName[6] - '0') * 10 +
                           (fileName[7] - '0');

            fileTime.hour = (fileName[8] - '0') * 10 +
                            (fileName[9] - '0');

            fileTime.min = (fileName[10] - '0') * 10 +
                           (fileName[11] - '0');

            fileTime.sec = (fileName[12] - '0') * 10 +
                           (fileName[13] - '0');

            uint32_t firstFileTimestamp = fileTime.year * 100000000 +
                                          fileTime.month * 1000000 +
                                          fileTime.day * 10000 +
                                          fileTime.hour * 100 +
                                          fileTime.min;

            uint32_t currentTimestamp = time.year * 100000000 +
                                        time.month * 1000000 +
                                        time.day * 10000 +
                                        time.hour * 100 +
                                        time.min;

            if (firstFileTimestamp > currentTimestamp)
            {
                memset(&path[0], 0, 30);
                sprintf((char *)&path[0], "%s/%s", UTE_MODULE_FILESYSTEM_NOTIFY_DIR, fileName);
                UTE_MODULE_LOG(UTE_LOG_NOTIFY_LVL, "%s,del file with future time=%s", __func__, &path[0]);
...
void uteModuleNotifyNotifycationHandlerMsg(void)
{
    ...
    if(uteModuleGuiCommonGetPowerSavingModeOpen())  //省电模式下不接收通知消息
    {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL, "%s,saving mode is not allow notify", __func__);
        return;
    }
#endif
    ...
    if (ble_ancs_is_connected()) // ios消息是utf8编码，不需要处理
    {
        ...
        {
            uteModuleCharencodeGetUtf8String(&uteModuleNotifyData.currNotify.content[0], UTE_NOTIFY_MSG_CONTENT_MAX_SIZE - 4, &utf8StrTemp[0], &utf8StrLen);

            if (utf8StrLen < uteModuleNotifyData.currNotify.size)
            {
                strncat((char *)utf8StrTemp, "...", 4);
                utf8StrLen += 4;
            }
            memset(uteModuleNotifyData.currNotify.content, 0, UTE_NOTIFY_MSG_CONTENT_MAX_SIZE);
            memcpy(&uteModuleNotifyData.currNotify.content[0], &utf8StrTemp[0], utf8StrLen);
        }
    }
    else
    {
        ...
        bool ret = uteModuleCharencodeUnicodeConversionUtf8(uteModuleNotifyData.currNotify.content, uteModuleNotifyData.currNotify.size, utf8StrTemp, &utf8StrLen, UTE_NOTIFY_MSG_CONTENT_MAX_SIZE - 4);
        if (ret)
        {
            strncat((char *)utf8StrTemp, "...", 4);
            utf8StrLen += 4;
...
void uteModuleNotifyAncsStartPairHandlerMsg(void)
{
    ...
    if (!ble_ancs_is_connected() || (!uteModuleCallBtIsConnected() && uteModuleCallBtIsPowerOn()))
    {
        ...
        else if (bt_is_connected() && !ble_ancs_is_connected())
        {
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL, "%s,ancs pair", __func__);
            ble_ancs_start();
        }
        else
        {
            ...
            bsp_change_bt_mac();
            if (!bt_is_connected() && bt_get_curr_scan() == BT_STA_SCANNING) // IOS
            {
...
*@brief        删除多条历史消息数据
*@details
*@param[in] uint8_t *当前多条内容数组
*@author       Yin
*@date       2025-9-5
*/
void uteModuleNotifyDelMultiHistoryData(uint8_t *bufIndex, uint8_t size)
{
    uint8_t path[30];
    ute_module_filesystem_dir_t *dirInfo = (ute_module_filesystem_dir_t *)uteModulePlatformMemoryAlloc(sizeof(ute_module_filesystem_dir_t));
    uteModuleFilesystemLs(UTE_MODULE_FILESYSTEM_NOTIFY_DIR,dirInfo, NULL);
    ...
    {
        memset(&path[0],0,30);
        sprintf((char *)&path[0],"%s/%s",UTE_MODULE_FILESYSTEM_NOTIFY_DIR,&dirInfo->filesName[dirInfo->filesCnt - bufIndex[i] - 1][0]);
        uteModuleFilesystemDelFile((char *)&path[0]);
    }
