    ...
    while (att_num)
    {
        ...
        switch (index)
        {
            case BT_ID3_TITLE:
                ...
                static const u8 not_provided_en[] = {0x4E, 0x6F, 0x74, 0x20, 0x50, 0x72, 0x6F, 0x76, 0x69, 0x64, 0x65, 0x64};
                static const u8 not_provided_zh[] = {0xE6, 0x9C, 0xAA, 0xE6, 0x8F, 0x90, 0xE4, 0xBE, 0x9B};
                static const u8 unknown[] = {0x55, 0x6E, 0x6B, 0x6E, 0x6F, 0x77, 0x6E};
                if ((memcmp(&buf[offset], not_provided_en, sizeof(not_provided_en)) == 0) || (memcmp(&buf[offset], not_provided_zh, sizeof(not_provided_zh)) == 0) || (memcmp(&buf[offset], unknown, sizeof(unknown)) == 0))
                {
                    memset(sys_cb.title_buf, 0, sizeof(sys_cb.title_buf));
                    uteModuleMusicSetPlayerTitle(NULL, 0);
                }
                else
                {
                    uteModuleMusicSetPlayerTitle(&buf[offset], length);
                }
                uteModuleMusicSetPlayerArtist(NULL, 0);
