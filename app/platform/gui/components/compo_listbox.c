...
//移动相关控制
#define FOCUS_AUTO_TICK_EXPIRE              5                  //松手后自动对齐焦点单位时间(ms)
...
compo_listbox_t *compo_listbox_create(compo_form_t *frm, u32 style)
{
    ...
    for (i=0; i<LISTBOX_ITEM_CNT; i++)
    {
        ...
        widget_rect_t *shape_bgimg = widget_rect_create(item_page);
        ...
        listbox->shape_bgimg[i] = shape_bgimg;
        ...
        if(style == COMPO_LISTBOX_STYLE_SPORT_HISTORY_LIST)
        {
            widget_text_t *item_text3 = widget_text_create(item_page, 10);
            widget_text_t *item_text4 = widget_text_create(item_page, 10);
            listbox->item_text3[i] = item_text3;
            listbox->item_text4[i] = item_text4;
        }
    }
...
//                font_w = 76;
            ...
            else
            {
                ...
                if (listbox->style == COMPO_LISTBOX_STYLE_TITLE_TWO_TEXT || listbox->style == COMPO_LISTBOX_STYLE_TITLE_TWO_TEXT_CIRCLE || listbox->style ==COMPO_LISTBOX_STYLE_SPORT_LIST_BY_CHUANYIN ||listbox->style == COMPO_LISTBOX_STYLE_SPORT_HISTORY_LIST)
                {
...
//                    int icon_x = listbox->item_width - area.wid;
        ...
        else
        {
            ...
            else if(listbox->style == COMPO_LISTBOX_STYLE_SPORT_LIST_BY_CHUANYIN)
            {

                widget_set_location(listbox->item_icon[i], icon_x, listbox->line_height / 3,84,84);
                font_x = listbox->item_width - 60;
                font_y = listbox->line_height / 4;
                widget_set_pos(listbox->item_icon2[i], font_x, listbox->line_height / 3);
                font_x = icon_x - listbox->icon_area.wid/2;
                font_y = listbox->line_height / 2 + 20;
                widget_set_location(listbox->item_text[i], font_x, font_y, 300, 50);
                widget_text_set_wordwrap(listbox->item_text[i],false);
                font_y = listbox->line_height / 2 + 20 + 40;
                widget_set_location(listbox->item_text2[i], font_x, font_y, 300, listbox->line_height/2);
                widget_text_set_color(listbox->item_text2[i],make_color(0x97,0xff,0x00));
            }
            else if(listbox->style == COMPO_LISTBOX_STYLE_SPORT_HISTORY_LIST)
            {
                icon_x = 24;
                widget_set_location(listbox->item_icon[i], icon_x, listbox->line_height_total / 2,84,84);
                font_x = listbox->item_width - 60;
                font_y = listbox->line_height / 2;
                widget_set_pos(listbox->item_icon2[i], 310, listbox->line_height_total / 2);
                font_x = icon_x + listbox->icon_area.wid/2 + 20;
                font_y = listbox->line_height / 2 - 30;
                widget_set_location(listbox->item_text[i], font_x, font_y, 200, listbox->line_height/2);//    widget_set_location(listbox->item_text[i], icon_x/2, listbox->line_height - font_height, font_w, listbox->line_height);
                font_y = listbox->line_height / 2 + 20;
                widget_set_location(listbox->item_text2[i], font_x, font_y, font_w, listbox->line_height/2);//    widget_set_location(listbox->item_text[i], icon_x/2, listbox->line_height - font_height, font_w, listbox->line_height);
            }
            else
            {
 ...
 * @brief 设置列表框背景(形状绘图)
 * @param[in] listbox : 列表指针
 * @param[in] x : 形状x坐标（中心点）
 * @param[in] y : 形状y坐标（中心点）
 * @param[in] width : 形状宽度
 * @param[in] height : 形状高度
 * @param[in] r : 形状导角（最大为宽/高的一半，即圆形）
 * @param[in] alpha : 形状透明度
 * @param[in] color : 形状颜色
 **/
void compo_listbox_set_bgimg_by_shape(compo_listbox_t *listbox,s16 x, s16 y, s16 width, s16 height,u16 r,u8 alpha,u16 color)
{
    if (listbox == NULL)
    {
        halt(HALT_GUI_COMPO_LISTBOX_SET_BGIMG);
    ...
    for (i=0; i<LISTBOX_ITEM_CNT; i++)
    {
        widget_set_location(listbox->shape_bgimg[i], x, y, width, height);
        widget_rect_set_radius(listbox->shape_bgimg[i], r);
        widget_set_alpha(listbox->shape_bgimg[i], alpha);
        widget_rect_set_color(listbox->shape_bgimg[i], color);
    }
    listbox->item_height = height;
    compo_listbox_init_update(listbox);
}

//设置图标
void comp_lisbox_set_icon_addr(compo_listbox_t *listbox,u8 index,u32 res_addr,s16 x,s16 y)
{
    if (listbox == NULL)
    {
        halt(HALT_GUI_COMPO_LISTBOX_SET_BGIMG);
    ...
    for (i=0; i<LISTBOX_ITEM_CNT; i++)
    {
        if(index == 0)
        {
            widget_icon_set(listbox->item_icon[i], res_addr);
            widget_set_pos(listbox->item_icon[i],x, y);
        }
        else
        {
            widget_icon_set(listbox->item_icon2[i], res_addr);
            widget_set_pos(listbox->item_icon2[i],x, y);
        }
    }
    compo_listbox_init_update(listbox);
}

void comp_lisbox_set_icon_pos(compo_listbox_t *listbox,u8 index,s16 x,s16 y)
{
    for (u8 i=0; i<LISTBOX_ITEM_CNT; i++)
    {
        if(index == 0)
        {
            widget_set_pos(listbox->item_icon[i],x, y);
...
void compo_listbox_update(compo_listbox_t *listbox)
{
    ...
    for (i=0; i<LISTBOX_ITEM_CNT; i++)
    {
        ...
        if (listbox->flag_text_modify == 0)         //默认用i18n里的文本内容
        {
            ...
            if(listbox->style == COMPO_LISTBOX_STYLE_SPORT_LIST_BY_CHUANYIN)
            {
                char text_box[31];
                memset(text_box,0,sizeof(text_box));
                extern void get_sport_taget_txt(compo_listbox_t *list,u8 i,char *txt);
                get_sport_taget_txt(listbox,listbox->item_idx[i],text_box);
                widget_text_set(listbox->item_text2[i],text_box);
            }
            else if(listbox->style == COMPO_LISTBOX_STYLE_SPORT_HISTORY_LIST)
            {
                char text_box[31];
                memset(text_box,0,sizeof(text_box));
                extern void get_sport_history_txt(u8 i,char *txt);
                get_sport_history_txt(listbox->item_idx[i],text_box);
                widget_text_set_font(listbox->item_text[i], UI_BUF_0FONT_FONT_NUM_48_BIN);
                widget_text_set(listbox->item_text[i],text_box);

                memset(text_box,0,sizeof(text_box));
                extern void get_sport_history_time(u8 i,char *txt);
                get_sport_history_time(listbox->item_idx[i],text_box);
                widget_text_set_font(listbox->item_text2[i], UI_BUF_0FONT_FONT_NUM_28_BIN);
                widget_text_set(listbox->item_text2[i],text_box);

                area_t leng_size = widget_text_get_area(listbox->item_text[i]);
                memset(text_box,0,sizeof(text_box));
                extern void get_sport_history_uint(u8 i,char *txt);
                u8 len = 0;
                get_sport_history_uint(listbox->item_idx[i],text_box);
                widget_set_location(listbox->item_text3[i], 80 + leng_size.wid + 10,36, 180, 46);
                widget_set_align_center(listbox->item_text3[i], false);
                widget_text_set(listbox->item_text3[i],text_box);

                memset(text_box,0,sizeof(text_box));
                extern void get_sport_history_pm(u8 i,char *txt);
                get_sport_history_pm(listbox->item_idx[i],text_box);
                widget_set_location(listbox->item_text4[i], 160,listbox->line_height / 2 + 15, 180, 46);
                widget_set_align_center(listbox->item_text4[i], false);
                widget_text_set(listbox->item_text4[i],text_box);
            }
            else
            {
                widget_text_set(listbox->item_text2[i], i18n[item->str_idx1]);
            }
        }
...
//                printf("[%d,%d] %s:%s\n", sizeof(str_txt_name), sizeof(str_txt_number), str_txt_name, str_txt_number);
        ...
        if (listbox->style != COMPO_LISTBOX_STYLE_SELECT && listbox->style != COMPO_LISTBOX_STYLE_LANGUAGE&&listbox->style != COMPO_LISTBOX_STYLE_SPORT_LIST_BY_CHUANYIN || listbox->style != COMPO_LISTBOX_STYLE_SPORT_HISTORY_LIST)
        {
        ...
        switch (listbox->style)
        {
            ...
            case COMPO_LISTBOX_STYLE_MENU_FOURGRID:
                ...
                break;
            case COMPO_LISTBOX_STYLE_SPORT_LIST_BY_CHUANYIN:
                flag_scale = (udy > udy_th);
                break;
            case COMPO_LISTBOX_STYLE_SPORT_HISTORY_LIST:
                widget_icon_set(listbox->item_icon2[i], listbox->res_sta_icon1);
                flag_scale = false;
                break;
            default:
...
void compo_listbox_move(compo_listbox_t *listbox)
{
    ...
    if (mcb->flag_drag)
    {
        ...
        else
        {
            ...
            {
                int temp = ctp_get_last_dxy().y;
                temp = abs(temp) < UTE_DRV_SCREEN_HEIGHT / 20 ? temp / 2 : temp;
                last_dy = abs(temp) < UTE_DRV_SCREEN_HEIGHT / 40 ? 0 : temp;
            }
            mcb->focus_y = listbox->ofs_y;
            ...
            {
                mcb->moveto_idx = listbox->focus_icon_idx - (last_dy * DRAG_AUTO_SPEED / 2 / GUI_SCREEN_HEIGHT);
            }
            else
            {
