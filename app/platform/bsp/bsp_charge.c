...
void bsp_charge_set_stop_curr(u8 stop_curr)
{
    charge_cfg.stop_curr = stop_curr;
}

void bsp_charge_set_const_curr(u8 const_curr)
{
    charge_cfg.const_curr = const_curr;
}
...
static void charge_timer_callback (co_timer_t *timer, void *param)
{
    ...
    switch(sys_cb.charge_sta)
    {
        ...
        case 1:             //充电中
            ...
            uint8_t max_temp = CHARGE_NTC_ADC_MAX_TEMP;
            uint8_t max_re_temp = CHARGE_NTC_ADC_MAX_RE_TEMP;
            if (uteModuleNewFactoryTestGetMode() == FACTORY_TEST_MODE_AGING
#if UTE_MODULE_NEW_FACTORY_TEST_SUPPORT
                || (uteModuleNewFactoryTestGetMode() == FACTORY_TEST_MODE_AGING)
#endif
               )
            {
                max_temp = UTE_DRV_NEW_FACTORY_TEST_BATTERY_CE_AUTH_NOT_ALLOW_TEMPERATURE;
                max_re_temp = UTE_DRV_NEW_FACTORY_TEST_BATTERY_CE_AUTH_ALLOW_TEMPERATURE;
            }
#if UTE_MODULE_BEDSIDE_MODE_SUPPORT
            else if (uteModuleBedsideModeIsOpen())
            {
                max_temp = CHARGE_NTC_ADC_MAX_TEMP + 10;
                max_re_temp = CHARGE_NTC_ADC_MAX_RE_TEMP + 10;
            }
#endif
            if((charge_ntc_stop_flag== 0)  && ((ambientTemperature > max_temp) || (ambientTemperature < CHARGE_NTC_ADC_MIN_TEMP)))
            {
                //温度异常 停止充
            ...
            if((charge_ntc_stop_flag== 1)  && (ambientTemperature < max_re_temp) && (ambientTemperature > CHARGE_NTC_ADC_MIN_RE_TEMP))
            {
                //温度恢复 继续充
