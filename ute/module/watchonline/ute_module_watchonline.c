...
{
    // uteModulePlaformUpdateConnectParam(12,36,50*1000);
    ...
    uteModuleWatchOnlineWriteIndexAutoConfirm();
#endif
...
//    if(/*isUpdateWatch||*/!uteModuleGuiCommonIsDisplayOn())
    {
#if UTE_MODULE_WATCHONLINE_NOT_SHOW_SYNC_WHEN_CHARGING_SUPPORT
        ...
        // else
        // {
        //     task_stack_insert_before_top(UTE_MOUDLE_SCREENS_SYNC_WATCH_ONLINE_ID);
        // }
#else
        uteTaskGuiStartScreen(UTE_MOUDLE_SCREENS_SYNC_WATCH_ONLINE_ID, 0, __func__);
#endif
...
uint8_t uteModuleWatchOnLineTSyncComplete(void)
{
    ...
    if (watchConfig.snNo == UTE_MODULE_WATCH_PHOTO_DEFAULT_ID)
    {
        ...
        if (img_area.wid == UTE_DRV_SCREEN_WIDTH && img_area.hei == UTE_DRV_SCREEN_HEIGHT)
        {
            headerNum = 1;
            uteModuleWatchOnlineData.photoWatchConfig[uteModuleWatchOnlineData.photoWatchPictureIndex].isVaild = 0;
            uteModulePlatformFlashNorWrite((uint8_t *)&uteModuleWatchOnlineData.photoWatchConfig[uteModuleWatchOnlineData.photoWatchPictureIndex], bassAddr, sizeof(photoWatchConfig_t));
        }
        else
        {
            headerNum = 0;
            memset(&uteModuleWatchOnlineData.photoWatchConfig[uteModuleWatchOnlineData.photoWatchPictureIndex], 0, sizeof(photoWatchConfig_t));
        }
    }
#endif
    ...
    if (uteModuleWatchOnlineData.fileCrc32 == watchConfig.fileCrc && headerNum > 0)
    {
        ...
        if (watchConfig.snNo != UTE_MODULE_WATCH_PHOTO_DEFAULT_ID)
        {
            uteModuleWatchOnlineData.lastWriteWatchIndex = uteModuleWatchOnlineData.writeWatchIndex;
            void *file;
            if (uteModuleFilesystemOpenFile(UTE_MODULE_FILESYSTEM_WATCHONLINE_LAST_INDEX, &file, FS_O_WRONLY | FS_O_CREAT | FS_O_TRUNC))
            {
                uteModuleFilesystemWriteData(file, &uteModuleWatchOnlineData.lastWriteWatchIndex, 1);
...
void uteModuleWatchOnlineStartSyncPhoto(uint8_t *data)
{
...
#if (UTE_MODULE_WATCH_PHOTO_MAX_PICTURE_CNT > 1)
    uteModuleWatchOnlineData.ReceivePhotoHead.isWatchVaild = 0;
#else
    uteModuleWatchOnlineData.ReceivePhotoHead.isWatchVaild = 1;
#endif
    uint32_t onePhotoMaxSize = uteModuleWatchOnlineGetOnePhotoMaxSize() - sizeof(photoWatchConfig_t);
    ...
    {
#if UTE_MODULE_WATCHONLINE_WRITE_INDEX_AUTO_SUPPORT
        uteModuleWatchOnlineSetWillUpdateDataIndex(UTE_MODULE_WATCHONLINE_PHOTO_WATCH_DEFAULT_INDEX);
#endif
...
#if UTE_MODULE_WATCHONLINE_WRITE_INDEX_AUTO_SUPPORT
uint8_t uteModuleWatchOnlineFindAvailableIndex(void)
{
    int watchConfigSize = sizeof(watchConfig_t);
    watchConfig_t watchConfig;
    for (uint8_t i = 0; i < UTE_MODULE_WATCHONLINE_MULTIPLE_MAX_CNT; i++)
    {
        // 跳过PHOTO_WATCH专用位置
        if (i == UTE_MODULE_WATCHONLINE_PHOTO_WATCH_DEFAULT_INDEX)
        {
            continue;
        }
        uteModulePlatformFlashNorRead((uint8_t *)&watchConfig, uteModuleWatchOnlineMultipleBaseAddress[i], watchConfigSize);
        if (watchConfig.isWatchVaild)
        {
            return i;
        }
    }
    return 0xff; // 表示没有可用位置
}

/**
 * @brief        自动确认在线表盘写入Index
 * @details
 * @return       void*
 * @author       Wang.Luo
 * @date         2025-09-01
...
{
    uint8_t index = uteModuleWatchOnlineFindAvailableIndex();
    UTE_MODULE_LOG(UTE_LOG_WATCHONLINE_LVL, "%s,index=%d", __func__, index);
    if(uteModuleWatchOnlineData.isStartReceivePhoto)
    {
        index = UTE_MODULE_WATCHONLINE_PHOTO_WATCH_DEFAULT_INDEX;
    }
    else if (index == 0xff || index >= UTE_MODULE_WATCHONLINE_MULTIPLE_MAX_CNT)
    {
        void *file;
        if (uteModuleFilesystemOpenFile(UTE_MODULE_FILESYSTEM_WATCHONLINE_LAST_INDEX, &file, FS_O_RDONLY))
        {
            uteModuleFilesystemReadData(file, &uteModuleWatchOnlineData.lastWriteWatchIndex, 1);
            uteModuleFilesystemCloseFile(file);
        }
        uint8_t start_index = (uteModuleWatchOnlineData.lastWriteWatchIndex + 1) % UTE_MODULE_WATCHONLINE_MULTIPLE_MAX_CNT;
        index = start_index;
        UTE_MODULE_LOG(UTE_LOG_WATCHONLINE_LVL, "%s,index=%d,start_index=%d", __func__, index, start_index);
        for (uint8_t i = 0; i < UTE_MODULE_WATCHONLINE_MULTIPLE_MAX_CNT; i++)
        {
            if (index != UTE_MODULE_WATCHONLINE_PHOTO_WATCH_DEFAULT_INDEX)
            {
                break;
            }
            index = (index + 1) % UTE_MODULE_WATCHONLINE_MULTIPLE_MAX_CNT;
    ...
    uteModuleWatchOnlineSetWillUpdateDataIndex(index);
    UTE_MODULE_LOG(UTE_LOG_WATCHONLINE_LVL, "%s,writeWatchIndex=%d", __func__, uteModuleWatchOnlineData.writeWatchIndex);
}
#endif
...
void uteModuleWatchOnlineSetInfo(uint8_t *data)
{
    ...
    uteModuleGuiCommonGetCurrWatchIndex(&index);
    data[0] = (UTE_DRV_SCREEN_WIDTH >> 8) & 0xFF;
    data[1] = UTE_DRV_SCREEN_WIDTH & 0xFF;
    data[2] = (UTE_DRV_SCREEN_HEIGHT >> 8) & 0xff;
    data[3] = UTE_DRV_SCREEN_HEIGHT & 0xFF;
    if (uteModuleWatchOnlineData.multipleValidWatchCnt)
    {
        data[4] = (uteModuleWatchOnlineData.watchConfig.snNo >> 24) & 0xFF;
        data[5] = (uteModuleWatchOnlineData.watchConfig.snNo >> 16) & 0xFF;
        data[6] = (uteModuleWatchOnlineData.watchConfig.snNo >> 8) & 0xFF;
        data[7] = uteModuleWatchOnlineData.watchConfig.snNo & 0xFF;
    }
    else
    {
        data[4] = 0xFF;
    ...
    data[8] = UTE_MODULE_SCREENS_WATCH_CNT_MAX;
    data[9] = index;
    data[10] = UTE_MODULE_WATCHONLINE_MULTIPLE_MAX_CNT;
    data[11] = uteModuleWatchOnlineData.multipleValidWatchCnt;
}

/**
 * @brief        是否正在传输表盘
 * @details
 * @return       true 表示正在传输表盘
 * @author       Wang.Luo
 * @date         2025-09-03
 */
bool uteModuleWatchOnlinegetIsStartReceive(void)
{
    return uteModuleWatchOnlineData.isStartReceive;
}
