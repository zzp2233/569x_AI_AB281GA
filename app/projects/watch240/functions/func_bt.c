...
compo_form_t *func_bt_form_create(void)
{
    ...
    //bg
    compo_picturebox_t *bg_pic = compo_picturebox_create(frm, UI_BUF_I341001_13_MUSIC_BG1_BIN);
    compo_setid(bg_pic, COMPO_ID_MUSIC_BG_FIG);
    compo_picturebox_set_pos(bg_pic, GUI_SCREEN_CENTER_X, GUI_SCREEN_CENTER_Y);
    ...
    compo_textbox_set_location(lyric_txt, GUI_SCREEN_CENTER_X, 87+20, 200, 30);
    compo_textbox_set_autoroll_mode(lyric_txt, TEXT_AUTOROLL_MODE_SROLL_CIRC);
    compo_setid(lyric_txt, COMPO_ID_TXT_MUSIC_LYRIC);
    compo_textbox_set(lyric_txt,title_buf);

    compo_button_t *btn = compo_button_create_by_image(frm, UI_BUF_I341001_13_MUSIC_PREVIOUSSONG01_BIN);///上一曲
    //if(!uteModuleCallBtIsConnected() && !ble_is_connect())
    compo_setid(btn, COMPO_ID_BTN_PREV);
    compo_button_set_pos(btn, 37+21, 214+23);

    //if(!uteModuleCallBtIsConnected() && !ble_is_connect())
    btn = compo_button_create_by_image(frm, UI_BUF_I341001_13_MUSIC_NEXTSONG01_BIN);///下一曲
    compo_setid(btn, COMPO_ID_BTN_NEXT);
    ...
    {
        // compo_button_set_bgimg(btn,UI_BUF_I335001_MUSIC_2_NOT_CONNECTED_ICON_96X96_X72_Y104_BIN);
        btn = compo_button_create_by_image(frm,UI_BUF_I341001_13_MUSIC_PLAY02_BIN);///灰色的暂停按钮
    }
    else
    {
        btn = compo_button_create_by_image(frm,bt_cb.music_playing ? UI_BUF_I341001_13_MUSIC_PAUSED_BIN:UI_BUF_I341001_13_MUSIC_PLAY01_BIN);///暂停 播放
    }
    compo_setid(btn, COMPO_ID_BTN_PLAY);
    compo_button_set_pos(btn, GUI_SCREEN_CENTER_X, 170+122/2);

    btn = compo_button_create_by_image(frm, UI_BUF_I341001_13_MUSIC_VOLUME00_BIN);///音量减
    compo_setid(btn, COMPO_ID_BTN_VOL_DOWN);
    compo_button_set_pos(btn, 46+13,382+16);

    btn = compo_button_create_by_image(frm, UI_BUF_I341001_13_MUSIC_VOLUME01_BIN);///音量加
    compo_setid(btn, COMPO_ID_BTN_VOL_UP);
...
static void func_bt_music_refresh_disp(void)
{
    ...
    {
        btn_play   = compo_getobj_byid(COMPO_ID_BTN_PLAY);
        compo_button_set_bgimg(btn_play,UI_BUF_I341001_13_MUSIC_PLAY02_BIN);///灰色的暂停按钮
    }
    else
    {
        btn_play   = compo_getobj_byid(COMPO_ID_BTN_PLAY);
        compo_button_set_bgimg(btn_play,bt_cb.music_playing ? UI_BUF_I341001_13_MUSIC_PAUSED_BIN:UI_BUF_I341001_13_MUSIC_PLAY01_BIN);///暂停 播放
    }
    uint8_t vol = uteModuleMusicGetPlayerVolume() / 6;
    if(vol>16)vol=16;
    compo_picturebox_cut(pic,vol,17);
