...
#define UTE_MOUDLE_SCREENS_OTA_ID FUNC_OTA_MODE
#define UTE_MOUDLE_SCREENS_SCREEN_SAVER_ID FUNC_CLOCK_SAVER
#define UTE_MOUDLE_SCREENS_SPORTS_DETAIL_ID FUNC_SPORT_SUB_RUN

//切换类型枚举
typedef enum
{
    SWITCH_MODE_NORMAL = 0,      // 平移
    SWITCH_PODE_SINGLE_SCALE,    // 单边缩放
    SWITCH_MODE_BILATERAL_SCALE, // 双边缩放
    SWITCH_MODE_ROTARY,          // 柱面体
    SWITCH_MODE_MAX
} ute_gui_switch_mode;
...
typedef struct
{
    ...
    uint16_t screenSaverTimeBucket; // 息屏显示时间段开关
    uint16_t screenSaverStartTimeHourMin;
    uint16_t screenSaverEndTimeHourMin;
    bool setScreenSaverType; //true 开始时间 false 结束时间
#endif
#if UTE_MODULE_ENCODER_SWITCH_WATCHMAIN_LOCK_SUPPORT
    bool isWatchMainLock;
#endif
    uint8_t switchMode; //切换效果
#if UTE_MODULE_SCREENS_MINI_CARD_SUPPORT
    uint8_t miniCardTblSort[MAX_FUNC_MINI_SORT_CNT];
    uint8_t miniCardSortCnt;
#endif
} ute_display_ctrl_t;
...
#if UTE_MODULE_SCREENS_SCREEN_SAVER_SUPPORT
    bool isScreenSaver;
    bool isScreenSaverUpdate; //唤醒刷屏
    uint16_t lastScreenId;
#endif
#if UTE_MODULE_COVER_DISPLAY_OFF_SUPPORT
    bool isCoverDisplayOffOpen; //覆盖息屏
#endif
    u32 tetrisMaxScore;
