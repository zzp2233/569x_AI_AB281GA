...
static uint8_t func_convert_to_12hour(uint8_t time)
{
    uint8_t hour12;
...
void func_charge_update(void)
{
    compo_picturebox_t *picbox   = compo_getobj_byid(COMPO_ID_BAT_PIC);
    compo_textbox_t *txt_title   = compo_getobj_byid(COMPO_ID_TTXT);
    compo_animation_t *animation = compo_getobj_byid(COMPO_ID_GIF_PIC);
    compo_textbox_t *txt_time    = compo_getobj_byid(COMPO_ID_TEXT_TIME);
    ...
    if (uteModuleSystemtime12HOn())
    {
        if (time.hour >= 12)
        {
            snprintf(txt_buf, sizeof(txt_buf), "%s   %02d:%02d", i18n[STR_PM], func_convert_to_12hour(time.hour), time.min);
        }
        else
        {
            snprintf(txt_buf, sizeof(txt_buf), "%s   %02d:%02d", i18n[STR_AM], func_convert_to_12hour(time.hour), time.min);
        }
    }
    else
    {
        snprintf(txt_buf, sizeof(txt_buf), "%02d:%02d", time.hour, time.min);
    }
...
//    compo_picturebox_set_visible(pic, bsp_charge_sta_get()==1);
    if (bsp_charge_sta_get() == 2)
    {
        ...
        compo_textbox_set(txt_title, i18n[STR_CHARGING_COMPLETED]);
    }
}
...
compo_form_t *func_charge_form_create(void)
{
    ...
    // ///设置动图
    compo_animation_t *animation = compo_animation_create(frm, UI_BUF_I341001_CHARGE_000_BIN);
    compo_animation_set_pos(animation, GUI_SCREEN_CENTER_X, 208);
    compo_animation_set_radix(animation, 24);
    compo_animation_set_interval(animation, 15);
    compo_setid(animation, COMPO_ID_GIF_PIC);

    compo_picturebox_t *picbox = compo_picturebox_create(frm, UI_BUF_I341001_CHARGE_000_BIN);
    compo_picturebox_set_pos(picbox, GUI_SCREEN_CENTER_X, 208);
    compo_picturebox_cut(picbox, 23, 24);
    ...
    compo_textbox_set_pos(textbox, GUI_SCREEN_CENTER_X, 50);
    compo_textbox_set(textbox, txt_buf);
    compo_setid(textbox, COMPO_ID_TEXT_TIME);
    ...
    textbox = compo_textbox_create(frm, 50);
    compo_textbox_set_location(textbox, GUI_SCREEN_CENTER_X, GUI_SCREEN_HEIGHT - 70, 320, 50);
    compo_textbox_set_forecolor(textbox, COLOR_GREEN);
    compo_textbox_set(textbox, i18n[STR_CHARGING]);
