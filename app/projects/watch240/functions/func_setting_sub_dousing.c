...
const uint8_t txt_num[4] = {5, 10, 15, 20}; // 固定数组
...
static void func_set_sub_dousing_get_timer(uint8_t *timer, uint8_t *timer_data, s8 num)
{
    // 1. 查找当前timer在txt_num中的索引
    unsigned char current_index = 0;
    for (unsigned char i = 0; i < sizeof(txt_num); i++)
    {
        if (txt_num[i] == *timer)
        {
            current_index = i;
    ...
    // 2. 应用偏移量并计算新索引（处理环形逻辑）
    int new_index = (current_index + num) % sizeof(txt_num);
    if (new_index < 0) new_index += sizeof(txt_num); // 处理负数索引
    ...
    *timer = txt_num[new_index];
    ...
    {
        // 计算相对于新索引的环形偏移
        unsigned char offset = (new_index + i + sizeof(txt_num)) % sizeof(txt_num);
        // 将结果放入以索引2为中心的位置
        timer_data[i + 2] = txt_num[offset];
    }
}
...
compo_form_t *func_set_sub_dousing_form_create(void)
{
    ...
    for(int idx=COMPO_ID_TXT_1; idx<=COMPO_ID_TXT_4; idx++) ///创建滑动文本
    {
        memset(txt_buf,0,sizeof(txt_buf));
        compo_textbox_t *txt = compo_textbox_create_for_page(frm,page,2);
    ...
    compo_textbox_t *txt = compo_textbox_create_for_page(frm,page,strlen(i18n[STR_SEC]));
    compo_textbox_set_align_center(txt,false);
    compo_textbox_set_pos(txt,GUI_SCREEN_CENTER_X+42,DOUSING_TIME_TXT_Y[2]);
    compo_textbox_set(txt,i18n[STR_SEC]);
...
#define TXT_SPACING    (94-48)//(135-82)
#define CENTER_TXT_Y   (140/2)
const uint8_t txt_num[5] = {5, 10, 15, 30, 60}; // 固定数组
const uint16_t DOUSING_TIME_TXT_Y[5]= {CENTER_TXT_Y-TXT_SPACING*2,CENTER_TXT_Y-TXT_SPACING,CENTER_TXT_Y,CENTER_TXT_Y+TXT_SPACING,CENTER_TXT_Y+TXT_SPACING*2}; ///文本Y轴
// 函数功能：获取设置时间（上下 & 上上下下 ）的数
...
compo_form_t *func_set_sub_dousing_form_create(void)
{
    ...
    compo_textbox_t *txt = compo_textbox_create(frm,strlen(i18n[STR_SEC]));
    compo_textbox_set_align_center(txt,false);
    compo_textbox_set_location(txt,150,90,100,35);
    compo_textbox_set(txt,i18n[STR_SEC]);
    ...
    compo_button_set_pos(btn_ok,GUI_SCREEN_CENTER_X,GUI_SCREEN_HEIGHT-gui_image_get_size(UI_BUF_I342001_28_SET_COMFIRM_BIN).hei/2-20);
    compo_setid(btn_ok,COMPO_ID_BTN_SURE);
...
#elif GUI_SCREEN_SIZE_240X284RGB_I335001_SUPPORT || GUI_SCREEN_SIZE_360X360RGB_I338001_SUPPORT || GUI_SCREEN_SIZE_240X240RGB_I342001_SUPPORT || GUI_SCREEN_SIZE_368X448RGB_I345001_SUPPORT \
      || GUI_SCREEN_SIZE_320X380RGB_I343001_SUPPORT
