...
{
#if (UTE_MODULE_SHIP_MODE_SUPPORT && UTE_MODULE_SHIP_MODE_ONLY_ALLOW_CHARGING_POWER_NO_SUPPORT)
    if (uteModuleShipModeIsShip())
    {
        if (uteDrvBatteryCommonGetChargerPlug())
        {
    ...
    if (!uteApplicationCommonData.isStartupFristFinish)
    {
        ...
        uteModuleSleepReadSyncTimeInfo();
#endif
        uteModuleCallInit();
        //uteModuleOtaInit();
        uteModuleMusicInit();
...
#if UTE_MODULE_POWERON_LANGUAGE_SELECT_SUPPORT
                else if ((uteModuleAppBindingGetHasBindingBefore() != HAS_BEEN_CONNECTED) && time.isWatchSetLangage == false)
                {
...
{
#if APP_MODULE_BT_DISCONNECT_NOTIFY_SUPPORT
    bool canNotify = false;
    if ((uteApplicationCommonData.bleConnectState.connectedSecond > 30)&&(uteApplicationCommonData.isPowerOn))//todo
    {
        canNotify = true;
    }
#endif
...
#endif
#if UTE_MODULE_WATCHONLINE_SUPPORT
        if(uteModuleWatchOnlinegetIsStartReceive())
        {
    ...
    {
#if APP_MODULE_BT_DISCONNECT_NOTIFY_SUPPORT
        uteApplicationCommonData.bleDisconnectCnt = 0;
#endif
        // uteModulePlatformSetFastAdvertisingTimeCnt(0);
...
#if APP_MODULE_BT_DISCONNECT_NOTIFY_SUPPORT
    if (uteApplicationCommonGetBTNotifyStatus() && (isConnected == false) && (canNotify == true)  && (uteDrvBatteryCommonGetChargerStatus() == BAT_STATUS_NO_CHARGE))
    {
        uteApplicationCommonData.bleDisconnectCnt = 1;
    }
#endif
}
/**
...
#endif
        UTE_MODULE_LOG(0, "%s,GetOurAppConnection = %d,GetBindingSecCnt = %d", __func__, uteModuleAppBindingGetOurAppConnection(),uteModuleAppBindingGetBindingSecCnt());
...
#endif
#if 0//UTE_MODULE_LOCAL_SET_LIFT_WRIST_SUPPORT&&UTE_MODULE_LOCAL_SET_NOT_DISTURB_SUPPORT
    ute_module_systemtime_time_t time;
    uteModuleSystemtimeGetTime(&time);
...
#endif
#if APP_MODULE_BT_DISCONNECT_NOTIFY_SUPPORT
    if (uteApplicationCommonData.bleDisconnectCnt > 0 && !uteApplicationCommonData.bleConnectState.isConnected)
    {
        uteApplicationCommonData.bleDisconnectCnt ++;
        if (uteApplicationCommonData.bleDisconnectCnt >= 4)
        {
            uteApplicationCommonData.bleDisconnectCnt = 0;
            uteDrvMotorStart(UTE_MOTOR_DURATION_TIME, UTE_MOTOR_INTERVAL_TIME, 3);
            sys_cb.cover_index = REMIND_GCOVER_APP_CONNECT;
            sys_cb.remind_tag = true;
            //uteTaskGuiStartScreen(UTE_MOUDLE_SCREENS_BT_NOTIFY_ID);
        }
    }
#endif
#if UTE_MODULE_UNBOUND_AUTO_POWER_OFF_SUPPORT
    if (uteModuleAppBindingGetHasBindingBefore() != HAS_BEEN_CONNECTED)
    {
        if (uteApplicationCommonData.systemPowerOnSecond >= UTE_MODULE_UNBOUND_AUTO_POWER_OFF_SECONDS)
        {
...
{
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL, "%s,mtu=%d,%d", __func__, uteApplicationCommonData.mtuSize, ble_get_gatt_mtu());
    uteApplicationCommonData.mtuSize = ble_get_gatt_mtu();
    if (uteApplicationCommonData.mtuSize > (UTE_BLE_MTU_MAX_SIZE - 3))
    {
        uteApplicationCommonData.mtuSize = (UTE_BLE_MTU_MAX_SIZE - 3);
    }
...
*@date        2022-11-18
*/
void uteApplicationCommonSaveCustomSettingInfo(void)
{
    /*! 保存到文件wuhuowang, 2022-11-18*/
    void *file;
    uint8_t writebuff[4];
    writebuff[0] = uteApplicationCommonData.customSetting.isBTDisconnectNotify;
    writebuff[1] = uteApplicationCommonData.customSetting.isSOSDialEnable;
    writebuff[2] = uteApplicationCommonData.customSetting.isAutoStartSport;
    writebuff[3] = uteApplicationCommonData.customSetting.femaleSet;
    if (uteModuleFilesystemOpenFile(UTE_MODULE_FILESYSTEM_SYSTEMPARM_CUSTOMINFO, &file, FS_O_WRONLY | FS_O_CREAT | FS_O_TRUNC))
    {
...
*@date        2022-11-18
*/
void uteApplicationCommonGetCustomSettingInfo(ute_custom_setting_info_t *customSetting)
{
    uteModulePlatformTakeMutex(uteApplicationCommonMute);
    memcpy(customSetting, &uteApplicationCommonData.customSetting, sizeof(ute_custom_setting_info_t));
    uteModulePlatformGiveMutex(uteApplicationCommonMute);
}
...
*@date       2022-11-18
*/
void uteApplicationCommonSetCustomSettingInfo(ute_custom_setting_info_t *customSetting)
{
    uteModulePlatformTakeMutex(uteApplicationCommonMute);
    memcpy(&uteApplicationCommonData.customSetting, customSetting, sizeof(ute_custom_setting_info_t));
    uteModulePlatformGiveMutex(uteApplicationCommonMute);
    uteApplicationCommonSaveCustomSettingInfo();
}

bool uteApplicationCommonGetSosEnableStatus(void)
{
    return uteApplicationCommonData.customSetting.isSOSDialEnable;
}

bool uteApplicationCommonGetBTNotifyStatus(void)
{
    return uteApplicationCommonData.customSetting.isBTDisconnectNotify;
}

uint8_t uteApplicationCommonGetFemaleSetting(void)
{
    return uteApplicationCommonData.customSetting.femaleSet;
}
