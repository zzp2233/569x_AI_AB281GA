...
//星期日,一,二,三,四,五,六 -> BIT(0) BIT(1) BIT(2) BIT(3) BIT(4) BIT(5) BIT(6)
static const uint8_t rtc_weak_mask[7] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20,0x40};
...
static bool ute_alarm_check(tm_t *now_time)
{
    ...
    for (uint8_t i = 0; i < uteModuleSystemtimeGetAlarmMaxCnt(); i++)
    {
        ...
        if ((alarm_p->alarmRepeat.repeatRemindHour == 0) || (alarm_p->alarmRepeat.repeatRemindMin == 0))
        {
            alarm_p->alarmRepeat.repeatRemindHour = alarm_p->hour;
            alarm_p->alarmRepeat.repeatRemindMin = alarm_p->min;
        }
#endif
        TRACE("i[%d] num[%d] cycle[0x%02x] mask[0x%02x] alarm[%02d:%02d] now[%02d:%02d] repeat[%02d,%02d,%d] \n", \
              ...
              alarm_p->alarmRepeat.repeatRemindHour, alarm_p->alarmRepeat.repeatRemindMin, alarm_p->alarmRepeat.isRepeatRemindOpen);
        if ((cycle & week_mask) > 0 || (cycle >> 7))  //当天或者只提醒一次
        {
            ...
            {}
#if UTE_MODULE_LOCAL_ALARM_REPEAT_REMIND_SUPPORT
            else if (alarm_p->alarmRepeat.isRepeatRemindOpen && alarm_p->alarmRepeat.repeatRemindTimes > 0) //贪睡时钟 ，闹钟贪睡功能打开，但是提醒次数为0时不响铃
            {
                if ((alarm_p->alarmRepeat.repeatRemindHour == now_time->hour) && (alarm_p->alarmRepeat.repeatRemindMin == now_time->min))  //到时间
                {
                    TRACE("repeat alarm[%d] ring, [%02d:%02d]\n", i, alarm_p->alarmRepeat.repeatRemindHour, alarm_p->alarmRepeat.repeatRemindMin);
