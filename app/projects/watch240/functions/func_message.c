...
{
    {
        .pic_x= -136,    .pic_y= -52,   .pic_w=44,    .pic_h=44,
...
compo_form_t *func_message_form_create(void)
{
    ...
    if(msg_num == 0)
    {
        ...
        compo_textbox_t* txt = compo_textbox_create(frm, strlen(i18n[STR_NO_MSG]));
        compo_textbox_set_location(txt, GUI_SCREEN_CENTER_X, 294 + 20, 300, 30);
        compo_textbox_set(txt, i18n[STR_NO_MSG]);
    ...
    for(int i=0; i<msg_num; i++)
    {
        ...
        compo_setid(cardbox,COMPO_ID_CARD_FIRST+i);
        compo_cardbox_set_pos(cardbox,GUI_SCREEN_CENTER_X,72+83+180*i);
    ...
    //       );
    //创建按钮
    compo_button_t* btn = compo_button_create_by_image(frm, UI_BUF_I341001_14_INFORMATION_DELETE_BIN);
