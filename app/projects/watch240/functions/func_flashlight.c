...
static void flashlight_restore_backlight(void)
{
    f_flashlight_t *f_flashlight = (f_flashlight_t *)func_cb.f_cb;

    if (f_flashlight == NULL)
    {
        return;
    }

    if (!f_flashlight->restored_brightness)
    {
        tft_bglight_set_level(f_flashlight->prev_light_level, true);
        f_flashlight->restored_brightness = true;
    }
}
#else
static inline void flashlight_restore_backlight(void)
{
}
#endif
...
static void func_flashlight_button_click(void)
{
    ...
    {
        f_flashlight->prev_light_level = sys_cb.light_level;
        f_flashlight->restored_brightness = false;
...
//    compo_form_set_title(frm, i18n[STR_FLASHLIGHT]);
    ...
    compo_shape_set_radius(shape, 74);
#else
    compo_shape_set_radius(shape, UTE_MODULE_WATCHONLINNE_R_VALUE);
#endif
    compo_setid(shape,COMPO_ID_BG_SHAPE);
    compo_shape_set_visible(shape, true);
...
static void func_flashlight_enter(void)
{
    ...
    f_flashlight_t *f_flashlight = (f_flashlight_t *)func_cb.f_cb;
#if !UTE_DRV_LED_SUPPORT
    f_flashlight->prev_light_level = uteModuleGuiCommonGetBackLightPercent();
    f_flashlight->restored_brightness = false;
#endif
    sys_cb.light_level = DEFAULT_BACK_LIGHT_PERCENT_MAX / BACK_LIGHT_PERCENT_INCREASE_OR_INCREASE;
    tft_bglight_set_level(sys_cb.light_level,false);
