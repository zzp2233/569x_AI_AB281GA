...
int webclient_get_file(const char* URI, const char* filename)
{
    ...
    ptr = (unsigned char *) webclient_malloc(WEBCLIENT_RESPONSE_BUFSZ);
    if (ptr == NULL)
    {
                        ...
                        const char* form_data)
{
    ...
    buffer = (unsigned char *) webclient_calloc(1, WEBCLIENT_RESPONSE_BUFSZ);
    if (buffer == NULL)
    {
    ...
    header = (char *) webclient_calloc(1, WEBCLIENT_HEADER_BUFSZ);
    if (header == NULL)
    {
...
__exit:
    ...
    if (buffer != NULL)
    {
        webclient_free(buffer);
    }

    if (header != NULL)
    {
        webclient_free(header);
    }
