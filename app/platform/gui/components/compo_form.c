...
//    widget_set_visible(time, false);
    ...
    compo_textbox_set_location(time, FORM_TITLE_RIGHT + 20, GUI_PAGE_HEAD_HEIGHT - FORM_TITLE_HEIGHT, GUI_PAGE_TIME_WIDTH, FORM_TITLE_HEIGHT);
#else
    ...
    frm->page_body = page_body;
    compo_textbox_set_font(title, UI_BUF_FONT_FORM_TIME);
    compo_textbox_set_location(title, FORM_TITLE_LEFT, GUI_PAGE_HEAD_HEIGHT - FORM_TITLE_HEIGHT, GUI_SCREEN_CENTER_X + GUI_SCREEN_CENTER_X / 4 - FORM_TITLE_LEFT, FORM_TITLE_HEIGHT);
    compo_textbox_set_multiline(title, false);
    compo_textbox_set_autosize(title, false);
    compo_textbox_set_align_center(title, false);
