...
static const u32 future_weather_icon[16] =
{
    ...
    UI_BUF_I341001_12_WEATHER_WEATHER_13_NIGHT_SUNNY_BIN,
    UI_BUF_I341001_12_WEATHER_WEATHER_14_NIGHT_CLOUDY_BIN,
    UI_BUF_I341001_12_WEATHER_WEATHER_15_NIGHT_RAIN_BIN,
};
static void weather_refresh(void)
{
    f_weather_t* f_weather = (f_weather_t*)func_cb.f_cb;
...
//        compo_textbox_set_font(txt, UI_BUF_0FONT_FONT_24_BIN);
        ...
        compo_picturebox_set_pos(picbox, GUI_SCREEN_CENTER_X,122+54);

        txt = compo_textbox_create(frm,strlen(i18n[STR_NO_GET_WEATHER]));
        compo_textbox_set_location(txt,GUI_SCREEN_CENTER_X,282+28/2, GUI_SCREEN_WIDTH/1.1,widget_text_get_max_height()*2);
        compo_textbox_set_multiline(txt, true);
        compo_textbox_set_align_center(txt, true);
        compo_textbox_set_multiline_drag(txt, false);      //避免既有滚动又有省略号的情况
        compo_textbox_set(txt,i18n[STR_NO_GET_WEATHER]);
    ...
    picbox = compo_picturebox_create(frm,UI_BUF_I341001_12_WEATHER_BG_BIN);///背景图片
    compo_picturebox_set_pos(picbox, GUI_SCREEN_CENTER_X,GUI_SCREEN_CENTER_Y);
    ...
    compo_textbox_set_pos(txt,64/2+186,64/2+118);
    compo_textbox_set(txt, str_buff);
    picbox = compo_picturebox_create(frm,UI_BUF_I341001_12_WEATHER_WEATHERSYM_BIN);
    compo_picturebox_set_pos(picbox,315+12,120+12);
    memset(str_buff,0,sizeof(str_buff));
    if(!weather_no_data_flag)
    {
        if(!displayInfo.isFahrenheit)      ///是否为华氏度
    ...
    txt = compo_textbox_create(frm,strlen(str_buff));
    compo_textbox_set_pos(txt,124/2+182,40/2+231);
    compo_textbox_set(txt, str_buff);

    txt = compo_textbox_create(frm,strlen(i18n[weather_list[get_weather_id[0]].txt_num]));
    compo_textbox_set_pos(txt,84/2+28,40/2+69);
    compo_textbox_set_forecolor(txt, make_color(0x98,0xd0,0xff));
    compo_textbox_set(txt, i18n[weather_list[get_weather_id[0]].txt_num]);
    //紫外线icon
    picbox = compo_picturebox_create(frm,UI_BUF_I341001_12_WEATHER_UV_BIN);///背景图片
    compo_picturebox_set_pos(picbox, 72+22,293+22);
    //紫外线data
    txt = compo_textbox_create(frm,strlen(i18n[weather_uv[uteModuleWeatherGetUltravioletLevel(weather_date.fristDayWeatherUltraviolet)]]));
    compo_textbox_set_pos(txt,30+132/2,336+40/2);
    compo_textbox_set(txt,i18n[weather_uv[uteModuleWeatherGetUltravioletLevel(weather_date.fristDayWeatherUltraviolet)]]);
    //紫外线txt
    txt = compo_textbox_create(frm,strlen(i18n[STR_UV]));
    compo_textbox_set_pos(txt,62+30,386+14);
    compo_textbox_set(txt,i18n[STR_UV]);
    //湿度icon
    picbox = compo_picturebox_create(frm,UI_BUF_I341001_12_WEATHER_HUMIDITY_BIN);///背景图片
    compo_picturebox_set_pos(picbox, 261+14,294+20);
    //湿度data
    memset(str_humidity_buff,0,sizeof(str_humidity_buff));
    snprintf(str_humidity_buff,sizeof(str_humidity_buff),"%d%%",weather_date.fristDayWeatherHumidity);
    txt = compo_textbox_create(frm,strlen(str_humidity_buff));
    compo_textbox_set_pos(txt,246+64/2,342+20);
    compo_textbox_set(txt,str_humidity_buff);
    //湿度txt
    txt = compo_textbox_create(frm,strlen(i18n[STR_HUMIDITY]));
    compo_textbox_set_pos(txt,254+40/2,386+14);
    compo_textbox_set(txt,i18n[STR_HUMIDITY]);
    ///设置标题栏名字///
    txt = compo_textbox_create(frm,strlen(i18n[STR_WEATHER]));
    ...
    //未来天气txt
    txt = compo_textbox_create(frm,strlen(i18n[STR_FUTURE_WEATHER]));
    compo_textbox_set_location(txt,GUI_SCREEN_WIDTH/12,GUI_SCREEN_HEIGHT+GUI_SCREEN_HEIGHT/21.8,140,GUI_SCREEN_HEIGHT/(284/28));
    compo_textbox_set(txt,i18n[STR_FUTURE_WEATHER]);
    compo_textbox_set_align_center(txt, false);
    //第二页
    for(int i=0; i<3; i++)
    {
        //bg pic
        picbox = compo_picturebox_create(frm,UI_BUF_I341001_12_WEATHER_FUTURE_WEATHER_BG_BIN);
        compo_picturebox_set_pos(picbox,18+46+(i)*(120),GUI_SCREEN_HEIGHT+150+93);

        //星期 txt
        txt = compo_textbox_create(frm,20);
        compo_textbox_set_location(txt, 29+34+(i)*(120),GUI_SCREEN_HEIGHT+116+24,68,widget_text_get_max_height());
        compo_textbox_set_align_center(txt,true);
        ...
        //图切的不对，80x80了，
        picbox = compo_picturebox_create(frm,future_weather_icon[get_weather_id[i]]);
        compo_picturebox_set_pos(picbox,31+32+(i)*(120),GUI_SCREEN_HEIGHT+180+32);
        compo_picturebox_set_size(picbox,64,64);
        if(get_weather_id[i] != WEATHER_TYPE_UNKNOWN)
        {
            snprintf(str_buff, sizeof(str_buff), "%02d°",weather_date.dayTemperatureMax[i]);//一周 小~大 温度
        }
        ...
        txt = compo_textbox_create(frm,strlen(str_buff));
        compo_textbox_set(txt,str_buff);
        compo_textbox_set_location(txt, 39+25+(i)*(120),GUI_SCREEN_HEIGHT+248+20,70,30);
        // compo_textbox_set_pos(txt, 16+(i-1)*(82),GUI_SCREEN_HEIGHT+167);
        compo_textbox_set_align_center(txt, true);
        if(get_weather_id[i] != WEATHER_TYPE_UNKNOWN)
        {
        ...
        txt = compo_textbox_create(frm,strlen(str_buff));
        compo_textbox_set(txt,str_buff);
        compo_textbox_set_location(txt, 39+25+(i)*(120),GUI_SCREEN_HEIGHT+304+20,70,30);
        compo_textbox_set_align_center(txt, true);
    }
    widget_page_set_client(frm->page_body, 0, page_y);
...
#elif GUI_SCREEN_SIZE_240X284RGB_I330001_SUPPORT || GUI_SCREEN_SIZE_240X284RGB_I335001_SUPPORT || GUI_SCREEN_SIZE_360X360RGB_I338001_SUPPORT || GUI_SCREEN_SIZE_240X240RGB_I342001_SUPPORT || GUI_SCREEN_SIZE_368X448RGB_I341001_SUPPORT
...
#if GUI_SCREEN_SIZE_240X284RGB_I330001_SUPPORT || GUI_SCREEN_SIZE_240X284RGB_I335001_SUPPORT || GUI_SCREEN_SIZE_360X360RGB_I338001_SUPPORT || GUI_SCREEN_SIZE_240X240RGB_I342001_SUPPORT || GUI_SCREEN_SIZE_368X448RGB_I341001_SUPPORT
