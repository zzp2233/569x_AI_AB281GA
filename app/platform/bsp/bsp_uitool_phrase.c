    ...
    if (use_num)
    {
        ...
        if (bond_compo_type == COMPO_TYPE_NUMBER)
        {
            ...
            for(u8 i=0; i<cnt; i++)
            {
                ...
                else if (uitool_res->bond_type == COMPO_BOND_TEMPERATURE)
                {
                    ute_module_weather_data_t *weather_date = NULL;
                    weather_date = ab_zalloc(sizeof(ute_module_weather_data_t));
                    if (weather_date)
                    {
                        uteModuleWeatherGetData(weather_date);
                    }
                    if (weather_date->DayWeather[i] >> 8 == WEATHER_TYPE_UNKNOWN)
                    {
                        compo_number_set_zero_replace(num, true);
                    }
                    else
                    {
                        compo_number_set_zero_replace(num, false);
                    }
