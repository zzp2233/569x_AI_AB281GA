...
bool lp7812c_eoc_check_inbox(void)
{
    ...
    sta = (lp7812c.charge_mode_tick && !tick_check_expire(lp7812c.charge_mode_tick, 2000)); //该时间段内满电可认为出仓
...
void lp7812c_init(void)
{
    ...
    lp7812c_work_mode_set(BOX_COMM_MODE);
    bsp_hw_timer_set(HW_TIMER3, 29, timer3_isr);
}
