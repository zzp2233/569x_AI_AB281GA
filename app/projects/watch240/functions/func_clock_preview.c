...
#define CUR_PREVIEW_STYLE UTE_MODULE_WATCH_PREVIEW_STYLE
#else
...
#elif (CUR_PREVIEW_STYLE == PREVIEW_ROWBOX_STYLE)
...
compo_form_t *func_clock_preview_form_create(void)
{
...
#ifdef UI_BUF_DIALPLATE_PREVIEW_BOX_BIN
    compo_picturebox_t *picSelect = compo_picturebox_create(frm, UI_BUF_DIALPLATE_PREVIEW_BOX_BIN);
    compo_picturebox_set_pos(picSelect, GUI_SCREEN_CENTER_X, GUI_SCREEN_CENTER_Y);
    compo_setid(picSelect, COMPO_ID_PIC_SELECT);
#else
    ...
    uint16_t total_width = img_area.wid * dialplate_max_num;
...
#ifdef UI_BUF_DIALPLATE_PREVIEW_BOX_BIN
    compo_picturebox_t *picboxSelect = compo_getobj_byid(COMPO_ID_PIC_SELECT);
    if (compo_rowbox_get_move_flag(rowbox) && compo_picturebox_get_visible(picboxSelect))
    {
        compo_picturebox_set_visible(picboxSelect, false);
    }
    if (!compo_rowbox_get_move_flag(rowbox) && !compo_picturebox_get_visible(picboxSelect))
    {
        compo_picturebox_set_visible(picboxSelect, true);
    }
#endif
...
static void func_clock_preview_message(size_msg_t msg)
{
    ...
    switch (msg)
    {
        ...
        //     uteModuleGuiCommonSetCurrWatchIndex(rowbox->img_idx[IMG_MAX / 2]);
...
#elif (CUR_PREVIEW_STYLE == PREVIEW_STACKLIST_STYLE)
