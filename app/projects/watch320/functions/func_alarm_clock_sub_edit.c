...
compo_form_t *func_alarm_clock_sub_edit_form_create(void)
{
    ...
    compo_button_set_location(btn, 160, 160, 320, 40);

    btn = compo_button_create(frm);
    compo_setid(btn, COMPO_ID_BTN_REPEAT);
    compo_button_set_location(btn, 160, 220, 320, 40);
	...
	compo_textbox_set_pos(txt, 30, 90);
    compo_textbox_set(txt, aclock_str);

    txt = compo_textbox_create(frm, 4);
    compo_textbox_set_align_center(txt, false);
	compo_textbox_set_pos(txt, 30, 140);
    compo_textbox_set(txt, "更改时间");

    txt = compo_textbox_create(frm, 4);
    compo_textbox_set_align_center(txt, false);
	compo_textbox_set_pos(txt, 30, 200);
    ...
    compo_picturebox_set_pos(pic_click, 283, 160);

    pic_click = compo_picturebox_create(frm, UI_BUF_ALARM_CLOCK_OPEN_BIN);
    compo_picturebox_set_pos(pic_click, 283, 220);
