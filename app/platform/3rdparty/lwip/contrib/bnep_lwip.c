...
static u8 *bnep_lwip_outgoing_buf;
...
static int bnep_lwip_netif_up(bd_addr_t network_address)
{
    ...
    bnep_lwip_outgoing_buf = mem_malloc(LWIP_OUTGOING_BUFFER_SIZE);
    if (!bnep_lwip_outgoing_buf) {
        log_error("bnep_lwip_netif_up: failed to allocate outgoing buffer\n");
        return -1;
    }
...
static int bnep_lwip_netif_down(void)
{
    ...
    if (bnep_lwip_outgoing_buf) {
        ...
        mem_free(bnep_lwip_outgoing_buf);
        bnep_lwip_outgoing_buf = NULL;
    }
    return 0;
}
