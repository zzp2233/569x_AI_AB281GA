...
void lowpwr_tout_ticks(void)
{
    ...
    if (uteDrvScreenCommonIsInit() && get_tft_spi_timeout()
        ...
        && !uteDrvScreenEsdTeIntErrorCheckIsErrorState()
#endif
       )
    {
...
//     }
    ...
    if(uteModuleGuiCommonIsInScreenSaver())
    {
        ...
        GPIOEDE |= BIT(3);
#endif
        GPIOADE |= BIT(0) | BIT(1) | BIT(2) | BIT(3) | BIT(4) | BIT(5) | BIT(7);
    }
#endif
...
//    GPIOFDE = pf_keep;
    ...
    while(bt_is_sleep())
    {
        ...
        if(uteModuleGuiCommonIsUpdateScreenSaver() && uteModuleGuiCommonIsInScreenSaver())
        {
            printf("update screen saver\n");
            break;
        }
...
bool sleep_process(is_sleep_func is_sleep)
{
    ...
    if ((*is_sleep)())
    {
        ...
        if(uteModulePlatformNotAllowSleep()) //ute add
        {
            ...
            {
#if UTE_MODULE_SCREENS_SCREEN_SAVER_SUPPORT
                if (uteModuleGuiCommonCurrIsScreenSaverTimeBucket() && !uteModuleGuiCommonIsInScreenSaver())
                {
        ...
        if (sys_cb.sleep_delay == 0 && !sleep_cb.sys_is_sleep)
        {
            ...
            {
#if UTE_MODULE_SCREENS_SCREEN_SAVER_SUPPORT
                if (!sys_cb.gui_sleep_sta && uteModuleGuiCommonCurrIsScreenSaverTimeBucket() && !uteModuleGuiCommonIsInScreenSaver())
                {
                    uteTaskGuiStartScreen(FUNC_CLOCK_SAVER, 0, __func__);
                    reset_sleep_delay_all();
                    return false;
