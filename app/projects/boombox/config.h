...
#define GUI_TE_WIRE_EN                  0                       //屏幕是否带TE脚,默认0，开TE
#define CTP_SENSOR_IIC_EN               0                       //tp和sensor是否共用iic,默认0，不共用
#define GUI_LOCK_WIRE_EN                1                       //童锁 默认0，关
...
#define BT_TWS_MS_SWITCH_EN             0   //是否支TWS主从切换（暂不支持）
#define BLE_TWS_MS_SWITCH_EN            0   //是否支BLE主从切换（暂不支持）
...
#define LE_ANCS_MANUAL_EN               0   //是否需要手动打开ancs, 需要调用发起ancs连接的相关接口
//AMS
...
#define FLASH_PKG_START                 UTE_OTA_TMP_ADDRESS//0x400000   //升级压缩包存放起始地址
#define FLASH_PKG_SIZE                  UTE_OTA_TMP_SIZE//0x300000   //升级压缩包大小
...
#define BT_SCO_FAR_NR_EN                1                           //是否打开远端降噪算法(Code: 5.5KB, Ram: 2.1KB)
...
#define DAC_AUTO_ONOFF_EN               0                           //是否DAC自动关闭模拟电源（不播放时自动关掉省电）

/*****************************************************************************
* Module    : 音频压缩算法配置
******************************************************************************/
#define OPUS_ENC_EN                     1   //是否打开opus压缩算法
...
#define WARNING_WAV_EN                  0            //是否支持wav提示音（可以和音乐、通话叠加播放）
...
#define WARNING_BT_INCALL               0            //是否打开蓝牙来电提示音
...
#define SECURITY_TRANSITCODE_EN         0   //打开乘车码功能
...
#define FLASH_CODE_SIZE                 1428K                   //程序使用空间大小
#undef  FLASH_UI_BASE
#define FLASH_UI_BASE                   0x165000                //UI资源起始地址
#undef  FLASH_UI_SIZE
#define FLASH_UI_SIZE                   0x296000                //UI资源大小
#undef  HEAP_FUNC_SIZE
#define HEAP_FUNC_SIZE                  1024                    //FUNC HEAP SIZE
...
#define MEM_HEAP_SIZE                   12*1024 + (2048 + 512 + 4096 + 4096 + 1024*1)*SECURITY_PAY_EN           //用于分配全部线程的总栈内存大小, 如果设置为 0, 采用中科平台库内置默认大小 16*1024
#define OS_THREAD_MAIN_STACK            1536 + (2048 + 512 + 4096 + 4096 + 1024*1)*SECURITY_PAY_EN            //分配main线程栈内存大小, 如果设置为 0, 采用中科平台库内置默认大小 2048 + 1536

#elif SECURITY_PAY_EN
#undef  FLASH_SIZE
#define FLASH_SIZE                      FSIZE_8M                //根据芯片信息配置实际FLASH SIZE
#undef  FLASH_CODE_SIZE
#define FLASH_CODE_SIZE                 700K                    //程序使用空间大小
#undef  FLASH_UI_BASE
#define FLASH_UI_BASE                   0xC0000                 //UI资源起始地址
#undef  sscanf

#define MEM_HEAP_SIZE                   16*1024 + (2048 + 512) * SECURITY_PAY_EN + (512) * OPUS_ENC_EN           //用于分配全部线程的总栈内存大小, 如果设置为 0, 采用中科平台库内置默认大小 16*1024
#define OS_THREAD_MAIN_STACK            2048 + 1536 + (2048 + 512) * SECURITY_PAY_EN        //分配main线程栈内存大小, 如果设置为 0, 采用中科平台库内置默认大小 2048 + 1536
...
#define SDA_STATUS()                    (GPIOH & BIT(4))
#define CHARGE_ENABLE_GPIO              IO_PH5              //充电启停控制脚
#endif

/*****************************************************************************
 * Module    :  智能充电仓UART通讯 功能选择
 *****************************************************************************/
#define SMART_VHOUSE_HOST_EN            1               //是否打开智能充电仓串口主机功能
