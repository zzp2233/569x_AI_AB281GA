...
void uteModuleFactoryTestEverySecond(void)
{
    ...
    if (uteModuleFactoryTestData.currItem == TEST_ITEM_VCXX_CHECK_LIGHT)
    {
        if (uteModuleFactoryTestData.u.vcxx.testMode == FACTORY_VCXX_TEST_MODE_BIO_B)
        {
            ...
            if (uteModuleFactoryTestData.u.vcxx.bioDiff >= bio_min && uteModuleFactoryTestData.u.vcxx.bioDiff <= bio_max)
            {
 ...
 * @brief        设置VK心率活体bioֵ
 * @details
 * @param[in]    bioData bioֵ
 * @param[in]    isAir 是否对空 true对空 false对导电布
 * @return       void*
 * @author       Wang.Luo
 * @date         2025-08-19
 */
void uteModuleFactoryTestSetVkBioData(uint16_t bioData, bool isAir)
{
    if (isAir)
    {
        uteModuleFactoryTestData.u.vcxx.bioA = bioData;
    }
    else
    {
        uteModuleFactoryTestData.u.vcxx.bioB = bioData;
    }
}

void uteModuleFactoryTestGetVkBioData(uint16_t *bioA, uint16_t *bioB, uint16_t *bioDiff)
{
    uteModuleFactoryTestData.u.vcxx.bioDiff = abs(uteModuleFactoryTestData.u.vcxx.bioA - uteModuleFactoryTestData.u.vcxx.bioB);
    (*bioDiff) = uteModuleFactoryTestData.u.vcxx.bioDiff;
    (*bioA) = uteModuleFactoryTestData.u.vcxx.bioA;
    (*bioB) = uteModuleFactoryTestData.u.vcxx.bioB;
}
