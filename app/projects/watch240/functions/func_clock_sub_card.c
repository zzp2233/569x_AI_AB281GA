...
static const ui_handle_t ui_handle =
{
    ...
    .card2 = {                                                      // 定义第二个卡片（睡眠数据卡片）的配置结构体
        ...
        .x  = SUB_CARD_X+SUB_CARD_W/2,                              // 设置卡片的X坐标：16+208/2=120像素，这是卡片中心点的X位置
        .y  = 190+(SUB_CARD_H+6)*1,                                 // 设置卡片的Y坐标：190+(94+6)*1=290像素，相对于第一个卡片向下偏移100像素
        .w  = SUB_CARD_W,                                           // 设置卡片宽度为SUB_CARD_W（208像素）
        .h  = SUB_CARD_H,                                           // 设置卡片高度为SUB_CARD_H（94像素）
        .rect = {                                                   // 定义卡片背景矩形的配置
            .idx    = 0,                                            // 矩形索引号为0，用于标识这是第一个矩形组件
    ...
    .card4 = {                                                      // 定义第四个卡片（定时器卡片）的配置结构体
        ...
        .x  = SUB_CARD_X+SUB_CARD_W/2,                              // 设置卡片的X坐标：16+208/2=120像素，这是卡片中心点的X位置
        .y  = 190+(SUB_CARD_H+6)*2,                                 // 设置卡片的Y坐标：190+(94+6)*2=390像素，相对于第一个卡片向下偏移200像素
        .w  = SUB_CARD_W,                                           // 设置卡片宽度为SUB_CARD_W（208像素）
        .h  = SUB_CARD_H,                                           // 设置卡片高度为SUB_CARD_H（94像素）
    ...
    .card5 = {                                                      // 定义第五个卡片（秒表卡片）的配置结构体
        ...
        .x  = SUB_CARD_X+SUB_CARD_W/2,                              // 设置卡片的X坐标：16+208/2=120像素，这是卡片中心点的X位置
        .y  = 190+(SUB_CARD_H+6)*3,                                 // 设置卡片的Y坐标：190+(94+6)*3=490像素，相对于第一个卡片向下偏移300像素
        .w  = SUB_CARD_W,                                           // 设置卡片宽度为SUB_CARD_W（208像素）
        .h  = SUB_CARD_H,                                           // 设置卡片高度为SUB_CARD_H（94像素）
    ...
    .card7 = {                                                      // 定义第七个卡片（最近应用卡片）的配置结构体
        ...
        .x  = SUB_CARD_X+SUB_CARD_W/2,                              // 设置卡片的X坐标：16+208/2=120像素，这是卡片中心点的X位置
        .y  = 190+(SUB_CARD_H+6)*5+7,                               // 设置卡片的Y坐标：190+(94+6)*5+7=697像素，相对于第一个卡片向下偏移507像素
        .w  = SUB_CARD_W,                                           // 设置卡片宽度为SUB_CARD_W（208像素）
        .h  = SUB_CARD_H,                                           // 设置卡片高度为SUB_CARD_H（94像素）
...
static int func_clock_sub_card_get_y_info(int card_compo_id)    // 定义静态函数，根据卡片组件ID获取对应的Y坐标信息
{
    ...
    if (card_compo_id == ui_handle.card1.id)                    // 检查传入的ID是否匹配第一个卡片（运动数据卡片）的组件ID
    {
    ...
    if (card_compo_id == ui_handle.card6.id)                    // 检查传入的ID是否匹配第六个卡片（常用应用卡片）的组件ID
    {
...
typedef struct f_card_t_                                        // 定义卡片功能控制结构体类型
{
    ...
    u32 activity_tick;                                         // 运动数据更新时间戳，用于控制运动卡片数据刷新频率
    ...
    u8 activity_state;                                         // 运动数据动画状态，0=向上动画，1=向下动画，2=显示实际数据
...
/**
  * @brief  更新第一个卡片（运动数据卡片）的显示内容
  * @param  None
  * @retval None
  * @note   该函数实现运动数据卡片的动画效果和数据显示更新
  *         包含三个状态：向上动画(0)、向下动画(1)、显示实际数据(2)
  *         管理卡路里、距离、步数三个弧形进度条的显示
  */
static void card1_updata_disp(void)                             // 定义运动数据卡片显示更新函数
{
...
//        f_activity->activity_tick = tick_get();               // 更新时间戳（已注释）
    ...
    compo_cardbox_t *cardbox = compo_getobj_byid(ui_handle.card1.id);    // 获取运动数据卡片容器组件指针
...
static void func_clock_sub_card_compo_update(s32 ofs_y, bool creating) // 定义组件更新函数
{
    ...
    while (compo != NULL)                                       // 遍历所有组件
    {
        ...
        switch (compo->id)                                      // 根据组件ID获取对应的初始位置和尺寸信息
        {
            ...
            case COMPO_ID_CARD_1:                               // 第一个卡片（运动数据卡片）
            ...
            case COMPO_ID_CARD_6:                               // 第六个卡片（常用应用卡片）
...
//    rect_t rect;                                              // 矩形结构体（已注释，可能用于位置检测）
    ...
    {
        case COMPO_ID_CARD_1 :                                  // 处理第一个卡片（运动数据/计算器卡片）的点击
...
static void func_clock_sub_card_data_update(void)             // 定义数据更新函数
{
    ...
    {
        // card1_updata_disp();                                // 运动数据卡片更新（已注释）
    }
