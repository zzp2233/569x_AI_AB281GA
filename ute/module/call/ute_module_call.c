...
void uteModuleCallBtPowerOff(UTE_BT_POWER_OFF_REASON reason)
{
    ...
    uteModuleCallData.isNeedStartOnePair = false;
...
#endif
            if (uteModuleCallData.isNeedStartOnePair)
            {
                if (bt_get_curr_scan() == BT_STA_SCANNING)
                {
                    uteModulePlatformSendMsgToUteApplicationTask(MSG_TYPE_MODULE_NOTIFY_ANCS_START_PAIR, 0);
                    uteModuleCallData.isNeedStartOnePair = false;
                }
                else if (bt_get_curr_scan() > BT_STA_SCANNING)
                {
                    uteModuleCallData.isNeedStartOnePair = false;
                }
            }
...
#if (UTE_BT30_AUTO_POWRER_OFF_SUPPORT && !UTE_CONNECT_PC_AUDIO_TEST)
...
void uteModuleCallBtStartOnePair(void)
{
    uteModuleCallData.onePairTimeoutSec = UTE_MODULE_BT_ONCE_PAIR_TIMEOUT_SECOND;
    uteModulePlatformDlpsDisable(UTE_MODULE_PLATFORM_DLPS_BIT_ONE_PAIR);
    ...
    {
        uteModuleCallBtPowerOn(UTE_BT_POWER_ON_NORMAL);
        uteModuleCallData.isNeedStartOnePair = true;
    }
    else if (bt_get_curr_scan() != BT_STA_SCANNING)
    {
        uteModuleCallData.isNeedStartOnePair = true;
...
void uteModuleCallSyncAddressBookData(uint8_t *receive,uint8_t length)
{
    ...
    UTE_MODULE_LOG(UTE_LOG_CALL_LVL, "%s,receive_onece_dataNum = %d", __func__,receive_onece_dataNum);
...
void uteModuleCallParseAddressBookContactNameAndNumber(uint8_t *data, uint8_t *name, uint8_t *nameLen, uint8_t *number, uint8_t *numberLen)
{
    ...
    contactNumberLen = data[0] > UTE_MODULE_CALL_ADDRESSBOOK_NUMBER_MAX_LENGTH ? UTE_MODULE_CALL_ADDRESSBOOK_NUMBER_MAX_LENGTH : data[0];
    contactNameLen = data[1] > UTE_MODULE_CALL_ADDRESSBOOK_NAME_MAX_LENGTH ? UTE_MODULE_CALL_ADDRESSBOOK_NAME_MAX_LENGTH : data[1];
...
void uteModuleCallSetBeforeCallStatus(uint8_t status)
{
    ...
    UTE_MODULE_LOG(UTE_LOG_CALL_LVL, "%s,status=%d", __func__,status);
}

/**
 * @brief        获取通话状态
 * @details
 * @return       CALL_STATUS_T 来电或去电
 * @author       Wang.Luo
 * @date         2024-11-14
 */
uint8_t uteModuleCallGetBeforeCallStatus(void)
{
    UTE_MODULE_LOG(UTE_LOG_CALL_LVL, "%s,status=%d", __func__,uteModuleCallData.callData.beforeCallStatus);
    return uteModuleCallData.callData.beforeCallStatus;
}
...
CALL_UPDATE_END:
    ...
    uteModuleCallData.callData.numberSize = 0;
    uteModuleCallData.callData.nameSize = 0;
    memset(&uteModuleCallData.callData.recordTime, 0x00, sizeof(ute_module_systemtime_time_t));
    isBtPbapNameUpdate = false;
...
uint8_t uteModuleCallGetCallRecordsSize(ute_module_call_records_t *pData)
{
    ...
    if(pData != NULL)
    {
        for(uint8_t i=0; i<recordsCnt; i++)
        {
            ...
            UTE_MODULE_LOG(UTE_LOG_CALL_LVL, "%s,%04d-%02d-%02d %02d:%02d", __func__,pData[i].callTime.year,pData[i].callTime.month,pData[i].callTime.day, pData[i].callTime.hour,pData[i].callTime.min);
        }
    }
...
void uteModuleCallGetSosContact(ute_module_call_addressbook_t *pData)
{
    ...
    if (uteModuleFilesystemOpenFile(&path[0], &file, FS_O_RDONLY))
    {
        ...
        sizeof(ute_module_call_addressbook_t);
        uteModuleFilesystemCloseFile(file);
    }
}
