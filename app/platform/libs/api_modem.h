...
// time: 2024-11-13T09:55:17.250351200+08:00
//! header syncer start
...
void modem_huart_rx_cb(u16 cnt, u8 flag);
...
typedef enum
{
    ...
    /// @brief 没有初始化
    MODEM_ENOINIT,
    /// @brief 收到 modem 发送的错误 "+CME ERROR"
    MODEM_RECV_ERROR,
} modem_err_t;
...
/**
 * @brief modem_check_can_send 的回调，库中weak，外部可重定义
 *
 * @param err 发送检测的状态 @ref modem_err_t
 * @param err_code @ref modem_cmd_notice
 * @return true 可以发送
 * @return false 不能发送
 */
 ...
 *
 */
void modem_at_reset(void);

// call.h
enum
{
    /// @brief +IMSCPI, 呼叫信息主动上报 call_notice_info
    MODEM_CALL_NOTICE_INFO,
    /// @brief ^DSCI, 呼叫状态主动上报 call_notice_state
    MODEM_CALL_NOTICE_STATE,
    /// @brief ^ORIG, 呼叫发起指示 call_notice_start
    MODEM_CALL_NOTICE_START,
    /// @brief ^CONF, 网络连通指示 call_notice_net_conn
    MODEM_CALL_NOTICE_NET_CONN,
    /// @brief ^CONN, 呼叫接通指示 call_notice_call_conn
    MODEM_CALL_NOTICE_CALL_CONN,
    /// @brief ^CEND, 通话结束指示 call_notice_end
    MODEM_CALL_NOTICE_END,
    /// @brief +IMSPLUS, 呼叫扩展设置 call_notice_ring
    MODEM_CALL_NOTICE_RING,
    /// @brief +CLCC, 当前呼叫列表 call_notice_current_call
    MODEM_CALL_NOTICE_CURRENT_CALL,
    /// @brief +CLIP, 来电指示 call_notice_line_id_present
    MODEM_CALL_LINE_ID_PRESENT,
};
...
NOTICE_STRUCT(call_notice_current_call,
              ...
              /// @brief 呼叫号码
              char number[16];);
NOTICE_STRUCT(call_notice_line_id_present,
              /// @brief 呼叫号码
              char number[16];
 ...
 */
bool modem_call_dial(const char *number);
 ...
 * @return true: 发送成功; false: 发送失败
 */
bool modem_call_get_current_call(void);

/**
 * @brief DTMF and tone generation
 *
 * @param tone Max length is 29
 * @return true: 发送成功; false: 发送失败
 */
bool modem_call_send_dtmf(const char *tone);
...
bool modem_sms_submit_ucs2(const char *number, const u8 *ucs2_msg, u16 ucs2_len);

/**
 * @brief Send UTF8 message
 *
 * @param number Phone number
 * @param utf8_msg UTF8 data
 * @param buf Used to cache UCS2 data
 * @param buf_size
 *
 * @return true: 发送成功; false: 发送失败
 */
bool modem_sms_submit_utf8(const char *number, const char *utf8_msg, u8 *buf, u16 buf_size);
...
enum
{
    ...
    /// @brief +CESQ; mtcas_notice_req_esq
    MODME_MTCAS_NOTICE_REQ_ESQ,
    /// @brief +CMUSLOT; mtcas_notice_hot_plug_report
    MODME_MTCAS_NOTICE_HOT_PLUG,
};
...
NOTICE_STRUCT(mtcas_notice_req_esq,
              /// @brief 信号强度指示 无驻留时为0 中间态为99
              ///        0: rssi <= -110dBm
...
NOTICE_STRUCT(mtcas_notice_hot_plug_report,
              /// @brief 卡槽
              u8 slot;
              /// @brief 状态 0:拔卡 1:插卡
              u8 slotstate;);
 ...
 * @param to [1,255]
 */
bool modem_mtcas_get_contains(u16 from, u16 to);
 ...
 * @return true: 发送成功; false: 发送失败
 */
bool modem_mtcas_set_sq_report(u8 enable, u8 range);

/**
 * @brief Request extern signal quality
 *
 * @return true: 发送成功; false: 发送失败
 */
bool modem_mtcas_req_esq(void);
 ...
 * @return true: 发送成功; false: 发送失败
 */
bool modem_general_req_manu_id(void);
 ...
 * @return true: 发送成功; false: 发送失败
 */
bool modem_general_req_model_id(void);
 ...
 * @return true: 发送成功; false: 发送失败
 */
bool modem_general_req_psn_id(te_psn_t psn);
 ...
 * @return true: 发送成功; false: 发送失败
 */
bool modem_general_set_chset(te_chset_t chset);
 ...
 * @return true: 发送成功; false: 发送失败
 */
bool modem_general_req_imsi(void);
 ...
 * @return true: 发送成功; false: 发送失败
 */
bool modem_general_req_iccid(void);
...
enum
{
    ...
    /// @brief +COPS; net_notice_plmn_selection
 ...
 * @return true: 发送成功; false: 发送失败
 */
bool modem_net_read_eps_regist(void);
 ...
 * @return true: 发送成功; false: 发送失败
 */
bool modem_net_read_plmn_select(void);
//! header syncer end
