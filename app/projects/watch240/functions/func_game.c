...
#define GAME_LIST_CNT                       ((int)(sizeof(tbl_game_list) / sizeof(tbl_game_list[0])))

static const compo_listbox_item_t tbl_game_list[] =
{
#if UTE_MODULE_SCREENS_BIRD_SUPPORT
    {STR_FLY_BIRD,    UI_BUF_I341001_17_GAME_BIRD_GAME_BIN,      .func_sta = FUNC_BIRD}, //小鸟
#endif
#if UTE_MODULE_SCREENS_TETRIS_SUPPORT
    {STR_TETIRIS,    UI_BUF_I341001_17_GAME_TETRIS_BIN,         .func_sta = FUNC_TETRIS},  //俄罗斯方块
#endif
};
//创建海拔窗体
compo_form_t *func_game_form_create(void)
{
    ...
    compo_listbox_set(listbox, tbl_game_list, GAME_LIST_CNT);
...
static void func_game_process(void)
{
    ...
    if(f_game->listbox)
    {
        compo_listbox_move(f_game->listbox);
    }
    func_process();
}
...
static void func_game_click(void)
{
    int icon_idx;
    f_game_t *f_game = (f_game_t *)func_cb.f_cb;
    compo_listbox_t *listbox = f_game->listbox;
    u8 func_sta;

    icon_idx = compo_listbox_select(listbox, ctp_get_sxy());
    if (icon_idx < 0 || icon_idx >= GAME_LIST_CNT)
    {
        return;
    }

    //根据图标索引获取应用ID
    func_sta = tbl_game_list[icon_idx].func_sta;
    //切入应用
    if (func_sta > 0)
    {
        {
...
#if GUI_SCREEN_SIZE_368X448RGB_I345001_SUPPORT
    f_game_t *f_game = (f_game_t *)func_cb.f_cb;
    f_game->listbox = compo_getobj_byid(COMPO_ID_LISTBOX);
    compo_listbox_t *listbox = f_game->listbox;
