...
#include "ute_drv_boe_s240x296_i183_jd9853_hkoe1912534b01.h"
#elif UTE_DRV_AMOLED_S368X448_GC3A71_193WG2014V1_QSPI_SUPPORT
#include "ute_drv_amoled_s368x448_gc3a71_193wg2014v1_qspi.h"
#endif
...
#elif DRV_BOE_S240X296_I183_JD9853_HKOE1912534B01_QSPI_SUPPORT
    uteDrvScreenCommonFunction = &uteDrvScreenBoe240X296Hkoe1912534b01Config;
#elif UTE_DRV_AMOLED_S368X448_GC3A71_193WG2014V1_QSPI_SUPPORT
    uteDrvScreenCommonFunction = &uteDrvScreenAmoledS368X448GC3A71193WG2014V1Config;
#else
...
/**
*@brief   amoled在idle模式下的背光设置,跟随系统背光亮度
*@details
*@param[in]
*@author        cxd
*@date        2022-05-04
...
{
    // if (uteDrvScreenCommonFunction->setBackLightIdleMode)
    // {
    //     uteDrvScreenCommonFunction->setBackLightIdleMode(UTE_MODULE_SCREEN_SAVE_DEFAULT_LIGHT_PERCENT);
    // }
    if (uteDrvScreenCommonFunction->setBackLight)
    {
        os_gui_draw_w4_done();
        uteDrvScreenCommonFunction->setBackLight(UTE_MODULE_SCREEN_SAVE_DEFAULT_LIGHT_PERCENT);
    }
}
/**
*@brief   进入息屏模式
...
{
    if (uteDrvScreenCommonFunction->idleMode)
    {
        os_gui_draw_w4_done();
        uteDrvScreenCommonFunction->idleMode(enable);
    }
}
#endif
...
*@date       2022-05-17
*/
__SCREEN_COMMON void uteDrvScreenEsdTeIntErrorCheckTimerCallback(void)
{
    if (sys_cb.gui_sleep_sta || tft_cb.tft_bglight_kick)
    {
        return;
    }
    if(uteDrvScreenEsdTeIntErrorCheckIsErrorState())
    {
...
__SCREEN_COMMON void uteDrvScreenEsdTeIntErrorCheckCntReset(void)
{
    screenEsdTeIntErrorCheckCnt = 0;
}

__SCREEN_COMMON bool uteDrvScreenEsdTeIntErrorCheckIsErrorState(void)
{
    if(screenEsdTeIntErrorCheckCnt >= 2)
    {
        return true;
    }
    else
