...
void uteModuleMessageUteApplicationTaskHandler(ute_task_application_message_t *msg)
{
    ...
    switch (type)
    {
        ...
        case MSG_TYPE_MODULE_SPORT_GPS_LOCATION_STATE:
        {
            uteModuleSportGPSMsgHandler(param);
        }
        break;
#endif
#if UTE_MODUEL_QUICK_REPLY_SUPPORT
        case MSG_TYPE_MODULE_QUICK_REPLY_READ_DATA:
        {
            uteModuleQuickReplyReadContentData();
        }
        break;
#endif
        case TO_APP_TASK_MSG_UPDATE_CONNECTION_PARAM_TIMEOUT_TIMER:
        {
            uteModulePlatformUpdateConnectionParamTimeoutMsgHandler();
        }
        break;
#if UTE_MODULE_PROTOCOL_HANDLER_INTO_USER_TASK_SUPPORT
        case MSG_TYPE_PROTOCOL_HANDLER:
        {
            uteModuleProfileBleProtocolHandler();
        }
        break;
#endif
        default:
