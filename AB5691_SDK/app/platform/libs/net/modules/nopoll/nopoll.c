...
#   endif
	...
	return net_vsnprintf (NULL, 0, format, args) + 1;
#endif
}
...
#    else
int net_vsnprintf(char *buffer, uint maxlen, const char *format, va_list param);
	size = net_vsnprintf (result, size + 1, chunk, args);
#    endif
#endif
	/* return the result */
	return result;
...
void delay_5ms(unsigned int n);
	uint32_t n = microseconds / 5 / 1000;
	if (n == 0) n = 1;
	delay_5ms(n);
