...
//转场的起始Alpha
#if UTE_MODULE_GUI_SCREEN_SWITCHING_LR_ALPHA
#define SWITCHING_LR_ALPHA              UTE_MODULE_GUI_SCREEN_SWITCHING_LR_ALPHA
#else
...
static bool func_switching_lr_alpha(u16 switch_mode, bool flag_auto, void* param)
{
...
#if EN_CUM_SWITCH_LR_TRANSLATION || EN_CUM_SWITCH_LR_UNIL_SCALE
    if (switch_mode == FUNC_SWITCH_LR_ZOOM_LEFT || switch_mode == FUNC_SWITCH_LR_ZOOM_RIGHT
#if EN_CUM_SWITCH_LR_UNIL_SCALE
        || switch_mode == FUNC_SWITCH_LR_UNIL_ZOOM_LEFT || switch_mode == FUNC_SWITCH_LR_UNIL_ZOOM_RIGHT
#endif
       )
    {
    ...
    else
    {
        ...
        compo_form_set_alpha(frm_cur, 255);
    }
#else
    compo_form_scale_to(frm_cur, SWITCHING_LR_WIDTH, SWITCHING_LR_HEIGHT);
    compo_form_set_alpha(frm_cur, SWITCHING_LR_ALPHA);
...
static bool func_switching_lr(u16 switch_mode, bool flag_auto)
{
...
#if EN_CUM_SWITCH_LR_UNIL_SCALE
    else if (switch_mode == FUNC_SWITCH_LR_UNIL_ZOOM_LEFT || switch_mode == FUNC_SWITCH_LR_UNIL_ZOOM_RIGHT)
    {
        x_distance = SWITCHING_LR_DISTANCE + (GUI_SCREEN_WIDTH / 12);     //带缩放的切换，两个场景中心距离要稍微近一点
    }
#endif
    else
    {
    ...
    switch (switch_mode)
    {
        ...
        case FUNC_SWITCH_LR_ZOOM_LEFT:
#if EN_CUM_SWITCH_LR_UNIL_SCALE
        case FUNC_SWITCH_LR_UNIL_ZOOM_LEFT:
#endif
            //左划，右侧任务向左移入
            xpos2_start = GUI_SCREEN_CENTER_X + x_distance;
            dx_auto = -SWITCHING_LR_STEP;
        ...
        case FUNC_SWITCH_LR_ZOOM_RIGHT:
#if EN_CUM_SWITCH_LR_UNIL_SCALE
        case FUNC_SWITCH_LR_UNIL_ZOOM_RIGHT:
#endif
            //右划，左侧任务向右移入
            xpos2_start = GUI_SCREEN_CENTER_X - x_distance;
            dx_auto = SWITCHING_LR_STEP;
