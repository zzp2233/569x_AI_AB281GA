...
void uteDrvHeartVcxxStopPressureSample(uint8_t stopReasion)
{
    ...
    {
        uteModuleEmotionPressureData.vkPressureValue = vkPressureData.strData + uteModuleHeartGetHeartValue() / 3; // 算法出值太小，使用心率补充
    }
    else
    {
        ...
        {
            uteModuleEmotionPressureData.vkPressureValue = uteModuleHeartGetHeartValue() / 3;
        }
        else
        {
...
#endif
    uteModuleEmotionPressureReadLastTestInfo();
...
void uteModuleEmotionPressureStartSingleTestingMsgHandler(uint32_t param)
{
    ...
    uteModuleEmotionPressureData.stopReasion = EP_STOP_REASION_TESTING;
#if 0 // UTE_MODULE_PRESSURE_MAX_AND_MIN_VAULE_SUPPORT
    uteModuleEmotionPressureData.curDayPressureMaxValue = 0;
    uteModuleEmotionPressureData.curDayPressureMinValue = 0xff;
    uteModuleEmotionPressureData.curDayPressureAvgValue = 0;
...
{
    uteModuleEmotionPressureData.stopReasion = (uint32_t)param;
    ...
    uteDrvHeartVcxxStopPressureSample(uteModuleEmotionPressureData.stopReasion);
    if ((uteDrvHeartVcxxGetVkPressureValue() == 0) || (uteDrvHeartVcxxGetVkPressureValue() == 0xff))
    {
        uteModuleEmotionPressureData.stopReasion = EP_STOP_REASION_TIMEOUT;
    }
#endif

    if ((!uteModuleEmotionPressureData.isEmotionPressureAutoTestFlag) && (uteModuleEmotionPressureData.stopReasion == EP_STOP_REASION_SUCCESS))
    {
        uteDrvMotorStart(UTE_MOTOR_DURATION_TIME, UTE_MOTOR_INTERVAL_TIME, 1);
    }
    ...
    stopCmd[5] = uteModuleEmotionPressureData.stopReasion;
    ...
    {
        if (uteModuleEmotionPressureData.stopReasion != EP_STOP_REASION_SUCCESS)
        {
            uteModuleEmotionPressureData.emotionValue = 0xFF;
            uteModuleEmotionPressureData.pressureValue = 0xFF;
            uteModuleEmotionPressureData.fatigueValue = 0xFF;
...
{
    UTE_MODULE_LOG(UTE_LOG_EMOTION_PRESSURE_LVL, "%s,stopReasion=%d", __func__, uteModuleEmotionPressureData.stopReasion);
    return uteModuleEmotionPressureData.stopReasion;
}
...
{
    uteModuleEmotionPressureData.lastTestInfo.lastEmotionValue = lastEmotionValue;
    uteModuleEmotionPressureData.lastTestInfo.lastPressureValue = lastPressureValue;
    uteModuleEmotionPressureData.lastTestInfo.lastFatigueValue = lastFatigueValue;
    uteModuleEmotionPressureData.lastTestInfo.lastTestTime.year = time.year;
    uteModuleEmotionPressureData.lastTestInfo.lastTestTime.month = time.month;
    uteModuleEmotionPressureData.lastTestInfo.lastTestTime.day = time.day;
    uteModuleEmotionPressureData.lastTestInfo.lastTestTime.hour = time.hour;
    uteModuleEmotionPressureData.lastTestInfo.lastTestTime.min = time.min;
    uteModuleEmotionPressureData.lastTestInfo.lastTestTime.sec = time.sec;

    void *file;
    if (uteModuleFilesystemOpenFile(UTE_MODULE_FILESYSTEM_SYSTEMPARM_EMOTION_PRESSURE_LAST_INFO, &file, FS_O_WRONLY | FS_O_CREAT | FS_O_TRUNC))
    {
        uteModuleFilesystemWriteData(file, &uteModuleEmotionPressureData.lastTestInfo, sizeof(ute_module_emotion_pressure_test_last_info_t));
        uteModuleFilesystemCloseFile(file);
    }

    UTE_MODULE_LOG(UTE_LOG_HEART_LVL, "%s,lastEmotionValue=%d,lastPressureValue=%d,lastFatigueValue=%d,year=%d,month=%d,day=%d,hour=%d,min=%d,sec=%d", __func__, lastEmotionValue, lastPressureValue, lastFatigueValue, time.year, time.month, time.day, time.hour, time.min, time.sec);
}
...
{
    uteModuleEmotionPressureData.lastTestInfo.lastEmotionValue = 0;
    uteModuleEmotionPressureData.lastTestInfo.lastPressureValue = 0;
    uteModuleEmotionPressureData.lastTestInfo.lastFatigueValue = 0;
    uteModuleEmotionPressureData.lastTestInfo.lastTestTime.year = 0;
    uteModuleEmotionPressureData.lastTestInfo.lastTestTime.month = 0;
    uteModuleEmotionPressureData.lastTestInfo.lastTestTime.day = 0;
    uteModuleEmotionPressureData.lastTestInfo.lastTestTime.hour = 0;
    uteModuleEmotionPressureData.lastTestInfo.lastTestTime.min = 0;
    uteModuleEmotionPressureData.lastTestInfo.lastTestTime.sec = 0;

    void *file;
    if (uteModuleFilesystemOpenFile(UTE_MODULE_FILESYSTEM_SYSTEMPARM_EMOTION_PRESSURE_LAST_INFO, &file, FS_O_RDONLY))
    {
        uteModuleFilesystemSeek(file, 0, FS_SEEK_SET);
        uteModuleFilesystemReadData(file, &uteModuleEmotionPressureData.lastTestInfo, sizeof(ute_module_emotion_pressure_test_last_info_t));
        uteModuleFilesystemCloseFile(file);
    }

    UTE_MODULE_LOG(UTE_LOG_HEART_LVL, "%s,lastEmotionValue=%d,lastPressureValue=%d,lastFatigueValue=%d,year=%d,month=%d,day=%d,hour=%d,min=%d,sec=%d", __func__, uteModuleEmotionPressureData.lastTestInfo.lastEmotionValue, uteModuleEmotionPressureData.lastTestInfo.lastPressureValue, uteModuleEmotionPressureData.lastTestInfo.lastFatigueValue, uteModuleEmotionPressureData.lastTestInfo.lastTestTime.year, uteModuleEmotionPressureData.lastTestInfo.lastTestTime.month, uteModuleEmotionPressureData.lastTestInfo.lastTestTime.day, uteModuleEmotionPressureData.lastTestInfo.lastTestTime.hour, uteModuleEmotionPressureData.lastTestInfo.lastTestTime.min, uteModuleEmotionPressureData.lastTestInfo.lastTestTime.sec);
}
...
{
    memcpy(info, &uteModuleEmotionPressureData.lastTestInfo, sizeof(ute_module_emotion_pressure_test_last_info_t));
}
...
void uteModuleEmotionPressureAutoSaveData(void)
{
    ...
    uteModuleEmotionPressureSetLastTestInfo(uteModuleEmotionPressureData.emotionValue, uteModuleEmotionPressureData.pressureValue, uteModuleEmotionPressureData.fatigueValue, time);
}
/**
...
bool uteModuleEmotionPressureLoadTodayPressureHistoryData(uint8_t *pressureHistoryGraph, uint8_t pressureHistoryGraphCount, uint8_t *pressureHistoryData, uint8_t pressureHistoryDataLen)
{
    ...
    memset(tempPressureHistoryData, 0, sizeof(tempPressureHistoryData));
#if 0//UTE_MODULE_PRESSURE_MAX_MIN_AVG_FOLLOW_HISTOGRAM_SUPPORT
    uteModuleEmotionPressureData.curDayPressureMaxValue = 0;
...
bool uteModuleEmotionPressureGetTodayPressureHistoryData(uint8_t *pressureHistoryGraph, uint8_t pressureHistoryGraphCount)
{
    ...
    if (!isHasData)
    {
        ...
        uteModuleEmotionPressureData.curDayPressureMaxValue = 0;
        uteModuleEmotionPressureData.curDayPressureMinValue = 0;
        uteModuleEmotionPressureData.curDayPressureAvgValue = 0;
#endif
        if (uteModuleFilesystemOpenFile((char *)&path[0], &file, FS_O_RDONLY))
        {
            ...
            for (uint16_t i = 0; i < 144; i++)
            {
                ...
                if (pressureHistoryData[i] > 0 && pressureHistoryData[i] < 255)
                {
                    ...
                    pressureSum += pressureHistoryData[i];
                    pressureCnt++;
                }
#endif
            }
...
#elif UTE_MODULE_PRESSURE_MAX_MIN_AVG_FOLLOW_HISTOGRAM_SUPPORT
            uteModuleEmotionPressureData.curDayPressureMaxValue = maxValue;
            uteModuleEmotionPressureData.curDayPressureMinValue = minValue == 0xff ? 0 : minValue;
            uteModuleEmotionPressureData.curDayPressureAvgValue = pressureCnt > 0 ? pressureSum / pressureCnt : 0;
#endif
        }
    }
