...
compo_form_t *func_power_on_form_create(void)
{
    ...
    compo_picturebox_t *pic = compo_picturebox_create(frm, UI_BUF_I341001_1_START_LOGO_GIF_BIN);///背景图片
    compo_picturebox_cut(pic, 0, 36);
    compo_picturebox_set_pos(pic,GUI_SCREEN_CENTER_X, GUI_SCREEN_CENTER_Y);
    compo_setid(pic, COMPO_PIC_ID);
...
{
    f_power_on_t *f_power_on = (f_power_on_t *)func_cb.f_cb;
    if(tick_check_expire(f_power_on->tick, 100))
    {
        compo_picturebox_t *pic  = compo_getobj_byid(COMPO_PIC_ID);
        ...
        if(f_power_on->pic_num_disp < 36)
        {
            compo_picturebox_cut(pic, f_power_on->pic_num_disp, 36);
        }
        else
        {
...
static void func_power_on_exit(void)
{
    ...
    if (bsp_charge_sta_get() == 1)
    {
        uteTaskGuiStartScreen(FUNC_CHARGE, 0, __func__);
    }
}
