...
#endif
    {FUNC_SLIDING_UNLOCK_SCREEN,        func_sliding_unlcok_screen},
};
...
void func_message(size_msg_t msg)
{
    switch (msg)
    {
        ...
        {
#if 0
            static bool test_tws_scan = false;
            if (test_tws_scan == false)
            {
                bt_tws_search_slave(10000);
                bt_tws_set_scan(0x03);

                test_tws_scan = true;
                printf("bt_tws_set_scan(0x03) >>> \n");
            }
            else
            {
                bt_tws_disconnect();
                bt_tws_set_scan(0x00);
                test_tws_scan = false;
                printf("bt_tws_set_scan(0x00) >>> \n");
            }
#endif
        }
        if (func_cb.flag_sort)
        ...
        case KU_LEFT:
//            bsp_piano_warning_play(WARNING_TONE, 2);
...
//
        ...
        case MSG_SYS_1S:
            //printf("bt_tws_get_scan %d sys_cb.mute %d \n",bt_tws_get_scan(),sys_cb.mute);
#if CALL_MGR_EN
...
void func_run(void)
{
    ...
    func_cb.tbl_sort[1] = FUNC_ECIG_SET_POWER;
    func_cb.tbl_sort[2] = FUNC_ACTIVITY;
    func_cb.tbl_sort[3] = FUNC_SLEEP;
    func_cb.tbl_sort[4] = FUNC_BT;
    func_cb.tbl_sort[5] = FUNC_COMPO_SELECT;
    func_cb.sort_cnt = 6;
