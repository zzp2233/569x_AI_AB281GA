...
//    bool val = true;
    ...
    char *send_data = (char *)SECURITY_MALLOC(256);
    ...
    printf("alipay_iot_dns request www.baidu.com ip : %d.%d.%d.%d\n", ip[0], ip[1], ip[2], ip[3]);
#if 1
    //创建套接字
    fd = alipay_iot_socket_create(ALIPAY_IOT_SOC_AF_INET, ALIPAY_IOT_SOC_SOCK_STREAM, 0);
    if(fd < 0)
    {
        printf("    *  *************  fd: %d \n", fd);
    ...
    sa.sa_family = ALIPAY_IOT_SOC_AF_INET;
    sa.data.sin_data.port = port;

    memcpy(sa.data.sin_data.ip, ip, sizeof(ip));
    //与目标建立连接
    int32_t res = alipay_iot_socket_connect(fd, &sa, sizeof(sa) < 0);
    if(res)
    {
        ALIPAY_TEST_LOG("alipay_iot_socket_connect(net->fd, &sa, sizeof(sa) WRONG: alipay_iot_socket_connect fail:%d", res);
    ...
    alipay_iot_fd_setResultbit(fd, &t_set1);
    printf("fd = %d is set to read\n", fd);
    if(alipay_iot_fd_isset(fd, &t_set1))
    {
        printf("set success\n");
    }
    ...
    ALIPAY_TEST_LOG("selectRet %d\n", selectRet);
    if (selectRet > 0)
    {
        char *buf = SECURITY_MALLOC(4096);
        int readLen = 0;
        memset(buf, 0, 4096);
        readLen = alipay_iot_socket_read(fd, buf, 4095);
        // readLen = read(3, buf, 4095);
