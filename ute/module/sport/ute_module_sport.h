...
#endif
/*! 活动时长,wang.luo 2025-07-14 */
#if APP_MODULE_SPORT_EXERCISE_TIME_SUPPORT
#define UTE_MODULE_FILESYSTEM_SPORT_EXERCISE_TIME_DIR "exerciseTime"
#endif
...
/*! 步数同步时间数据 dengli.lu, 2023-03-15  */
#define UTE_MODULE_FILESYSTEM_SYSTEMPARM_SPORT_STEP_SYNC_TIME_INFO "systemparam/stepSyncTime"
/*! 活动时长同步时间数据 dengli.lu, 2023-03-15  */
#define UTE_MODULE_FILESYSTEM_SYSTEMPARM_EXERCISE_TIME_SYNC_TIME_INFO "systemparam/exerciseTimeSyncTime"
/*! 多运动同步时间数据 dengli.lu, 2023-03-15  */
#define UTE_MODULE_FILESYSTEM_SYSTEMPARM_MORESPORTS_SYNC_TIME_INFO "systemparam/moreSportsSyncTime"
#endif

#if APP_MODULE_SPORT_TODAY_ADD_ALL_SPORT_OTHER_DATA_SUPPORT
#define UTE_MODULE_FILESYSTEM_SYSTEMPARM_MORESPORTS_OTHER_INFO "systemparam/moreSportsOther"
#endif
...
#define SPORTS_HEART_DATAS_CACHE_LENGTH 96
#define SPORTS_HEAD_DATAS_LENGTH        56
#define SPORTS_SEND_DATA_READ_LENGTH    192
#else
#define SPORTS_HEART_DATAS_CACHE_LENGTH 90
#define SPORTS_HEAD_DATAS_LENGTH        33
#define SPORTS_SEND_DATA_READ_LENGTH    170
#endif
...
#if APP_MODULE_SPORT_HEARTRATE_RANGE_SUPPORT
    uint8_t  saveInfoType; //保存什么类型的数据 0x00:HR;0x01:hr+pace+stepfre+lat+lon;0x02:hr+pace+stepfre;0x03:hr+speed+reserve+lat+lon;0x04:hr+speed+reserve
    uint8_t  maxTimeMinute; //最慢配速，2bytes 格式为”分+秒”。0x08代表8分，0x32代表50秒，平均配速的意思是，“移动一公里需要多少时间”；
    uint8_t  maxTimeSecond;
    uint8_t  minTimeMinute; //最快配速，2bytes 格式为”分+秒”。0x08代表8分，0x32代表50秒，平均配速的意思是，“移动一公里需要多少时间”；
    uint8_t  minTimeSecond;
    uint8_t  maxStepfrequency;  //最大步频
    uint8_t  minStepfrequency;  //最小步频
    uint8_t  avgStepfrequency;  //平均步频
    uint32_t hrWarmupCnt;           //热身 大于/等于（最大心率220-年龄）*0.5、 小于（最大心率220-年龄）*0.6
    uint32_t hrWeightReductionCnt;  //减肥 大于/等于（最大心率220-年龄）*0.6、 小于（最大心率220-年龄）*0.7
    uint32_t hrEnduranceCnt;        //耐力 大于/等于（最大心率220-年龄）*0.7、 小于（最大心率220-年龄）*0.8
    uint32_t hrAnaerobicCnt;        //无氧 大于/等于（最大心率220-年龄）*0.8、 小于（最大心率220-年龄）*0.9
    uint32_t hrExtremeCnt;          //极限 大于/等于（最大心率220-年龄）*0.9、等占比
    uint32_t totalTime;
#endif
...
#if APP_MODULE_SPORT_HEARTRATE_RANGE_SUPPORT
    uint8_t  realTimeStepfrequency;
    uint8_t  realTimeMinute; //实时配速，2bytes 格式为”分+秒”。0x08代表8分，0x32代表50秒，平均配速的意思是，“移动一公里需要多少时间”；
    uint8_t  realTimeSecond;
    uint8_t  realTimeSpeedInteger; //实时速度，（整数）
    uint8_t  realTimeSpeedDecimals; //实时速度，（小数）
    uint16_t avgSpeed;
    uint16_t stepfrequencyTotalCnt;
    uint32_t stepfrequencyTotalSum;
    uint16_t speedTotalCnt;
    uint32_t speedTotalSum;
#endif
    uint8_t lastKM;
    uint8_t lastKMPaceMinute;
    uint8_t lastKMPaceSecond;
    uint16_t lastKcal;
    uint16_t increaseKcal;
    uint32_t lasttotalSportTime;
...
typedef struct
{
    ...
    bool isTodayExerciseTimeTargetNotify;
...
typedef struct
{
    ...
    bool isExerciseTimeTargetNotifyOpen;//活动时长
    bool isExerciseTimeTargetHasNotify;
    //uint16_t currExerciseTimedMin;
    uint16_t exerciseTimeTargetSetMin;//分钟
...
typedef struct
{
    ...
    uint8_t everyHourExerciseTime[24];
    uint32_t weekDayTotalStep[7];
    uint16_t weekDayTotalKcal[7];
    uint16_t weekDayTotalExerciseTime[7];
...
#endif
    uint32_t sportChangeHourSteps;
    uint32_t sportChangeHourKcal;
#if APP_SYNC_RECORD_TIME_FLAG_SUPPORT
    ute_module_systemtime_measure_time_t exerciseTimeSyncTime;
#endif
...
typedef enum
{
    GPS_LACATION_IDLE = 0,  //定位初始状态
    GPS_LACATION_OK,    //定位成功
    GPS_LACATION_FAIL,    //定位失败
    GPS_LACATION_DISCONNECT,    //信号中断
    GPS_LACATION_ING,    //定位中
} GPS_STATUS_T;
typedef struct
{
    uint32_t latitudeSport;
    uint32_t longitudeSport;
    uint32_t gpsIncreaseDistance;   //单位：米
    uint32_t gpsTotalDistance;  //单位：米
    uint16_t gpsSpeed;
    GPS_STATUS_T gpsStatus;
    uint8_t gpsInvalidCnt;
    uint16_t gpsUsesecond;
    uint16_t gpsReserve;
} ute_module_sports_gps_data_t;
#endif
#if APP_MODULE_SPORT_HEARTRATE_RANGE_SUPPORT
typedef struct
{
    uint8_t saveIndex;          //运动数据buffer填充多少位
    uint8_t hrWarmup;           //热身 大于/等于（最大心率220-年龄）*0.5、 小于（最大心率220-年龄）*0.6
    uint8_t hrWeightReduction;  //减肥 大于/等于（最大心率220-年龄）*0.6、 小于（最大心率220-年龄）*0.7
    uint8_t hrEndurance;        //耐力 大于/等于（最大心率220-年龄）*0.7、 小于（最大心率220-年龄）*0.8
    uint8_t hrAnaerobic;        //无氧 大于/等于（最大心率220-年龄）*0.8、 小于（最大心率220-年龄）*0.9
    uint8_t hrExtreme;          //极限 大于/等于（最大心率220-年龄）*0.9、等占比
    uint8_t  lastsportDistanceInteger;  //运动量,距离（整数） //距离（2byte前一个字节是整数位，后一个字节是小数位）
    uint8_t  lastsportDistanceDecimals; //运动量,距离（小数）
    uint32_t lastSportStepCnt;  //记录每一个10秒开始的步数起始值
} ute_module_sports_calc_data_t;
#endif
#if APP_MODULE_SPORT_EXERCISE_TIME_SUPPORT
typedef struct
{
    uint32_t totalStepCnt;
    uint16_t totalKcalCnt;
    uint16_t totalTimeCnt;
    uint8_t totalDistanceInteger;
    uint8_t totalDistanceDecimals;
    uint16_t currHourKcalCnt;
    uint16_t currHourDistanceCnt;
} ute_module_sports_exercisetime_data_t;
#endif

typedef enum
{
    FULL_TYPE_IDLE = 0,         //空间足够
    FULL_TYPE_MAX_NUM,          //已达最大条数
    FULL_TYPE_NOT_ENOUGH,       //开始运动空间不足
    FULL_TYPE_NOT_ENOUGH_ING,   //运动中空间不足
} MEMORY_FULL_TYPE_T;
...
#if APP_MODULE_SPORT_HEARTRATE_RANGE_SUPPORT
    ute_module_sports_calc_data_t moreSportsCalc;
#endif
#if APP_MODULE_SPORT_GPS_TRANSLATE_SUPPORT
    ute_module_sports_gps_data_t gpsData;
#endif
#if APP_MODULE_SPORT_EXERCISE_TIME_SUPPORT
    uint16_t exerciseTime[13];//total 13 types
    uint8_t lastReadHour;
    uint8_t exerCiseHeartValue;
    uint8_t exerciseType;       //活动时长保存类型
    uint32_t lastMinuteSportStepCnt;
    uint32_t lastMinuteStepCnt;
    ute_module_sports_exercisetime_data_t exerciseData;
#endif
    uint32_t recentTotalTime;   //最近运动总时长
    uint32_t recentTotalKcal;   //最近运动总消耗
    uint32_t leftSpaceSize;  //可用来存储运动数据空间
    MEMORY_FULL_TYPE_T memoryFullType; //空间不足提醒类型
    uint16_t lastHourRecordKcal;  //上一小时记录的消耗值
    uint16_t lastHourRecordDistance;    //上一小时记录的距离
    uint32_t lastHourRecordStep;    //上一小时记录的步数
#if APP_SYNC_RECORD_TIME_FLAG_SUPPORT
    ute_module_systemtime_measure_time_t stepSyncTime;
    ute_module_systemtime_measure_time_t moreSportsSyncTime;
#endif
#if APP_MODULE_SPORT_TODAY_ADD_ALL_SPORT_OTHER_DATA_SUPPORT
    ute_sports_today_other_data_t todaySportOtherData;
#endif
