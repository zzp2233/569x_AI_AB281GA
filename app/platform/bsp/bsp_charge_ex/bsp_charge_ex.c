...
#if (CHARGE_EX_IC_SELECT == CHARGE_IC_LP7812C)
    bool l_inbox = !l_eoc;     //满电可认为拔出, 未满电认为插入
    bool r_inbox = !r_eoc;
...
//            printf("percent change time disallow[%d]\n", tick_pr);
    ...
    {
//        *percent_cb = cm_read8(PARAM_BOX_PERCENT);        //todo
//        if ((*percent_cb == 0) || (*percent_cb > 100) ||    //数值不合理
...
//        if (!sys_cb.bat_low) {
//            if (sys_cb.tft_bglight_kick || sys_cb.te_bglight_cnt || sys_cb.tft_bglight_frist_set) { //刚开机屏幕还没亮
//                duty_bkp = sys_cb.tft_bglight_duty ? sys_cb.tft_bglight_duty : BGLIGHT_LV_DEFAULT;
//                sys_cb.tft_bglight_duty = BOX_LPWR_BRIGHTNESS;
//            } else {
//                duty_bkp = sys_cb.tft_bglight_duty;
//                tft_bglight_set_level(BOX_LPWR_BRIGHTNESS, true);   //调低亮度
//            }
//            sys_cb.bat_low = true;
//            lpwr_change_tick = tick_get();
//            func_popup(FUNC_CHARGE);                                //弹出低电提示界面
//            bsp_smart_vhouse_cmd_enqueue_no_repeat(VHOUSE_CMD_PWROFF, LEFT_CHANNEL_USER);   //发关机码
