...
void uteDrvBatteryCommonChargerChangeCb(uint8_t state)
{
    ...
    else if(state < 2)
    {
        ...
        else if(uteDrvBatteryCommonData.lastLvl == 100)
        {
            uteDrvBatteryCommonData.chargerStatus = BAT_STATUS_CHARGED;
        }
#endif
        else if (uteDrvBatteryCommonData.chargingTimeoutSeconds >= UTE_MODULE_BATTERY_CHARGING_TIMEOUT_SECONDS)
        {
            uteDrvBatteryCommonData.chargerStatus = BAT_STATUS_CHARGED;
        }
        else
    ...
    {
        if(uteDrvBatteryCommonData.lastLvl < 100 && uteDrvBatteryCommonData.chargerTimeSecond < 30)
        {
            uteDrvBatteryCommonData.chargerStatus = BAT_STATUS_CHARGING;
        }
        else
 ...
 * @date         2025-07-30
 */
uint8_t uteDrvBatteryCommonCountBatLvl(uint16_t voltage)
{
    uint8_t lvl = 0;
...
uint8_t uteDrvBatteryCommonCalibrationBatLvl(uint16_t voltage,int16_t current,ute_drv_battery_charger_status_t status)
{
    ...
    {
        lvl = uteDrvBatteryCommonCountBatLvl(voltage);
        UTE_MODULE_LOG(UTE_LOG_DRV_BAT_LVL, "%s,lvl=%d,tmpVoltage=%d", __func__,lvl,voltage);
    ...
    else
    {
        if(uteDrvBatteryCommonData.chargerTimeSecond==-1)
        {
            ...
            uteDrvBatteryCommonData.chargedTimeSecond = 0;
            uteDrvBatteryCommonData.voltLvlFiltered = 0.0f;
            uteDrvBatteryCommonData.chargingTimeoutSeconds = 0;
        }
        ...
        uint8_t currentLvl = uteDrvBatteryCommonData.startChargerLvl + (uint8_t)(uteDrvBatteryCommonData.chargerMAS*100.0f/(3600.0f*(UTE_DRV_BATTERY_ELECTRICITY_POWER_MAH*UTE_MODULE_BATTERY_COMPENSATION_PERCENT)));
#else
        uint8_t currentLvl = uteDrvBatteryCommonData.startChargerLvl + (uint8_t)(uteDrvBatteryCommonData.chargerMAS*100.0f/(3600.0f*UTE_DRV_BATTERY_ELECTRICITY_POWER_MAH));
#endif
        if (status == BAT_STATUS_CHARGING)
        {
            if (currentLvl > 99)
            {
                if (uteDrvBatteryCommonData.chargingTimeoutSeconds < UTE_MODULE_BATTERY_CHARGING_TIMEOUT_SECONDS)
                {
                    currentLvl = 99;
                    uteDrvBatteryCommonData.chargingTimeoutSeconds++;
                }
                else
                {
                    currentLvl = 100;
                }
                UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL, "%s,chargingTimeoutSeconds=%d", __func__, uteDrvBatteryCommonData.chargingTimeoutSeconds);
            }
            else
            {
                uteDrvBatteryCommonData.chargingTimeoutSeconds = 0;
        ...
        // 电压结果
        uint8_t voltLvlRaw = uteDrvBatteryCommonCountBatLvl(voltage);
        // 正常滞后滤波，系数可调
        uteDrvBatteryCommonData.voltLvlFiltered = (voltLvlRaw * 2.0f + uteDrvBatteryCommonData.voltLvlFiltered * 8.0f) / 10.0f;
        uint8_t voltLvl = uteDrvBatteryCommonData.voltLvlFiltered;
        // 如果差值很小，直接使用原始值
        if (abs((uint8_t)uteDrvBatteryCommonData.voltLvlFiltered - voltLvlRaw) <= 2)
        {
            voltLvl = voltLvlRaw;
        }
        // 信任区间，渐进修正
        int16_t delta = (int16_t)currentLvl - (int16_t)voltLvl;

        // 电流结果偏低：把积分器起点抬高 1%，一次只修 1%，电压低时忽略 */
        // if (voltage > UTE_DRV_BATTERY_060 && delta < -20)
        // {
        //     uteDrvBatteryCommonData.startChargerLvl = MIN(voltLvl, uteDrvBatteryCommonData.startChargerLvl + 1);
        //     uteDrvBatteryCommonData.chargerMAS = 0; // 重置已积电量
        //     lvl = uteDrvBatteryCommonData.startChargerLvl;
        // }
        // // 电流结果偏高：不改变结果
        // else
        if (delta > 5)
        {
            lvl = uteDrvBatteryCommonData.lastLvl;
        }
        // 在区间内：完全信任电流结果
        else
        {
        ...
        UTE_MODULE_LOG(UTE_LOG_DRV_BAT_LVL, "%s,voltLvlRaw: %d, voltLvl: %d, voltLvlFiltered: %d, currentLvl: %d, delta: %d", __func__, voltLvlRaw, voltLvl, (uint8_t)uteDrvBatteryCommonData.voltLvlFiltered, currentLvl, delta);
    }
...
void uteDrvBatteryCommonUpdateBatteryInfo(void)
{
    ...
    else if (uteDrvBatteryCommonData.lvl > 90)
    {
        uteDrvBatteryCommonData.current = current * 50 / 100;
    }
    else if (uteDrvBatteryCommonData.lvl > 80)
    {
...
#if (!UTE_MODULE_BATTERY_CHARGED_DISPLAY_ON_SUPPORT)
    if ((uteDrvBatteryCommonData.chargerStatus == BAT_STATUS_CHARGING) && (uteDrvBatteryCommonData.lvl == 100) && uteDrvBatteryCommonData.chargingTimeoutSeconds < UTE_MODULE_BATTERY_CHARGING_TIMEOUT_SECONDS)
    {
#if UTE_MODULE_BATTERY_LINEAR_CONSUME_SUPPORT
        ...
        else
        {
            if(uteApplicationCommonIsStartupFinish())
            {
                ...
                if(uteModuleGuiCommonGetCurrentScreenId() == UTE_MOUDLE_SCREENS_NEW_FACTORY_AGING_ID ||
                   ...
                   uteModuleGuiCommonGetCurrentScreenId() == FUNC_ONLINE_FACTORY_TEST
                   ||
                   uteModuleGuiCommonGetCurrentScreenId() == UTE_MOUDLE_SCREENS_POWERON_ID)
                {
                    return;
                }
#endif
...
uint8_t uteDrvBatteryCommonGetBatteryIndex(uint8_t totalIndex)
{
    ...
    {
        if((uteDrvBatteryCommonData.lastLvl<UTE_DRV_BATTERY_LOW_POWER_PERECNT-1)&&(chargerStatus==BAT_STATUS_NO_CHARGE))
        {
            // 电池电量小于低电值，让电池显示感叹号
        }
        else
