...
ute_module_systemtime_measure_time_t uteModuleSleepSyncTime;
#endif
...
#if APP_SYNC_RECORD_TIME_FLAG_SUPPORT
    memset(&uteModuleSleepSyncTime, 0, sizeof(ute_module_systemtime_measure_time_t));
#endif
...
void uteModuleSleepStartSendSleepHistoryData(ute_module_systemtime_time_t time)
{
    ...
    if(uteModuleSleepSyncTime.year == 0)
    {
        diffday = 0;
    }
    else
    {
        diffday = uteModuleSystemtimeGetDiffDay(uteModuleSleepSyncTime.year,uteModuleSleepSyncTime.month,uteModuleSleepSyncTime.day,curryTime.year,curryTime.month,curryTime.day);
    }
    if((diffday >= 0)&&(diffday <= UTE_MODULE_SLEEP_DATA_MAX_DAYS))
    {
...
*@date        2022-03-08
*/
void uteModuleSleepReadSyncTimeInfo(void)
{
    /*! 读取数据*/
    void *file;
    uint8_t readbuff[7];
    memset(&readbuff[0],0,sizeof(readbuff));
    if(uteModuleFilesystemOpenFile(UTE_MODULE_FILESYSTEM_SYSTEMPARM_SLEEP_SYNC_TIME_INFO,&file,FS_O_RDONLY))
    {
        uteModuleFilesystemSeek(file,0,FS_SEEK_SET);
        uteModuleFilesystemReadData(file,&readbuff[0],7);
        uteModuleFilesystemCloseFile(file);
    }
    uteModuleSleepSyncTime.year = (readbuff[0] << 8) | readbuff[1];
    uteModuleSleepSyncTime.month = readbuff[2];
    uteModuleSleepSyncTime.day = readbuff[3];
    uteModuleSleepSyncTime.hour = readbuff[4];
    uteModuleSleepSyncTime.min = readbuff[5];
    uteModuleSleepSyncTime.sec = readbuff[6];
}
/**
*@brief  保存睡眠同步时间
*@details
...
*@date        2022-03-08
*/
void uteModuleSleepSaveSyncTimeInfo(void)
{
    /*! 保存到文件zn.zeng, 2021-08-20*/
    void *file;
    uint8_t writeBuff[7];
    memset(&writeBuff[0],0,sizeof(writeBuff));
    writeBuff[0] = uteModuleSleepSyncTime.year >> 8 & 0xff;
    writeBuff[1] = uteModuleSleepSyncTime.year & 0xff;
    writeBuff[2] = uteModuleSleepSyncTime.month;
    writeBuff[3] = uteModuleSleepSyncTime.day;
    writeBuff[4] = uteModuleSleepSyncTime.hour;
    writeBuff[5] = uteModuleSleepSyncTime.min;
    writeBuff[6] = uteModuleSleepSyncTime.sec;
    if (uteModuleFilesystemOpenFile(UTE_MODULE_FILESYSTEM_SYSTEMPARM_SLEEP_SYNC_TIME_INFO, &file, FS_O_RDONLY|FS_O_WRONLY|FS_O_CREAT))
    {
        uteModuleFilesystemSeek(file,0,FS_SEEK_SET);
...
*@date        2022-03-08
*/
void uteModuleSleepSetSyncTime(ute_module_systemtime_measure_time_t setTime)
{
    uteModuleSleepSyncTime.year = setTime.year;
    uteModuleSleepSyncTime.month = setTime.month;
    uteModuleSleepSyncTime.day = setTime.day;
    uteModuleSleepSyncTime.hour = setTime.hour;
    uteModuleSleepSyncTime.min = setTime.min;
    uteModuleSleepSyncTime.sec = setTime.sec;
    uteModuleSleepSaveSyncTimeInfo();
}
/**
*@brief  获取睡眠同步时间
*@details
...
*@date        2022-10-24
*/
void uteModuleSleepGetSyncTime(ute_module_systemtime_measure_time_t *time)
{
    memcpy(time, &uteModuleSleepSyncTime, sizeof(ute_module_systemtime_measure_time_t));
}
#endif
#if UTE_MODULE_CYWEE_MOTION_SUPPORT
/*
