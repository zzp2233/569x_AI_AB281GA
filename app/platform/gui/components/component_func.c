 ...
 * @return 返回组件的指针
 **/
void *ute_compo_getobj_byid_for_btn(u16 id)
{
    if (id == 0)
    {
        return NULL;
...
void compo_set_bonddata(component_t *compo, tm_t tm)
{
    ...
    if (sys_cb.weather_idx == WEATHER_TYPE_HEAVY_SNOW || sys_cb.weather_idx == WEATHER_TYPE_LIGHT_SNOW)
    {
        sys_cb.weather_idx = WEATHER_TYPE_SNOW;
    }
    else if (sys_cb.weather_idx == WEATHER_TYPE_OVERCAST)
    {
        sys_cb.weather_idx = WEATHER_TYPE_CLOUDY;
    }
    else if (sys_cb.weather_idx == WEATHER_TYPE_CLOUDY)
    {
        sys_cb.weather_idx = WEATHER_TYPE_OVERCAST;
    }
#endif
...
//            value = tmp_time_hour / 10;
        ...
        case COMPO_BOND_IMAGE_WEATHER:
            ...
            u8 radix = ((compo_picturebox_t *)compo)->radix - 1;
            if (value > radix)
            {
                value = 0;
            }
        ...
        case COMPO_BOND_DISTANCE_PROGRESS:
            ...
            if (compo->type == COMPO_TYPE_PICTUREBOX && sys_cb.distance_goal)
            {
                ...
                if (sys_cb.distance_cur < sys_cb.distance_goal && value >= radix)
                {
                    radix -= 1;
                }
                value = MAX(0, MIN(radix, value));
            }
#else
        ...
        case COMPO_BOND_KCAL_PROGRESS:
            ...
            if (compo->type == COMPO_TYPE_PICTUREBOX && sys_cb.kcal_goal)
            {
                ...
                if (sys_cb.kcal_cur < sys_cb.kcal_goal && value >= radix)
                {
                    radix -= 1;
                }
                value = MAX(0, MIN(radix, value));
        ...
        case COMPO_BOND_STEPS_PROGRESS:
            ...
            if (compo->type == COMPO_TYPE_PICTUREBOX && sys_cb.step_goal)
            {
                u8 radix = ((compo_picturebox_t *)compo)->radix - 1;
                value = ((compo_picturebox_t *)compo)->radix * sys_cb.step_cur / sys_cb.step_goal;
                if (sys_cb.step_cur < sys_cb.step_goal && value >= radix)
                {
                    radix -= 1;
                }
                value = MAX(0, MIN(radix, value));
        ...
        case COMPO_BOND_VBAT_PROGRESS:
            ...
            {
                value = uteDrvBatteryCommonGetBatteryIndex(((compo_picturebox_t*)compo)->radix);
            }
            sprintf(value_str, "%d", value);
            break;

        case COMPO_BOND_HEART_PROGRESS:
            value = 0;
            uint8_t HeartValue = uteModuleHeartGetHeartValue();
            if (compo->type == COMPO_TYPE_PICTUREBOX && HeartValue > UTE_MODULE_HEART_MIN_VALUE && HeartValue < UTE_MODULE_HEART_MAX_VALUE)
            {
                ute_personal_info_t info;
                uteApplicationCommonGetPersonalInfo(&info);
                uint8_t maxHeat = 220 - info.age;
                if (((compo_picturebox_t *)compo)->radix == 6)
                {
                    // 图片数量等于6时，按心率区间输出0-5的数值
                    uint8_t lowZone = maxHeat * 0.5f;     // 50%以下为zone 0
                    uint8_t mediumZone = maxHeat * 0.6f;  // 50%-60%为zone 1
                    uint8_t highZone = maxHeat * 0.7f;    // 60%-70%为zone 2
                    uint8_t higherZone = maxHeat * 0.8f;  // 70%-80%为zone 3
                    uint8_t highestZone = maxHeat * 0.9f; // 80%-90%为zone 4
                    // 90%以上为zone 5
                    if (HeartValue < lowZone)
                    {
                        value = 0;
                    }
                    else if (HeartValue < mediumZone)
                    {
                        value = 1;
                    }
                    else if (HeartValue < highZone)
                    {
                        value = 2;
                    }
                    else if (HeartValue < higherZone)
                    {
                        value = 3;
                    }
                    else if (HeartValue < highestZone)
                    {
                        value = 4;
                    }
                    else
                ...
                {
                    // 图片数量不等于6时，按实际图片数量等分
                    uint8_t radix = ((compo_picturebox_t *)compo)->radix;
                    if (radix > 0)
                    {
                        value = (HeartValue * radix) / maxHeat;
                        if (value >= radix)
                        {
                            value = radix - 1;
                        }
        ...
        case COMPO_BOND_HEARTRATE:
            ...
            {
                uteModuleHeartExternalStartAutoTesting();
            }
            break;
        ...
        case COMPO_BOND_WEATHER:
            value = sys_cb.weather_idx;
            const u16 weather_lang_id[] = {STR_UNKNOWN, STR_SUNNY, STR_SUN_CLOUDY, STR_CLOUDY, STR_MODERATE_RAIN, STR_RAINY_SHOWERS, STR_SLEET, STR_DRIZZLE, STR_HEAVY_RAIN, STR_SNOW_WEAK, STR_SAND_AND_DUST, STR_HAZE, STR_WINDY};
        ...
        case COMPO_BOND_BATTERY:
            value = uteDrvBatteryCommonGetLvl();
            sprintf(value_str, "%d%%", value);
            break;
        ...
        case COMPO_BOND_TIME_SYSTEM:
            value = uteModuleSystemtime12HOn() == true ? 0 : 1; //0:12H 1:24H
            break;
    ...
    switch (rcb->sta)
    {
        ...
        case COMPO_ROLL_STA_STOP:
            ...
            else if (tick_check_expire(rcb->tick, COMPO_TEXT_AUTOROLL_STOP_TICK))
            {
                ...
                if (multiline)
                {
#if UTE_MODULE_TEXT_IS_DRAG_OFFSET_AUTO_BACK
                    if (!rcb->is_drag)
                    {
                        rcb->offset = 0;
                    }
...
int compo_get_button_id(void)
{
    ...
    while (compo != NULL)
    {
        ...
        if (compo->type == COMPO_TYPE_SHAPE)
        {
            compo_shape_t * shape = (compo_button_t *)compo;
            if(((shape->id & 0x8000) >> 15) == 1)
            {
                rect_t rect = widget_get_absolute(shape->rect);
                if ((widget_get_visble(shape->rect)) && abs_s(pt.x - rect.x) * 2 <= rect.wid && abs_s(pt.y - rect.y) * 2 <= rect.hei)
                {
                    id = (shape->id & 0x7fff);
                }
            }
