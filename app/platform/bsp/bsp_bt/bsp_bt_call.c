...
void hfp_hf_call_notice(uint32_t evt)
{
    switch (evt)
    {
        ...
        {
            const char *last_number = hfp_get_last_call_number(0);
            printf("===>>> Number: %s, is 3way:%d\n", last_number, hfp_hf_check_is_3way());
#if !UTE_BT_CALL_THREE_WAY_SUPPORT
            if (hfp_hf_check_is_3way())
            {
                break;
...
#endif
            uteModuleCallSetContactsNumberAndName((uint8_t *)last_number, strlen(last_number), NULL, 0);
            if (strlen(sys_cb.pbap_result_Name) == 0)
            {
                uint8_t nameLen;
                uteModuleCallGetAddressBookContactName((uint8_t *)last_number, strlen(last_number), (uint8_t *)&sys_cb.pbap_result_Name[0], &nameLen);
            ...
            {
                if(uteModuleCallGetBeforeCallStatus() == 2)
                {
                    uteModuleCallSetBeforeCallStatus(0);
                }
            }
            ...
            {
                printf("===>>> InComing, call_type:%d\n", bt_cb.call_type);
                if (bt_cb.call_type != CALL_TYPE_PHONE)
                {
                    if (strlen(last_number) > 0 && strncmp(last_number, "10000000", 8) != 0) // 10000000为网络来电
                    {
