...
u32 destArr[STOPWATCH_REC_NUM_MAX];

#define TO_BEGIN_MODE_PLAY_PIC       UI_BUF_I341001_27_MORE_STOPWATCH_STOPWATCH_START_BIN
#define SART_MODE_PLAY_PIC           UI_BUF_I341001_27_MORE_STOPWATCH_YELLOW_START_BIN
#define PAUSE_MODE_PAUSE_PIC         UI_BUF_I341001_27_MORE_STOPWATCH_YELLOW_STOP_BIN
#define PAUSE_MODE_RESET_PIC         UI_BUF_I341001_27_MORE_STOPWATCH_GRAY_AGIN_BIN
#define SART_MODE_COUNT_PIC          UI_BUF_I341001_27_MORE_STOPWATCH_STOPWATCH_BIN
#define PAUSE_MODE_END_PIC           UI_BUF_I341001_27_MORE_STOPWATCH_GRAY_YELLOW_START_BIN

co_timer_t stopwatch_timer;

static const compo_listbox_item_t tbl_stopwatch_list[STOPWATCH_REC_NUM_MAX+5];

void reverseArrayPtr(u32 *arr, u8 size)
{
    u32 *start = arr;
    u32 *end = arr + size - 1;
    while (start < end)
...
static bool stopwatch_set_text_callback(u32 item_cnt, char* str_txt1, u16 str_txt1_len, char* str_txt2, u16 str_txt2_len, u16 index)
{
    ...
    memcpy(destArr, sys_cb.stopwatch_rec_view, sizeof(destArr));
    reverseArrayPtr(destArr, sys_cb.stopwatch_rec_cnt);

    if (index < item_cnt && index < sys_cb.stopwatch_rec_cnt)
    {
        min = ((destArr[index] / 1000) / 60) % 100;
        sec = (destArr[index] / 1000) % 60;
        msec = destArr[index] % 1000;
//        min = ((sys_cb.stopwatch_rec_view[index] / 1000) / 60) % 100;
//        sec = (sys_cb.stopwatch_rec_view[index] / 1000) % 60;
//        msec = sys_cb.stopwatch_rec_view[index] % 1000;
...
compo_form_t *func_stopwatch_form_create(void)
{
    ...
    compo_listbox_set_text1_color_callback(listbox, stopwatch_text1_callback);
    compo_listbox_set_text2_color_callback(listbox, stopwatch_text2_callback);
    compo_listbox_set_focus_byidx(listbox, 4);//sys_cb.stopwatch_rec_cnt+1);
    compo_listbox_update(listbox);

    compo_shape_t *shape = compo_shape_create_for_page(frm, frm->page_body, COMPO_SHAPE_TYPE_RECTANGLE);
    compo_shape_set_location(shape, GUI_SCREEN_CENTER_X, GUI_SCREEN_CENTER_Y-79, 320, 1);
    compo_shape_set_color(shape, make_color(51,51,51));
    compo_setid(shape, COMPO_ID_LISTBOX_SHAPE);
    compo_shape_set_visible(shape,sys_cb.stopwatch_rec_cnt ? true : false);
    ...
    txt_num = compo_textbox_create(frm, 8);     //当前计时
    compo_setid(txt_num, COMPO_ID_NUM_STOPWATCH_TIME);
    compo_textbox_set_pos(txt_num, GUI_SCREEN_CENTER_X/2, sys_cb.stopwatch_rec_cnt ? (55 + 20) : (148 + 20));
    compo_textbox_set_font(txt_num, UI_BUF_0FONT_FONT_NUM_64_BIN);
    snprintf(str_buff, sizeof(str_buff), "%02d:%02d.%02d", min, sec, msec / 10);
    compo_textbox_set(txt_num, str_buff);
    compo_textbox_set_align_center(txt_num, false);
    ...
    compo_setid(btn, COMPO_ID_BTN_PAUSE_PLAY);
    compo_button_set_pos(btn, (sys_cb.stopwatch_total_msec == 0) ? GUI_SCREEN_CENTER_X : GUI_SCREEN_WIDTH*3/4-5,
                         GUI_SCREEN_HEIGHT - 40 - 24);

    res_addr = sys_cb.stopwatch_sta ? (sys_cb.stopwatch_rec_cnt < STOPWATCH_REC_NUM_MAX ? SART_MODE_COUNT_PIC : UI_BUF_I341001_27_MORE_STOPWATCH_GRAY_STOPWATCH_BIN) : PAUSE_MODE_RESET_PIC;//复位/计次
    btn = compo_button_create_page_by_image(frm,frm->page,res_addr);
    compo_setid(btn, COMPO_ID_BTN_RECORD_REST);
    compo_button_set_pos(btn, GUI_SCREEN_WIDTH/4+5,GUI_SCREEN_HEIGHT - 40 - 24);
    compo_button_set_visible(btn,!(sys_cb.stopwatch_total_msec == 0));
    return frm;
}

//50ms秒表毫秒刷新回调函数（10ms误差大）
void stopwatch_50ms_pro(co_timer_t *timer, void *param)
{
    static u32 real_total_msec;
    if (sys_cb.stopwatch_sta)      //休眠不计时
    {
...
static void func_stopwatch_button_click(u32 key_flag)
{
    ...
    switch(id)
    {
        case COMPO_ID_BTN_RECORD_REST://记录数/重新开始
            if(sys_cb.stopwatch_sta && (sys_cb.stopwatch_total_msec / 1000 / 60 <= 99)) //计次
            {
                if (sys_cb.stopwatch_rec_cnt < STOPWATCH_REC_NUM_MAX)
                {
                    ...
                    compo_listbox_set(listbox, tbl_stopwatch_list, sys_cb.stopwatch_rec_cnt+4);
                    compo_listbox_set_text_modify_by_idx_callback2(listbox, stopwatch_set_text_callback);
                    compo_listbox_set_focus_byidx(listbox, 4);
                    compo_listbox_move_init_modify(listbox, 137+56*2, compo_listbox_gety_byidx(listbox,sys_cb.stopwatch_rec_cnt+1));
                    compo_listbox_update(listbox);
                    uteDrvMotorStart(300, 300, 1);
                }

                if (sys_cb.stopwatch_rec_cnt == STOPWATCH_REC_NUM_MAX)
                {
                    compo_button_set_bgimg(btn_rest, UI_BUF_I341001_27_MORE_STOPWATCH_GRAY_STOPWATCH_BIN);
                }
            }
            else//复位
            {
                ...
                compo_button_set_bgimg(btn_play,TO_BEGIN_MODE_PLAY_PIC);
                compo_button_set_visible(btn_rest,false);
                compo_button_set_pos(btn_play, GUI_SCREEN_WIDTH/2,GUI_SCREEN_HEIGHT - 40 - 24);
        ...
        case COMPO_ID_BTN_PAUSE_PLAY://暂停/开始
            ...
            if(sys_cb.stopwatch_total_msec == 0)
            {
                sys_cb.stopwatch_sta = 1;
                compo_button_set_visible(btn_rest,true);
                compo_button_set_pos(btn_play,  GUI_SCREEN_WIDTH*3/4-5, GUI_SCREEN_HEIGHT - 40 - 24);
            }
            else
            {
                sys_cb.stopwatch_sta ^= 1;
            ...
            u32 res_addr = sys_cb.stopwatch_sta ? PAUSE_MODE_PAUSE_PIC : SART_MODE_PLAY_PIC;
            compo_button_set_bgimg(btn_play,res_addr);
            res_addr = sys_cb.stopwatch_sta ? (sys_cb.stopwatch_rec_cnt < STOPWATCH_REC_NUM_MAX ? SART_MODE_COUNT_PIC : UI_BUF_I341001_27_MORE_STOPWATCH_GRAY_STOPWATCH_BIN) : PAUSE_MODE_RESET_PIC;
            compo_button_set_bgimg(btn_rest,res_addr);
            f_stopwatch->exitFlag = true;
            f_stopwatch->tick = tick_get();
            break;
    ...
    compo_textbox_set_pos(num_time, GUI_SCREEN_CENTER_X/2, sys_cb.stopwatch_rec_cnt ? (55 + 20) : (148 + 20));
}
...
#define TO_BEGIN_MODE_PLAY_PIC       UI_BUF_I343001_27_MORE_STOPWATCH_START_BIN
#define SART_MODE_PLAY_PIC           UI_BUF_I343001_27_MORE_STOPWATCH_YELLOW_START_BIN
#define PAUSE_MODE_PAUSE_PIC         UI_BUF_I343001_27_MORE_STOPWATCH_YELLOW_STOP_BIN
#define PAUSE_MODE_RESET_PIC         UI_BUF_I343001_27_MORE_STOPWATCH_GRAY_AGIN_BIN
#define SART_MODE_COUNT_PIC          UI_BUF_I343001_27_MORE_STOPWATCH_STOPWATCH_BIN
#define PAUSE_MODE_END_PIC           UI_BUF_I343001_27_MORE_STOPWATCH_GRAY_YELLOW_START_BIN
...
compo_form_t *func_stopwatch_form_create(void)
{
    ...
    compo_listbox_t *listbox = compo_listbox_create(frm, COMPO_LISTBOX_STYLE_TITLE_STOPWATCH_RECORD);
    compo_listbox_set_location(listbox,GUI_SCREEN_CENTER_X,GUI_SCREEN_CENTER_Y+20,GUI_SCREEN_WIDTH,138);
    compo_listbox_set(listbox, tbl_stopwatch_list, sys_cb.stopwatch_rec_cnt+2);
    compo_setid(listbox, COMPO_ID_LISTBOX);
    compo_listbox_set_bgimg(listbox, UI_BUF_I343001_27_MORE_GROUP_BIN);
    compo_listbox_set_item_height(listbox, 50);
    compo_listbox_set_text_modify_by_idx_callback2(listbox, stopwatch_set_text_callback);
    compo_listbox_set_text1_color_callback(listbox, stopwatch_text1_callback);
    compo_listbox_set_text2_color_callback(listbox, stopwatch_text2_callback);
    ...
    compo_shape_t *shape = compo_shape_create_for_page(frm, frm->page_body, COMPO_SHAPE_TYPE_RECTANGLE);
    compo_shape_set_location(shape,GUI_SCREEN_CENTER_X,GUI_SCREEN_CENTER_Y+5-61,278,1);
    compo_shape_set_color(shape,make_color(51,51,51));
    compo_setid(shape,COMPO_ID_LISTBOX_SHAPE);
    compo_shape_set_visible(shape,sys_cb.stopwatch_rec_cnt ? true : false);
    ...
    txt_num = compo_textbox_create(frm, 8);     //当前计时
    compo_setid(txt_num, COMPO_ID_NUM_STOPWATCH_TIME);
    compo_textbox_set_pos(txt_num, GUI_SCREEN_CENTER_X/2, sys_cb.stopwatch_rec_cnt ? 65 : 134);
    compo_textbox_set_font(txt_num, UI_BUF_0FONT_FONT_NUM_54_BIN);
    snprintf(str_buff, sizeof(str_buff), "%02d:%02d.%02d", min, sec, msec / 10);
    compo_textbox_set(txt_num, str_buff);
    compo_textbox_set_align_center(txt_num, false);
...
static void func_stopwatch_button_click(u32 key_flag)
{
    ...
    switch(id)
    {
        case COMPO_ID_BTN_RECORD_REST://记录数/重新开始
            if(sys_cb.stopwatch_sta && (sys_cb.stopwatch_total_msec / 1000 / 60 <= 99)) //计次
            {
                if(sys_cb.stopwatch_rec_cnt < STOPWATCH_REC_NUM_MAX)
                {
                    ...
                    if (sys_cb.stopwatch_rec_cnt > 3)
                        compo_listbox_move_init_modify(listbox, 110+56*2, compo_listbox_gety_byidx(listbox,sys_cb.stopwatch_rec_cnt+1));
                    compo_listbox_update(listbox);
                }
            }
            else//复位
            {
                ...
                compo_button_set_bgimg(btn_play,TO_BEGIN_MODE_PLAY_PIC);
                compo_button_set_visible(btn_rest,false);
                compo_button_set_pos(btn_play, GUI_SCREEN_WIDTH/2,GUI_SCREEN_HEIGHT - 40 - 20);
        ...
        case COMPO_ID_BTN_PAUSE_PLAY://暂停/开始
            ...
            if(sys_cb.stopwatch_total_msec == 0)
            {
                sys_cb.stopwatch_sta = 1;
                compo_button_set_visible(btn_rest,true);
                compo_button_set_pos(btn_play,  GUI_SCREEN_WIDTH*3/4-5, GUI_SCREEN_HEIGHT - 40 - 20);
            }
            else
            {
                sys_cb.stopwatch_sta ^= 1;
    ...
    compo_textbox_set_pos(num_time, GUI_SCREEN_CENTER_X/2, sys_cb.stopwatch_rec_cnt ? 65 : 148);
}
...
//            sys_cb.stopwatch_total_msec = 0;
    ...
    if (sys_cb.stopwatch_sta)
    {
        ...
        if (tick_check_expire(f_stopwatch->tick, (60 * 5 - uteModuleGuiCommonGetDisplayOffTime()) * 1000))
        {
            f_stopwatch->exitFlag = false;
        }
    }
