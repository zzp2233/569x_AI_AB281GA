...
#define WEEK_SPACING_X   (34)

#define CARD_W (318)
#define CARD_H (112)
#define CARD_X (26)
#define CARD_Y (94)
#define CARD_SPACE (132)
...
compo_form_t *func_alarm_clock_form_create(void)
{
    ...
    if (ALARM_ENABLE_CNT())
    {
        ...
        for(u8 i=0; i<ALARM_ENABLE_CNT(); i++)
        {
            compo_cardbox_t *cardbox = compo_cardbox_create(frm,0,3,3,318,132);
            compo_cardbox_set_pos(cardbox,GUI_SCREEN_CENTER_X,94+56+(i*132));
            compo_setid(cardbox, COMPO_ID_CARD_0 + i);


            compo_cardbox_icon_set_pos(cardbox,0,PRO_X(286) + 58 / 2,PRO_Y(94) + 34 / 2);
            ...
            compo_cardbox_icon_set_pos(cardbox,1,PRO_X(26) + 58 / 2,PRO_Y(98) + 58 / 2);
            compo_cardbox_icon_set(cardbox, 1,  alarm_label[ute_moduleSystemtimeAlarmGetLabel(i)]);

            memset(str_buff,0,sizeof(str_buff));
            func_alarm_hour_format_t clock_time = func_alarm_convert_to_12hour(ALARM_GET_HOUR(i));
            snprintf(str_buff, sizeof(str_buff), "%02d:%02d", clock_time.hour, ALARM_GET_MIN(i));
            compo_cardbox_text_set_location(cardbox,0,PRO_X(94),PRO_Y(94),180,55);
            //compo_cardbox_text_set_forecolor(cardbox, 0, ALARM_GET_SWITCH(i) ? MAKE_GRAY(255) : MAKE_GRAY(128));
            ...
            memset(str_buff,0,sizeof(str_buff));
            snprintf(str_buff, sizeof(str_buff), "%s", clock_time.am_pm ? (clock_time.am_pm ==1 ? i18n[STR_AM] : i18n[STR_PM]) : " ");
            compo_cardbox_text_set_location(cardbox,1,PRO_X(205),PRO_Y(94),180,35);
            ...
            uteModuleSystemtimeGetAlarmLableData(data,&data_len,i);
            if(data_len && data_len + 46 <= sizeof(str_buff))
            {
                snprintf(str_buff, sizeof(str_buff), "%s:%s",i18n[STR_EVENT_DESCRIPTION],data);
            }
            else if(data_len + 46 > sizeof(str_buff))
            {
                snprintf(str_buff, sizeof(str_buff), "%s:%s...",i18n[STR_EVENT_DESCRIPTION],data);
            }
            else
            ...
            compo_cardbox_text_set_location(cardbox,2,PRO_X(24),PRO_Y(176),GUI_SCREEN_WIDTH,46);
            compo_cardbox_text_set_align_center(cardbox,2, false);
            compo_cardbox_text_set_forecolor(cardbox,2, MAKE_GRAY(128));
            compo_cardbox_text_set(cardbox,2,str_buff);

            if(i < ALARM_ENABLE_CNT() - 1)
            {

                compo_cardbox_icon_set_pos(cardbox,2,PRO_X(24) + 320 / 2,PRO_Y(215));
                compo_cardbox_icon_set(cardbox, 2, UI_BUF_I341001_20_ALARM_CLOCK_LINE_BIN);
            }

            if (ALARM_GET_CYCLE(i) & BIT(7))
            {
                compo_textbox_t *textbox = compo_textbox_create_for_page(frm,cardbox->page,strlen(i18n[STR_ONCE]));
                compo_textbox_set_pos(textbox, PRO_X(96),PRO_Y(133));
                compo_textbox_set_align_center(textbox, false);
                compo_textbox_set(textbox,i18n[STR_ONCE]);
            }
            else if (ALARM_GET_CYCLE(i) == 0x7f)
            ...
            else
            {
                if(uteModuleSystemtimeReadLanguage() == CHINESE_LANGUAGE_ID || uteModuleSystemtimeReadLanguage() == TRADITIONAL_CHINESE_ID)
                {
                    for (u8 j = 0; j < 7; j++)
                    {
                        u8 temp = (j + 1) % 7;
                        compo_picturebox_t *picbox = compo_picturebox_create_for_page(frm,cardbox->page,0);
                        compo_picturebox_set_pos(picbox, PRO_X(120)+(WEEK_SPACING_X*j), PRO_Y(156));
                        compo_picturebox_set(picbox,(ALARM_GET_CYCLE(i) & BIT(temp)) ? clock_on_day[j] : clock_off_day[j]);
                ...
                else
                {
                    for (u8 j = 7; j < 14; j++)
                    {
                        u8 temp = (j + 1) % 7;
                        compo_picturebox_t *picbox = compo_picturebox_create_for_page(frm,cardbox->page,0);
                        compo_picturebox_set_pos(picbox, PRO_X(120)+(WEEK_SPACING_X*(j - 7)), PRO_Y(156));
                        compo_picturebox_set(picbox,(ALARM_GET_CYCLE(i) & BIT(temp)) ? clock_on_day[j] : clock_off_day[j]);
                    }
                }
            }
        ...
        compo_textbox_t* textbox = compo_textbox_create(frm,strlen(i18n[STR_ONLY_SET]));
        compo_textbox_set_location(textbox,GUI_SCREEN_CENTER_X,84 + 40 +(ALARM_ENABLE_CNT()*132),320,46);
        compo_textbox_set(textbox,i18n[STR_ONLY_SET]);
        compo_textbox_set_forecolor(textbox,MAKE_GRAY(128));
        compo_textbox_set_visible(textbox,false);
        compo_setid(textbox,COMPO_ID_TXT_MAX);
    }
    else
    {
        compo_button_t *btn = compo_button_create_by_image(frm, UI_BUF_I341001_20_ALARM_CLOCK_ADD_BIN);
        ...
        compo_picturebox_t *picbox = compo_picturebox_create(frm,UI_BUF_I341001_20_ALARM_CLOCK_NO_ALARM_CLOCK_BIN);
        compo_picturebox_set_pos(picbox, GUI_SCREEN_CENTER_X, 114 + 130 / 2);

        textbox = compo_textbox_create(frm,strlen(i18n[STR_ADD_CLOCK]));
        compo_textbox_set_location(textbox,GUI_SCREEN_CENTER_X,266 + 48 / 2,330,48);
    ...
    for(u8 i=0; i<ALARM_ENABLE_CNT(); i++)
    {
        ...
        if (compo_cardbox_btn_is(cardbox, pt))
        {
                ...
                //if (pt.x > (GUI_SCREEN_WIDTH - gui_image_get_size(UI_BUF_I335001_20_ALARM_CLOCK_2_1_ALARM_CLOCK_ICON_ALARM_40X20_X176_Y65_X176_Y151_01_OPEN_BIN).wid))   //开关
            {
                ALARM_ENABLE(i, !ALARM_GET_SWITCH(i));
                //刷新
                //(cardbox, 0, ALARM_GET_SWITCH(i) ? MAKE_GRAY(255) : MAKE_GRAY(128));
                //compo_cardbox_text_set_forecolor(cardbox, 1, ALARM_GET_SWITCH(i) ? MAKE_GRAY(255) : MAKE_GRAY(128));
                compo_cardbox_icon_set(cardbox, 0,  ALARM_GET_SWITCH(i) ? UI_BUF_I341001_20_ALARM_CLOCK_OPEN_BIN:UI_BUF_I341001_20_ALARM_CLOCK_GRAY_BUTTON_BIN);
...
compo_form_t *func_alarm_clock_form_create(void)
{
    ...
    if (ALARM_ENABLE_CNT())
    {
        for(u8 i=0; i<ALARM_ENABLE_CNT(); i++)
        {
            compo_cardbox_t *cardbox = compo_cardbox_create(frm,1,1,1,300,104);
            compo_cardbox_set_pos(cardbox,GUI_SCREEN_CENTER_X,60+104/2+(i*116));
            compo_setid(cardbox, COMPO_ID_CARD_0 + i);

            compo_cardbox_rect_set_location(cardbox,0,0,0,300,104,10);
            compo_cardbox_rect_set_color(cardbox, 0,make_color(36,36,36));

            compo_cardbox_icon_set_pos(cardbox,0,227-300/2+58/2,18+28/2-104/2);
            compo_cardbox_icon_set(cardbox, 0,  ALARM_GET_SWITCH(i) ? UI_BUF_I343001_20_ALARM_CLOCK_OPEN_BIN:UI_BUF_I343001_20_ALARM_CLOCK_GRAY_BUTTON_BIN);

            memset(str_buff,0,sizeof(str_buff));
            func_alarm_hour_format_t clock_time = func_alarm_convert_to_12hour(ALARM_GET_HOUR(i));
            snprintf(str_buff, sizeof(str_buff), "%02d:%02d %s", clock_time.hour, ALARM_GET_MIN(i),clock_time.am_pm ? (clock_time.am_pm ==1 ? i18n[STR_AM] : i18n[STR_PM]) : " ");
            compo_cardbox_text_set_location(cardbox,0,WEEK_LOCATION_X-20,-40,180,35);
            compo_cardbox_text_set_forecolor(cardbox, 0, ALARM_GET_SWITCH(i) ? MAKE_GRAY(255) : MAKE_GRAY(128));
            //compo_cardbox_text_set_font(cardbox,0, UI_BUF_0FONT_FONT_NUM_38_BIN);
            compo_cardbox_text_set_align_center(cardbox,0, false);
            compo_cardbox_text_set(cardbox,0,str_buff);
        ...
        compo_button_t *btn = compo_button_create_by_image(frm,UI_BUF_I343001_20_ALARM_CLOCK_BUTTON_ADD_BIN);
        compo_button_set_pos(btn, GUI_SCREEN_CENTER_X,84 + 40 +(ALARM_ENABLE_CNT()*116));
        compo_setid(btn,COMPO_ID_BTN_ADD);

    }
    else
    {
        compo_button_t *btn = compo_button_create_by_image(frm, UI_BUF_I343001_20_ALARM_CLOCK_ADD_BIN);
        compo_button_set_pos(btn, GUI_SCREEN_CENTER_X,122+77);
        compo_setid(btn,COMPO_ID_BTN_ADD);
...
static void func_alarm_clock_process(void)
{
    ...
    compo_button_t *btn  = compo_getobj_byid(COMPO_ID_BTN_ADD);
    compo_textbox_t *textbox  = compo_getobj_byid(COMPO_ID_TXT_MAX);
    if(ALARM_ENABLE_CNT() >= 10)
    {
    ...
    if(f_aclock->alarm_count != ALARM_ENABLE_CNT())
    {
        f_aclock->alarm_count = ALARM_ENABLE_CNT();
...
#if  GUI_SCREEN_SIZE_240X284RGB_I330001_SUPPORT || GUI_SCREEN_SIZE_368X448RGB_I345001_SUPPORT
