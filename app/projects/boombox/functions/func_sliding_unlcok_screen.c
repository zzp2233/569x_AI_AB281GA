...
void func_sliding_unlcok_value_change_callback(int32_t change_value)
{
    ...
    compo_picturebox_set_pos(pic, 90 + num, GUI_SCREEN_CENTER_Y);
    compo_shape_t *shape;
    shape = compo_getobj_byid(COMPO_ID_REC_SLIDER);
    compo_shape_set_location(shape, GUI_SCREEN_CENTER_X + num/2, GUI_SCREEN_CENTER_Y, 220 - num,80);
...
compo_form_t *func_sliding_unlcok_screen_form_create(void)
{
    ...
    compo_bar_set_location(slider, GUI_SCREEN_CENTER_X, GUI_SCREEN_CENTER_Y, 220, 80);
    compo_bar_set_rabardius(slider, 40);
    compo_bar_set_color(slider,COLOR_BLACK, COLOR_BLACK);
    compo_bar_set_range(slider, 0, 220);
    compo_bar_set_value(slider, 220);
    ...
    compo_shape_set_location(shape, GUI_SCREEN_CENTER_X, GUI_SCREEN_CENTER_Y, 220,80);
    compo_shape_set_radius(shape, 40);
    compo_shape_set_color(shape, COLOR_GRAY);
    compo_setid(shape, COMPO_ID_REC_SLIDER);

    compo_textbox_t * compo = compo_textbox_create(frm, 15);
    compo_textbox_set_font(compo, UI_BUF_0FONT_FONT_BIN);
    compo_textbox_set_pos((compo_textbox_t *)compo, GUI_SCREEN_CENTER_X, GUI_SCREEN_CENTER_Y);
    compo_textbox_set((compo_textbox_t *)compo, "Slide to Unlock");
    compo_setid(compo, COMPO_ID_TXT);


    //创建图片
    compo_picturebox_t *pic = compo_picturebox_create(frm, UI_BUF_LOCK_LOCK_BIN);
    compo_picturebox_set_pos(pic, 90, GUI_SCREEN_CENTER_Y);
    compo_setid(pic, COMPO_ID_PIC_ICON);

    shape = compo_shape_create(frm, COMPO_SHAPE_TYPE_RECTANGLE);
    compo_shape_set_location(shape, GUI_SCREEN_CENTER_X, GUI_SCREEN_CENTER_Y, 60,40);
    compo_shape_set_color(shape, COLOR_GRAY);
