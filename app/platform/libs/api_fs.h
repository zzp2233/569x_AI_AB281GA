...
void fs_create_time_modi(u16 year,u8 month,u8 day,u8 hour,u8 mins,u8 sec);              //修改文件创建时间
...
uint32_t fs_getdiskfree(void);                                      //获取磁盘剩余容量(以1024Bytes为单位,即1KBytes)
uint32_t fs_getdisksize(void);                                      //获取磁盘容量(以1024Bytes为单位,即1KBytes)
FRESULT fs_delete_sfn_file(const char* path);                       //删除指定文件
FRESULT fs_rename (const char* path_old,const char* path_new,const char* lfn_new);    //重命名文件(暂不支持EXFAT)
