...
#endif
#define HEART_COUNT 288 //144：10分钟自动测量一次; 288 : 5分钟测量一次
#if UTE_MODULE_HEART_SUPPORT
// #include "ute_module_sleep.h"
/*! 心率模块数据zn.zeng, 2021-07-13  */
bool sport_heart_waring = false;
...
void uteModuleHeartInit(void)
{
    ...
    uteModuleHeartReadSyncTimeInfo();
#endif
#if UTE_DRV_HEART_VCXX_SUPPORT
    uteDrvHeartVcxxInit();
#else
    uteModuleHeartReadLastTestInfo();
...
void uteModuleHeartReadConfig(void)
{
    ...
    /*! 心率最大值,用于计算心率区间  */
    readbuff[0] = 220 - DEFAULT_PERSONALINFO_AGE;
    if (uteModuleFilesystemOpenFile(UTE_MODULE_FILESYSTEM_SYSTEMPARM_HEART_MAXVALUE_INFO, &file, FS_O_RDONLY))
    {
        uteModuleFilesystemSeek(file, 0, FS_SEEK_SET);
        uteModuleFilesystemReadData(file, &readbuff[0], 1);
        uteModuleFilesystemCloseFile(file);
    }
    uteModuleHeartData.maxheartValue = readbuff[0];
#endif
}
/**
*@brief        同步心率最大最小跟平均值
...
void uteModuleHeartEverySecond(void)
{
    ...
    if(uteModuleHeartIsAutoTesting())
    {
        ...
        else if (uteModuleHeartGetWorkMode() == HR_WORK_MODE_HR && isNeedAutoTest && !uteModuleHeartData.isAutoTestFlag)
        {
            if (uteModuleHeartData.heartValue > 0 && uteModuleHeartData.heartValue < 0xff)
            {
                isNeedAutoTest = false;
                uteModuleHeartAutoSaveHeartData();
            }
        }
        if(uteModuleSportMoreSportGetStatus() == ALL_SPORT_STATUS_OPEN || uteModuleSportMoreSportGetStatus() == ALL_SPORT_STATUS_CONTINUE)
        {
            uteModuleHeartData.isAutoTestFlag = true;
        }
        ...
        if(uteModuleHeartData.isAutoTestFlag)
        {
            ...
            else if (sport_heart_waring == true)
            {
                uteModuleHeartData.autoTestSecond = 0;
            }
            else
 ...
 * @date         2025-09-26
 */
uint8_t uteModuleHeartSetLastTestInfo(uint8_t heartValue, ute_module_systemtime_time_t time)
{
    uteModuleHeartData.lastTestInfo.lastValue = heartValue;
    uteModuleHeartData.lastTestInfo.lastTestTime.year = time.year;
    uteModuleHeartData.lastTestInfo.lastTestTime.month = time.month;
    uteModuleHeartData.lastTestInfo.lastTestTime.day = time.day;
    uteModuleHeartData.lastTestInfo.lastTestTime.hour = time.hour;
    uteModuleHeartData.lastTestInfo.lastTestTime.min = time.min;
    uteModuleHeartData.lastTestInfo.lastTestTime.sec = time.sec;

    void *file;
    if (uteModuleFilesystemOpenFile(UTE_MODULE_FILESYSTEM_SYSTEMPARM_HEART_LAST_INFO, &file, FS_O_WRONLY | FS_O_CREAT | FS_O_TRUNC))
    {
        uteModuleFilesystemWriteData(file, &uteModuleHeartData.lastTestInfo, sizeof(ute_module_heart_test_last_info_t));
        uteModuleFilesystemCloseFile(file);
    }

    UTE_MODULE_LOG(UTE_LOG_HEART_LVL, "%s,lastValue=%d,year=%d,month=%d,day=%d,hour=%d,min=%d,sec=%d", __func__, heartValue, time.year, time.month, time.day, time.hour, time.min, time.sec);
}
 ...
 * @date         2025-09-26
 */
void uteModuleHeartReadLastTestInfo(void)
{
    uteModuleHeartData.lastTestInfo.lastValue = 0;
    uteModuleHeartData.lastTestInfo.lastTestTime.year = 0;
    uteModuleHeartData.lastTestInfo.lastTestTime.month = 0;
    uteModuleHeartData.lastTestInfo.lastTestTime.day = 0;
    uteModuleHeartData.lastTestInfo.lastTestTime.hour = 0;
    uteModuleHeartData.lastTestInfo.lastTestTime.min = 0;
    uteModuleHeartData.lastTestInfo.lastTestTime.sec = 0;

    void *file;
    if (uteModuleFilesystemOpenFile(UTE_MODULE_FILESYSTEM_SYSTEMPARM_HEART_LAST_INFO, &file, FS_O_RDONLY))
    {
        uteModuleFilesystemSeek(file, 0, FS_SEEK_SET);
        uteModuleFilesystemReadData(file, &uteModuleHeartData.lastTestInfo, sizeof(ute_module_heart_test_last_info_t));
        uteModuleFilesystemCloseFile(file);
    }

    UTE_MODULE_LOG(UTE_LOG_HEART_LVL, "%s,lastValue=%d,year=%d,month=%d,day=%d,hour=%d,min=%d,sec=%d", __func__, uteModuleHeartData.lastTestInfo.lastValue, uteModuleHeartData.lastTestInfo.lastTestTime.year, uteModuleHeartData.lastTestInfo.lastTestTime.month, uteModuleHeartData.lastTestInfo.lastTestTime.day, uteModuleHeartData.lastTestInfo.lastTestTime.hour, uteModuleHeartData.lastTestInfo.lastTestTime.min, uteModuleHeartData.lastTestInfo.lastTestTime.sec);
}

/**
* @brief        获取最后一次心率测量信息
* @details
* @param info   存储最后一次心率测量信息
* @author       Wang.Luo
* @date         2025-09-26
*/
void uteModuleHeartGetLastTestInfo(ute_module_heart_test_last_info_t *info)
{
    memcpy(info, &uteModuleHeartData.lastTestInfo, sizeof(ute_module_heart_test_last_info_t));
}
...
void uteModuleHeartStartSingleTestingMsgHandler(uint32_t param)
{
    ...
    else if (uteModuleHeartData.type == TYPE_HR_SPO2)
    {
        bsp_sensor_hr_init(HR_WORK_MODE_HRSPO2);
    }
    else
    {
...
/**
 * @brief        外部开启测试
 * @details      用于带心率的表盘发起心率测量
 * @return       void*
 * @author       Wang.Luo
 * @date         2025-09-17
 */
void uteModuleHeartExternalStartAutoTesting(void)
{
    if (uteModuleHeartIsAutoTesting() && !uteModuleHeartData.isAutoTestFlag)
    {
        if (
#if UTE_MODULE_BLOODOXYGEN_SUPPORT
            !uteModuleBloodoxygenIsTesting() &&
...
#endif
                {
                    if ((sys_cb.cover_index != REMIND_HEART_WARING) && uteModuleGuiCommonGetCurrentScreenId() != FUNC_UP_WATCH_DIAL)
                    {
                        //uteTaskGuiStartScreen(FUNC_HEART_WARNING, 0, __func__);
...
void uteModuleHeartAutoSaveHeartData(void)
{
    ...
    uint16_t buffSize = HEART_COUNT + 4;
    ...
    uteModuleHeartSetLastTestInfo(uteModuleHeartData.heartValue, time);
}
/**
*@brief        开始发送24小时心率数据
*@details
...
void uteModuleHeartSendHistoryData(void)
{
    ...
    SEND_NEXT_DATA:
        ...
        for(uint16_t i = sendParam->currSendMinIndex; i < HEART_COUNT;)
        {
        ...
        if (sendParam->currSendMinIndex >= HEART_COUNT)
        {
            ...
            {
                sendParam->currSendMinIndex = HEART_COUNT;
            }
        }
...
void uteModuleHeartStartSendHeartAutoTestHistoryData(ute_module_systemtime_time_t time)
{
    ...
    if(uteModuleHeartData.syncTime.year == 0)
    {
        diffday = 0;
    }
    else
    {
        diffday = uteModuleSystemtimeGetDiffDay(uteModuleHeartData.syncTime.year,uteModuleHeartData.syncTime.month,uteModuleHeartData.syncTime.day,curryTime.year,curryTime.month,curryTime.day);
    }
    if((diffday >= 0)&&(diffday <= UTE_MODULE_HEART_SAVE_DATA_MAX_DAYS))
    {
...
*@date       2021-12-22
*/
static bool uteModuleHeartLoadTodayHistoryData(uint8_t *heartHistoryGraph, uint8_t heartHistoryGraphCount, uint8_t *heartHistoryData, int heartHistoryDataLen)
{
...
#else
#if APP_SAVE_HR_OXYGEN_EVERY_FIVE_MINUTES_SUPPORT
    uint8_t tempHeartHistoryData[HEART_COUNT];
    uint8_t m = HEART_COUNT / heartHistoryGraphCount;
#else
    uint8_t tempHeartHistoryData[144];
    uint8_t m = 144 / heartHistoryGraphCount;
#endif
    ...
    for (uint16_t i = 0; i < heartHistoryDataLen && i <= currentTimePoint; i++)
    {
...
bool uteModuleHeartGetTodayHistoryData(uint8_t *heartHistoryGraph, uint8_t heartHistoryGraphCount)
{
    ...
    uint8_t path[40];
    bool result = false;
    uint8_t *heartHistoryData = (uint8_t *)uteModulePlatformMemoryAlloc(HEART_COUNT); // 一天24h的数据，十分钟一条，一共144条
    ute_module_systemtime_time_t time;
    memset(heartHistoryData, 0, HEART_COUNT);
    ...
    if (uteModuleFilesystemOpenFile((char *)&path[0], &file, FS_O_RDONLY))
    {
        ...
        uteModuleFilesystemReadData(file, heartHistoryData, HEART_COUNT);
        uteModuleFilesystemCloseFile(file);
        uteModuleHeartLoadTodayHistoryData(heartHistoryGraph, heartHistoryGraphCount, heartHistoryData, HEART_COUNT);
        result = true;
    }
    else
    {
...
*@date       2022-03-08
*/
bool uteModuleHeartGetSportHeartWaringIsFristHasNotify(void)
{
    return uteModuleHeartData.sportWarning.isFristhHasNotify;
}
/**
*@brief        设置运动心率首次已经提醒
*@details
...
*@date       2022-03-08
*/
void uteModuleHeartSetSportHeartWaringIsFristHasNotify(bool isFristhHasNotify)
{
    uteModuleHeartData.sportWarning.isFristhHasNotify = isFristhHasNotify;
}
#endif
/**
*@brief       系统时间变化
...
*@date       2022-09-29
*/
void uteModuleHeartSetMaxValue(uint8_t maxvalue)
{
    void *file;
    uint8_t writebuff[1];

    uteModuleHeartData.maxheartValue = maxvalue;
    writebuff[0] = uteModuleHeartData.maxheartValue;
    if (uteModuleFilesystemOpenFile(UTE_MODULE_FILESYSTEM_SYSTEMPARM_HEART_MAXVALUE_INFO, &file, FS_O_WRONLY | FS_O_CREAT | FS_O_TRUNC))
    {
        uteModuleFilesystemWriteData(file, &writebuff[0], 1);
        uteModuleFilesystemCloseFile(file);
    }
...
*@date       2022-09-29
*/
uint8_t uteModuleHeartGetSaveMaxHeartValue(void)
{
    return uteModuleHeartData.maxheartValue;
}
#endif
...
*@date        2022-03-08
*/
void uteModuleHeartReadSyncTimeInfo(void)
{
    /*! 读取数据*/
    void *file;
    uint8_t readbuff[7];
    memset(&readbuff[0],0,sizeof(readbuff));
    if(uteModuleFilesystemOpenFile(UTE_MODULE_FILESYSTEM_SYSTEMPARM_HEART_SYNC_TIME_INFO,&file,FS_O_RDONLY))
    {
        uteModuleFilesystemSeek(file,0,FS_SEEK_SET);
        uteModuleFilesystemReadData(file,&readbuff[0],7);
        uteModuleFilesystemCloseFile(file);
    }
    uteModuleHeartData.syncTime.year = (readbuff[0] << 8) | readbuff[1];
    uteModuleHeartData.syncTime.month = readbuff[2];
    uteModuleHeartData.syncTime.day = readbuff[3];
    uteModuleHeartData.syncTime.hour = readbuff[4];
    uteModuleHeartData.syncTime.min = readbuff[5];
    uteModuleHeartData.syncTime.sec = readbuff[6];
}
/**
*@brief  保存心率同步时间
*@details
...
*@date        2022-03-08
*/
void uteModuleHeartSaveSyncTimeInfo(void)
{
    /*! 保存到文件zn.zeng, 2021-08-20*/
    void *file;
    uint8_t writeBuff[7];
    memset(&writeBuff[0],0,sizeof(writeBuff));
    writeBuff[0] = uteModuleHeartData.syncTime.year >> 8 & 0xff;
    writeBuff[1] = uteModuleHeartData.syncTime.year & 0xff;
    writeBuff[2] = uteModuleHeartData.syncTime.month;
    writeBuff[3] = uteModuleHeartData.syncTime.day;
    writeBuff[4] = uteModuleHeartData.syncTime.hour;
    writeBuff[5] = uteModuleHeartData.syncTime.min;
    writeBuff[6] = uteModuleHeartData.syncTime.sec;
    if (uteModuleFilesystemOpenFile(UTE_MODULE_FILESYSTEM_SYSTEMPARM_HEART_SYNC_TIME_INFO, &file, FS_O_RDONLY|FS_O_WRONLY|FS_O_CREAT))
    {
        uteModuleFilesystemSeek(file,0,FS_SEEK_SET);
...
*@date        2022-03-08
*/
void uteModuleHeartSetSyncTime(ute_module_systemtime_measure_time_t setTime)
{
    uteModuleHeartData.syncTime.year = setTime.year;
    uteModuleHeartData.syncTime.month = setTime.month;
    uteModuleHeartData.syncTime.day = setTime.day;
    uteModuleHeartData.syncTime.hour = setTime.hour;
    uteModuleHeartData.syncTime.min = setTime.min;
    uteModuleHeartData.syncTime.sec = setTime.sec;
    uteModuleHeartSaveSyncTimeInfo();
}
/**
*@brief  获取心率同步时间
*@details
...
*@date        2022-10-24
*/
void uteModuleHeartGetSyncTime(ute_module_systemtime_measure_time_t *time)
{
    memcpy(time, &uteModuleHeartData.syncTime, sizeof(ute_module_systemtime_measure_time_t));
}
#endif
