...
static const f_clock_sub_sider_t f_clock_sub_sider[] =
{
    [WEATHER_TYPE_UNKNOWN]={.wether_res=UI_BUF_I343001_12_WEATHER_WEATHER_00_UNKNOWEN_BIN},
    [WEATHER_TYPE_SUNNY]  ={.wether_res=UI_BUF_I343001_12_WEATHER_WEATHER_01_SUNNY_BIN},
    [WEATHER_TYPE_CLOUDY]  ={.wether_res=UI_BUF_I343001_12_WEATHER_WEATHER_02_CLOUDY_BIN},
    [WEATHER_TYPE_OVERCAST]  ={.wether_res=UI_BUF_I343001_12_WEATHER_WEATHER_03_OVERCAST_BIN},
    [WEATHER_TYPE_SHOWER_RAIN]  ={.wether_res=UI_BUF_I343001_12_WEATHER_WEATHER_04_SHOWER_BIN},
    [WEATHER_TYPE_THUNDERSHOWER_RAIN]  ={.wether_res=UI_BUF_I343001_12_WEATHER_WEATHER_05_T_STORM_BIN},
    [WEATHER_TYPE_RAIN_SNOW]  ={.wether_res=UI_BUF_I343001_12_WEATHER_WEATHER_06_SLEET_BIN},
    [WEATHER_TYPE_LIGHT_RAIN]  ={.wether_res=UI_BUF_I343001_12_WEATHER_WEATHER_07_LIGHT_RAIN_BIN},
    [WEATHER_TYPE_HEAVY_RAIN]  ={.wether_res=UI_BUF_I343001_12_WEATHER_WEATHER_08_HEAVY_RAIN_BIN},
    [WEATHER_TYPE_SNOW]  ={.wether_res=UI_BUF_I343001_12_WEATHER_WEATHER_09_SNOW_BIN},
    [WEATHER_TYPE_SAND_STORM]  ={.wether_res=UI_BUF_I343001_12_WEATHER_WEATHER_10_SAND_STORM_BIN},
    [WEATHER_TYPE_HAZE]  ={.wether_res=UI_BUF_I343001_12_WEATHER_WEATHER_11_HAZE_BIN},
    [WEATHER_TYPE_WIND]  ={.wether_res=UI_BUF_I343001_12_WEATHER_WEATHER_12_WINDY_BIN},
    [WEATHER_TYPE_NIGHT]  ={.wether_res=UI_BUF_I343001_12_WEATHER_WEATHER_13_NIGHT_SUNNY_BIN},
    [WEATHER_TYPE_NIGHT+1]  ={.wether_res=UI_BUF_I343001_12_WEATHER_WEATHER_14_NIGHT_CLOUDY_BIN},
    [WEATHER_TYPE_NIGHT+2]  ={.wether_res=UI_BUF_I343001_12_WEATHER_WEATHER_15_NIGHT_RAIN_BIN},
};
...
static const f_clock_sub_sider_t f_clock_sub_sider_app[] =
{
    {.last_app_x=GUI_SIDE_CENTER_X-35, .last_app_y =221+58/2, .last_app_id=COMPO_ID_APP_1 },
    {.last_app_x=GUI_SIDE_CENTER_X+35, .last_app_y =221+58/2, .last_app_id=COMPO_ID_APP_2 },
    {.last_app_x=GUI_SIDE_CENTER_X-35, .last_app_y =287+58/2, .last_app_id=COMPO_ID_APP_3 },
    {.last_app_x=GUI_SIDE_CENTER_X+35, .last_app_y =287+58/2, .last_app_id=COMPO_ID_APP_4 },
};
...
static void func_clock_sub_side_form_create(void)
{
    ...
    compo_textbox_t *textbox = compo_textbox_create(frm, 2);
    compo_textbox_set_font(textbox, UI_BUF_0FONT_FONT_NUM_54_BIN);
    compo_textbox_set_pos(textbox,GUI_SIDE_CENTER_X/2,50);
    compo_bonddata(textbox, COMPO_BOND_HOUR);
    compo_set_bonddata((component_t *)textbox, time_to_tm(compo_cb.rtc_cnt));
    ...
    textbox = compo_textbox_create(frm, 1);
    compo_textbox_set_font(textbox, UI_BUF_0FONT_FONT_NUM_54_BIN);
    compo_textbox_set_pos(textbox,GUI_SIDE_CENTER_X,50);
    compo_textbox_set(textbox, ":");

    // min
    textbox = compo_textbox_create(frm, 2);
    compo_textbox_set_font(textbox, UI_BUF_0FONT_FONT_NUM_54_BIN);
    compo_textbox_set_pos(textbox,GUI_SIDE_CENTER_X+GUI_SIDE_CENTER_X/2,50);
    compo_bonddata(textbox, COMPO_BOND_MINUTE);
    compo_set_bonddata((component_t *)textbox, time_to_tm(compo_cb.rtc_cnt));
    ...
    snprintf(txt_buf,sizeof(txt_buf),"%02d/%02d %s",time.month,time.day,i18n[STR_SUNDAY+time.week]);
    textbox = compo_textbox_create(frm,strlen(txt_buf));
    compo_textbox_set_location(textbox,GUI_SIDE_CENTER_X,105+8,GUI_SCREEN_CENTER_X-10,40);
    compo_textbox_set(textbox,txt_buf );

    compo_picturebox_t *picbox = compo_picturebox_create(frm, UI_BUF_I343001_PRIMARY_FUNCTION_BG1_BIN);
    compo_picturebox_set_pos(picbox,GUI_SIDE_CENTER_X,60/2+131);

    picbox = compo_picturebox_create(frm, UI_BUF_I343001_PRIMARY_FUNCTION_BG2_BIN);
    compo_picturebox_set_pos(picbox,GUI_SIDE_CENTER_X,148/2+209);
    ...
    if(uteModuleWeatherGetCurrDay() == time.day) //当前日期是否与系统日期一致
    {
        ...
        picbox = compo_picturebox_create(frm, f_clock_sub_sider[weather_date.DayWeather[0]].wether_res);
        compo_picturebox_set_pos(picbox,GUI_SIDE_CENTER_X-34,42/2+140);
        compo_picturebox_set_size(picbox,42,42);
    ...
    textbox = compo_textbox_create(frm,strlen(txt_buf));
    compo_textbox_set_location(textbox,GUI_SIDE_CENTER_X+30,40/2+137,70,40);
    compo_textbox_set(textbox,txt_buf );
    compo_setid(textbox, COMPO_ID_WEA_DATA);
    ...
    for(int i =0; i<DEFAULT_LATEST_TASK_NUM; i++)
    {
        //创建按钮
        compo_button_t *btn_app = compo_button_create_by_image(frm, side_get_latest_icon(i));
        compo_button_set_location(btn_app, f_clock_sub_sider_app[i].last_app_x,f_clock_sub_sider_app[i].last_app_y,58,58);
        compo_setid(btn_app,f_clock_sub_sider_app[i].last_app_id);
    }
...
{
#if GUI_SCREEN_SIZE_240X284RGB_I335001_SUPPORT || GUI_SCREEN_SIZE_368X448RGB_I345001_SUPPORT || GUI_SCREEN_SIZE_320X380RGB_I343001_SUPPORT
