...
void compo_kaleidoscope_icon_add(compo_kaleidoscope_t *kale, menu_kale_item_t const *item, int item_cnt)
{
    ...
    // 不再限制item_cnt的最大值，允许少于一圈的数量
    // 计算需要填充的总图标数
    int total_icons_needed = KALE_FUNC_LAYER * KALE_CIR_ICON_NUM;

    kale->dcir[kale->ctl.oidx].lay  = (total_icons_needed - 1) / KALE_CIR_ICON_NUM;
    kale->dcir[kale->ctl.iidx].lay  = KALE_FUNC_LAY_0;

    kale->real_item_cnt = item_cnt;
    kale->floor_cur = item_cnt / 8;
    if (item_cnt % 8)
    {
        kale->floor_cur += 1;
    }
    for (int i = 0; i < total_icons_needed; i++)
    {
        // 使用循环方式获取图标，当图标不足时重复使用前面的图标
        if ((i == item_cnt) && (i % KALE_CIR_ICON_NUM == 0))
        {
            break;
        }
        int src_idx = i % item_cnt;
        u8 idx_cir  = i / KALE_CIR_ICON_NUM;
        u8 idx_icon = i % KALE_CIR_ICON_NUM;

        widget_icon_t *icon = widget_icon_create(kale->page, item[src_idx].res_addr);
        if (i == 0)
        {
        ...
        kale->ofunc[idx_cir].fsub[idx_icon].func_sta = item[src_idx].func_sta;
    ...
    // 如果total_cnt不是KALE_CIR_ICON_NUM的整数倍，需要调整
    //kale->total_cnt = total_icons_needed;
}
...
bool compo_kaleidoscope_ctl_swap_pro(compo_kaleidoscope_t *kale, u8 swap)
{
    ...
    if (swap == KALE_SWAP_LAST)                              //上一级
    {
        ...
        // 使用实际的层数进行循环，而不是基于total_cnt计算
        cycle_sub((u8 *)&lay, KALE_FUNC_LAY_0, kale->floor_cur - 1);
    ...
    else if (kale->ctl.swap == KALE_SWAP_NEXT)             //下一级
    {
        ...
        // 使用实际的层数进行循环，而不是基于total_cnt计算
        cycle_plus((u8 *)&lay, KALE_FUNC_LAY_0, kale->floor_cur - 1);
...
void compo_kaleidoscope_adm_animation_pro(compo_kaleidoscope_t *kale)
{
    ...
    // 使用固定的KALE_FUNC_LAYER而不是基于total_cnt计算
    if (kale->dcir[kale->ctl.iidx].lay >= kale->floor_cur)
    {
        kale->dcir[kale->ctl.iidx].lay = KALE_FUNC_LAY_0;
    }
...
static s16 kale_map_index(s16 idx, s16 item_cnt)
{
    if (item_cnt == 0)
        return 0;
    return idx % item_cnt;
    ...
    for (u8 k = 0; k < KALE_CIR_MAX; k++)
    {
        for (u8 i = 0; i < KALE_CIR_ICON_NUM; i++)
        {
            ...
            if (p->fsub.widget)
            {
                ...
                idx = kale_map_index(kale->dcir[k].lay * KALE_CIR_ICON_NUM + i, kale->real_item_cnt);
                if (select)
                {
...
{
    if (kale == NULL || idx < 0 || idx >= kale->floor_cur * KALE_CIR_ICON_NUM)
    {
        kale->sel_icon = NULL;
        kale->sel_idx = -1;
        return NULL;
    ...
    int real_idx = kale_map_index(idx, kale->real_item_cnt); // 需要保存原始item_cnt
    int idx_cir = real_idx / KALE_CIR_ICON_NUM;
    int idx_icon = real_idx % KALE_CIR_ICON_NUM;

    compo_kale_func_t *fp = &kale->ofunc[idx_cir].fsub[idx_icon];
    kale->sel_icon = fp->widget;
    kale->sel_idx = idx;
