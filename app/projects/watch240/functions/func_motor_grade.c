    ...
    if(f_disturd_set->touch_flag == true)
    {
        ...
        if(f_disturd_set->touch_flag == true)//触摸状态
        {
            ...
            if( dy != 0 && f_disturd_set->num_offset != dy/TXT_SPACING)
            {
                ...
                func_motor_grade_get_timer(&f_disturd_set->mode,f_disturd_set->breathe_mode,set_offset);///获取时间
...
compo_form_t *func_motor_grade_form_create(void)
{
    ...
    compo_button_t * btn_ok = compo_button_create_by_image(frm,UI_BUF_I342001_28_SET_COMFIRM_BIN);///确定按钮
    compo_button_set_pos(btn_ok,GUI_SCREEN_CENTER_X,GUI_SCREEN_HEIGHT-gui_image_get_size(UI_BUF_I342001_28_SET_COMFIRM_BIN).hei/2-10);
    compo_setid(btn_ok,COMPO_ID_BTN_SURE);

    compo_shape_t *shape = compo_shape_create(frm, COMPO_SHAPE_TYPE_RECTANGLE);
    compo_shape_set_location(shape, GUI_SCREEN_CENTER_X, 105-10, 220, 1);
    compo_shape_set_color(shape,make_color(47,47,47));

    shape = compo_shape_create(frm, COMPO_SHAPE_TYPE_RECTANGLE);
    compo_shape_set_location(shape, GUI_SCREEN_CENTER_X, 150-10, 220, 1);
    compo_shape_set_color(shape,make_color(47,47,47));
...
const static uint16_t MOTOR_GRADE_TXT_Y[5]= {CENTER_TXT_Y-TXT_SPACING*2,CENTER_TXT_Y-TXT_SPACING,CENTER_TXT_Y,CENTER_TXT_Y+TXT_SPACING,CENTER_TXT_Y+TXT_SPACING*2}; ///文本Y轴
const static uint16_t txt_mode[3] = {STR_UV_WEAK, STR_NONE, STR_UV_STRONG};
...
static u8 func_motor_grade_get_mode(void)
{
    switch (uteDrvMotorGetVibrationLevel())
    {
        case UTE_DRV_MOTOR_PWM_VIBRATION_LOW:
            return 1;
            break;
        case UTE_DRV_MOTOR_PWM_VIBRATION_NONE:
            return 2;
            break;
        case UTE_DRV_MOTOR_PWM_VIBRATION_HIGH:
            return 0;
...
static u8 func_motor_grade_get_mode_motor(u8 grade)
{
    ...
    {
        case 2:
            return UTE_DRV_MOTOR_PWM_VIBRATION_NONE;
            break;
        case 1:
            return UTE_DRV_MOTOR_PWM_VIBRATION_LOW;
            break;
...
compo_form_t *func_motor_grade_form_create(void)
{
    ...
    //创建一个页面用于限制滚动的时间文本
    widget_page_t* page = widget_page_create(frm->page_body);
    widget_set_location(page, GUI_SCREEN_CENTER_X, 85+226/2, GUI_SCREEN_WIDTH,226);
    for(int idx=COMPO_ID_TXT_1; idx<=COMPO_ID_TXT_5; idx++) ///创建滑动文本
    {
        // printf("idx:%d\n",txt_data[idx-COMPO_ID_TXT_1]);
        compo_textbox_t *txt = compo_textbox_create_for_page(frm,page,50);
    ...
    compo_button_set_pos(btn_ok,GUI_SCREEN_CENTER_X,GUI_SCREEN_HEIGHT-gui_image_get_size(UI_BUF_I341001_28_SET_CONFIRM_BIN).hei/2-10);
    compo_setid(btn_ok,COMPO_ID_BTN_SURE);
    ...
    shape = compo_shape_create(frm, COMPO_SHAPE_TYPE_RECTANGLE);
    compo_shape_set_location(shape, GUI_SCREEN_CENTER_X, 240, 320, 1);
    compo_shape_set_color(shape,make_color(47,47,47));
...
//s8 txt_buf[20];
        ...
        if(f_disturd_set->touch_flag == true)//触摸状态
        {
            ...
            if(f_disturd_set->offset != f_disturd_set->offset_old)
            {
                ...
                uteDrvMotorSetTempVibrationLevel(func_motor_grade_get_mode_motor(f_disturd_set->mode));
                if (func_motor_grade_get_mode_motor(f_disturd_set->mode) != 0)
                    uteDrvMotorStart(UTE_MOTOR_DURATION_TIME, UTE_MOTOR_INTERVAL_TIME, 1);
            }
...
compo_form_t *func_motor_grade_form_create(void)
{
    ...
    //创建一个页面用于限制滚动的时间文本
    widget_page_t* page = widget_page_create(frm->page_body);
    widget_set_location(page, GUI_SCREEN_CENTER_X, 65+214/2, GUI_SCREEN_WIDTH,214);
    for(int idx=COMPO_ID_TXT_1; idx<=COMPO_ID_TXT_5; idx++) ///创建滑动文本
    {
        // printf("idx:%d\n",txt_data[idx-COMPO_ID_TXT_1]);
        compo_textbox_t *txt = compo_textbox_create_for_page(frm,page,50);
    ...
    compo_button_set_pos(btn_ok,GUI_SCREEN_CENTER_X,GUI_SCREEN_HEIGHT-gui_image_get_size(UI_BUF_I343001_COMM_OK_BIN).hei/2-10);
    compo_setid(btn_ok,COMPO_ID_BTN_SURE);

    compo_shape_t *shape = compo_shape_create(frm, COMPO_SHAPE_TYPE_RECTANGLE);
    compo_shape_set_location(shape, GUI_SCREEN_CENTER_X, 133, 277, 1);
    compo_shape_set_color(shape,make_color(47,47,47));

    shape = compo_shape_create(frm, COMPO_SHAPE_TYPE_RECTANGLE);
    compo_shape_set_location(shape, GUI_SCREEN_CENTER_X, 212, 277, 1);
    compo_shape_set_color(shape,make_color(47,47,47));
...
{
#if (GUI_SCREEN_SIZE_240X284RGB_I335001_SUPPORT || GUI_SCREEN_SIZE_360X360RGB_I338001_SUPPORT || GUI_SCREEN_SIZE_240X240RGB_I342001_SUPPORT || GUI_SCREEN_SIZE_368X448RGB_I345001_SUPPORT \
