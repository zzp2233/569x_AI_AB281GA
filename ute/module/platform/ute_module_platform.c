...
void *uteModulePlatformUpdateConnectionParamTimerPointer = NULL;
/**
*@brief      更新连接间隔定时器回调
*@details
*@author     casen
*@date       2021-11-30
*/
void uteModulePlatformUpdateConnectionParamTimerCallback(void * pxTimer)
{
    ute_ble_connect_state_t state;
    uteApplicationCommonGetBleConnectionState(&state);
    if(state.isConnected == true)
    {
        uteModulePlatformSendMsgToAppTask(TO_APP_TASK_MSG_UPDATE_CONNECTION_PARAM_TIMEOUT_TIMER,NULL);
    }
    else
    {
        uteModulePlatformStopTimer(&uteModulePlatformUpdateConnectionParamTimerPointer);
        uteModulePlatformDeleteTimer(&uteModulePlatformUpdateConnectionParamTimerPointer);
        uteModulePlatformUpdateConnectionParamTimerPointer = NULL;
    }
}

void uteModulePlatformUpdateConnectionParamTimeoutMsgHandler(void)
{
#if UTE_MODULE_WATCHONLINE_SUPPORT
    if(uteModuleWatchOnlinegetIsStartReceive())
    {
        uteModulePlatformRestartTimer(&uteModulePlatformUpdateConnectionParamTimerPointer,5000);
        return;
    }
#endif
    ...
    ble_update_conn_param(240, 0, 500);
...
*@details
                    ...
                    see bluetooth4.0, specification,Volume3,part C,Section12.5
                    1) peripheral latency <= 30;
                    2) 2s <= connection Timeout <= 6s;
                    3) interval Min modulo 15ms ==0;
                    4) interval Min + 15ms <=interval Max;
                         interval Min ==interval Max ==15ms;
                    5) interval Max *(peripheral latency +1)<= 2s;
                    6) interval Max *(peripheral latency +1) *3 < connection Timeout;

updateTimeoutMs : 启动定时器的到期时间,用于恢复连接间隔回常规状态
*@author  casen
*@date    2021-11-30
*/
void uteModulePlaformUpdateConnectParam(uint16_t intvmin,uint16_t intvmax,uint16_t updateTimeoutMs)
{
    ute_ble_connect_state_t state;
    uteApplicationCommonGetBleConnectionState(&state);
    if(state.isConnected)
    {
        ...
        {
            ble_update_conn_param(intvmin, 0, 500);
            uteApplicationCommonSetHasUpdateConnectionParam(true);
        }
        if(uteModulePlatformUpdateConnectionParamTimerPointer == NULL)
        {
            uteModulePlatformCreateTimer(&uteModulePlatformUpdateConnectionParamTimerPointer, "UpdateConnectionParamTimer", 1, updateTimeoutMs, false, uteModulePlatformUpdateConnectionParamTimerCallback);
        }
        uteModulePlatformRestartTimer(&uteModulePlatformUpdateConnectionParamTimerPointer,updateTimeoutMs);
    }
}
...
*@date        Jul 05, 2021
*/
void uteModulePlatformBleAdvStart(void)
{
    ble_adv_en();
}
/**
...
*@date        2021-11-06
*/
void uteModulePlatformBleAdvStop(void)
{
    ble_adv_dis();
}
/**
...
*@date        2021-11-06
*/
void uteModulePlatformBleDisconnect(uint8_t conId)
{
    ble_disconnect();
}
...
void uteModulePlatformAdvDataInit(void)
{
    ...
    uteModulePlatformAdvDataModifySub(true, 0x01, (const uint8_t *)"\x06", 1, ADV_APPEND);
    uteModulePlatformAdvDataModifySub(true, 0x03, (const uint8_t *)UTE_ADVERTISE_DATA, strlen(UTE_ADVERTISE_DATA), ADV_APPEND);
    uteModulePlatformAdvDataModifySub(true, 0x03, (const uint8_t *)UTE_APP_DISPLAY_BT_NAME_ADVERTISE_DATA, strlen(UTE_APP_DISPLAY_BT_NAME_ADVERTISE_DATA), ADV_APPEND);
