...
void msa_param_init(void)
{
    ...
    Bright_Structure.accSquareMinLimit = 43909;
    ...
    Bright_Structure.xzAngelLimitMax = 7;
    Bright_Structure.xzAngelLimitMin = -7;
    Bright_Structure.yzAngelLimitMax = 40;
    Bright_Structure.yzAngelLimitMin = 0;
...
AT(.text.lowpwr.sleep)
int peak_Search(short int newData, short int oldData)
{
...
AT(.text.lowpwr.sleep)
void step_Count(void)
{
    Step_Structure.lastStepTime =  Step_Structure.thisStepTime;
    Step_Structure.thisStepTime =  Step_Structure.timeCnt;
    //uint=500ms/data_count, 40*uint 内连续触发，步数有效，否则清除
    if (Step_Structure.thisStepTime - Step_Structure.lastStepTime <= 40)
    {
        if (Step_Structure.step_cnt < 6)
        {
            Step_Structure.step_cnt++;
        }
        else if(Step_Structure.step_cnt == 6)
        {
            Step_Structure.step_cnt++;
...
AT(.text.lowpwr.sleep)
void valueThreshold_Refresh(void)
{
    short int average;
...
AT(.text.lowpwr.sleep)
unsigned char isStatic(int val)
{
...
AT(.text.lowpwr.sleep)
unsigned char light_off_detect(short int x,short int y,short int z)
{
    ...
    //      if (isStatic(Bright_Structure.accSquareSum[0]) && (ABS(x) > 110 || y < -80)) {
    ...
    //  #endif
    //灭角度调节
    if (isStatic(Bright_Structure.accSquareSum[0]) && (ABS(y) > 180 && z < 180))
    {
        Bright_Structure.backlightOffCnt = 0;
...
AT(.text.lowpwr.sleep)
unsigned char isLevel_detect(short int x,short int y,short int z)
{
...
AT(.text.lowpwr.sleep)
unsigned char light_on_detect(short int x,short int y,short int z)
{
#ifdef KCT_GSETURE_WAKEUP
    ...
    // printf("0->[%d %d %d]\n", isStatic(Bright_Structure.accSquareSum[0]), Bright_Structure.accSquareSum[1], !isStatic(Bright_Structure.accSquareSum[1]));
    if (isStatic(Bright_Structure.accSquareSum[0]) && Bright_Structure.accSquareSum[1] > 0 /* && !isStatic(Bright_Structure.accSquareSum[1]) */)
    {
    ...
    // printf("1->[%d %d %d %d]\n", Bright_Structure.autoBrightFlag, y, z, isStatic(Bright_Structure.accSquareSum[0]));
    if (Bright_Structure.autoBrightFlag && y > 0 &&  z > 0 && isStatic(Bright_Structure.accSquareSum[0]))
    {
        Bright_Structure.enableCnt ++;
        // printf("2->[%d %d]\n", Bright_Structure.enableCnt, Bright_Structure.enableCntLimit);
        //亮角度调节
        if (Bright_Structure.enableCnt >= Bright_Structure.enableCntLimit)
        {
            ...
            // printf("3->[%d %d]\n", Bright_Structure.xzAngel, Bright_Structure.yzAngel);
            if (z >= 245)
            {
...
static int sleep_this_status = MSA_AWAKE;
