...
static bool recreate_dial_flag = false; // 重建表盘标志
...
static void func_clock_button_click(void)
{
    ...
    if (btn_id)
    {
        func_switch_to(quick_btn_tbl[btn_id], sys_cb.switch_lr_left_mode_choose | FUNC_SWITCH_AUTO);
    }
    ...
    switch (msg)
    {
        ...
        case MSG_CTP_SHORT_RIGHT:
            ...
            if(UTE_CUI_SCREEN_WATCHDIAL_LEFT != FUNC_NULL)
            {
                ...
                {
                    func_switch_to(UTE_CUI_SCREEN_WATCHDIAL_LEFT, sys_cb.switch_lr_rigth_mode_choose);
                }
        ...
        case MSG_CTP_SHORT_LEFT:
            if(uteModuleGuiCommonGetDisplayCtrlSwitchMode() == SWITCH_MODE_ROTARY)
            {
                uteTaskGuiStartScreen(FUNC_SWITCH_MODE_ROTARY, 0, __func__);
            }
            else
            {
                func_message(msg);
        ...
        case MSG_CTP_DOUBLE_CLICK:
#if UTE_MODULE_SCREENS_TBL_SORT_CUBE_SUPPORT
            uteTaskGuiStartScreen(FUNC_TBL_SORT_CUBE, 0, __func__);
#endif
            break;
