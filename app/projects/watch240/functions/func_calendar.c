...
compo_form_t *func_calender_form_create(void)
{
    ...
    widget_icon_t *img = widget_icon_create(frm->page_body, UI_BUF_I340001_CALENDAR_LEFT_BIN);
    widget_set_pos(img, 60, 85);

    img = widget_icon_create(frm->page_body, UI_BUF_I340001_CALENDAR_RIGHT_BIN);
    widget_set_pos(img, 315, 85);
...
compo_form_t *func_calender_form_create(void)
{
    ...
    //year_text
    cale_label = compo_label_create(frm, 4);
    compo_label_set_font(cale_label, UI_BUF_0FONT_FONT_NUM_28_BIN);
    compo_label_set_pos(cale_label, GUI_SCREEN_CENTER_X - 18,85);
    sprintf(date_str, "%04d", time.year);
    compo_label_set(cale_label, date_str);
    compo_setid(cale_label, COMPO_ID_YEAR_TEXT);
    ...
    widget_icon_t *img = widget_icon_create(frm->page_body, UI_BUF_I338001_CALENDAR_LEFT_BIN);
    widget_set_pos(img, 60, 85);

    img = widget_icon_create(frm->page_body, UI_BUF_I338001_CALENDAR_RIGHT_BIN);
    widget_set_pos(img, 315, 85);
