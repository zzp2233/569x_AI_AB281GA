...
void uteModuleMusicSetPlayerArtist(uint8_t *data, uint16_t size)
{
    ...
    else
    {
        ...
        uteModuleCharencodeGetUtf8String(data, size, &uteModuleMusicData.playerArtist[0], &uteModuleMusicData.playerArtistSize);
    }
...
void uteModuleMusicSetPlayerTitle(uint8_t *data, uint16_t size)
{
    ...
    if (data == NULL || size == 0)
    {
        ...
        uteModuleMusicData.playerTitleSize = 0;
        memset(&uteModuleMusicData.playerArtist[0], 0, UTE_MUSIC_ARTLIST_MAX_SIZE);
        uteModuleMusicData.playerArtistSize = 0;
    }
    else
...
#if 0//BT_ID3_TAG_EN // 防止切换歌曲后歌手信息没更新
...
#endif
        memset(&uteModuleMusicData.playerTitle[0], 0, UTE_MUSIC_TITLE_MAX_SIZE);
        uteModuleCharencodeGetUtf8String(data, size, &uteModuleMusicData.playerTitle[0], &uteModuleMusicData.playerTitleSize);
    }
...
void uteModuleMusicGetPlayerTitle(uint8_t *data,uint16_t *size)
{
    ...
    *size = uteModuleMusicData.playerTitleSize == 1 ? strlen(data) : uteModuleMusicData.playerTitleSize;
    uteModulePlatformGiveMutex(uteModuleMusicMute);
}
...
void uteModuleMusicGetPlayerArtistSize(uint8_t *data,uint16_t *size)
{
    ...
    *size = uteModuleMusicData.playerArtistSize == 1 ? strlen(data) : uteModuleMusicData.playerArtistSize;
    uteModulePlatformGiveMutex(uteModuleMusicMute);
}
