...
#define TXT_SPACING  133
    ...
    if(sport_history_page)
    {
        ...
        u8 index = sportsHistoryRecordData.displayIndex;
        u8 mode = sportsHistoryRecordData.oneHistoryRecordData[index].sportsType;
        sport_type = uteModuleSportGetCurrSportShowDataType(mode);
        icon_addr = tbl_sport_list[mode].res_addr;
        str_id = tbl_sport_list[mode].str_idx;
        memcpy(&sport_data.saveData,&sportsHistoryRecordData.oneHistoryRecordData[index],sizeof(ute_sports_save_data_t));
    }
    else
    {
        sport_type = uteModuleSportGetCurrSportShowDataType(uteModuleSportMoreSportGetType());
        str_id = func_sport_get_str(uteModuleSportTargetGetShowIndex());
        icon_addr = func_sport_get_ui(uteModuleSportTargetGetShowIndex());
    }
    ...
    //设置标题栏
    compo_form_set_title(frm,i18n[STR_SPORT_EXIT_MSG1]);
    compo_form_set_mode(frm, COMPO_FORM_MODE_SHOW_TITLE | COMPO_FORM_MODE_SHOW_TIME);

    //第一页
    //运动类型图片
    picbox = compo_picturebox_create(frm, icon_addr);
    compo_picturebox_set_pos(picbox, 33 + 70 / 2,71 + 70 / 2);

    textbox = compo_textbox_create(frm, strlen(i18n[str_id]));
    compo_textbox_set_align_center(textbox, false);
    compo_textbox_set_location(textbox,20,144,330,40);
    compo_textbox_set(textbox, i18n[str_id]);
    ...
    textbox = compo_textbox_create(frm, strlen(txt_buf));
    compo_textbox_set_align_center(textbox, false);
    compo_textbox_set_pos(textbox, 20, 157 + 27);
    compo_textbox_set(textbox, txt_buf);
...
///////////////////////////////////////////////////////////////////////////////////
    ...
    compo_picturebox_set_pos(picbox, 20 + 66 / 2, 215 + 66 / 2 + 27);

    textbox = compo_textbox_create(frm, strlen(i18n[STR_DURATION]));
    compo_textbox_set_align_center(textbox, false);
    compo_textbox_set_forecolor(textbox, COLOR_GRAY);
    compo_textbox_set_location(textbox,113,207 + 27,220,46);
    compo_textbox_set(textbox, i18n[STR_DURATION]);

    /*运动时长->数据*/
    memset(txt_buf,0,sizeof(txt_buf));
    if(sport_history_page)
    {
        snprintf(txt_buf,sizeof(txt_buf),"%02ld:%02ld:%02ld",sport_data.saveData.totalTime/60/60,sport_data.saveData.totalTime/60%60,sport_data.saveData.totalTime%60);
    }
    else
    {
        snprintf(txt_buf,sizeof(txt_buf),"%02ld:%02ld:%02ld",sport_data.totalSportTime/60/60,sport_data.totalSportTime/60%60,sport_data.totalSportTime%60);
    ...
    compo_textbox_set_align_center(textbox, false);
    compo_textbox_set_pos(textbox,113,246 - 5 + 27);
    compo_textbox_set(textbox, txt_buf);

    compo_shape_t * shape = compo_shape_create(frm, COMPO_SHAPE_TYPE_RECTANGLE);
    compo_shape_set_location(shape,GUI_SCREEN_CENTER_X,316 + 27,320,1);
    compo_shape_set_color(shape, make_color(0x33,0x33,0x33));
    u8 index_space = 0;
    if(sport_type != NO_DATA)
    {
        index_space++;
        ///////////////////////////////////////////////////////////////////////////////////
        picbox = compo_picturebox_create(frm, UI_BUF_I341001_3_EXERCISE_4_COUNTDOWN_ICON_SPORTS_DIS_BIN);
        compo_picturebox_set_pos(picbox, 20 + 66 / 2, 215 + 66 / 2 + 27 + TXT_SPACING * index_space);

        textbox = compo_textbox_create(frm, strlen(i18n[STR_DISTANCE]));
        compo_textbox_set_align_center(textbox, false);
        compo_textbox_set_forecolor(textbox, COLOR_GRAY);
        compo_textbox_set_location(textbox,113,207 + 27 + TXT_SPACING * index_space,220,46);
        compo_textbox_set(textbox, i18n[STR_DISTANCE]);
        ...
        compo_textbox_set_font(textbox, UI_BUF_0FONT_FONT_NUM_48_BIN);
        compo_textbox_set_align_center(textbox, false);
        compo_textbox_set_pos(textbox,113,246 - 5 + 27 + TXT_SPACING * index_space);
        compo_textbox_set(textbox, txt_buf);
        ...
        textbox = compo_textbox_create(frm, strlen(txt_buf));
        compo_textbox_set_align_center(textbox, false);
        compo_textbox_set_location(textbox,113+8+txt_leng.wid,246 + 5 + 27+TXT_SPACING * index_space,88,40);
        compo_textbox_set(textbox, txt_buf);


        shape = compo_shape_create(frm, COMPO_SHAPE_TYPE_RECTANGLE);
        compo_shape_set_location(shape,GUI_SCREEN_CENTER_X,316 + 27 + TXT_SPACING * index_space,320,1);
        compo_shape_set_color(shape, make_color(0x33,0x33,0x33));
    }
    ...
    compo_picturebox_set_location(picbox, 20 + 66 / 2, 215 + 27 + 66 / 2 + TXT_SPACING * index_space,66,66);
    ...
    if((sport_type == MULTIPLE_DATA) || (sport_type == MID_DATA))
    {
        ...
        //步数
        index_space++;
        picbox = compo_picturebox_create(frm, UI_BUF_I341001_3_EXERCISE_4_COUNTDOWN_ICON_SPORTS_STEPS_BIN);
        compo_picturebox_set_pos(picbox, 20 + 66 / 2, 215 + 27 + 66 / 2 + TXT_SPACING * index_space);
        ...
        memset(txt_buf,0,sizeof(txt_buf));
        snprintf(txt_buf,sizeof(txt_buf),"%d",sport_data.saveData.sportStep);
        textbox = compo_textbox_create(frm, strlen(txt_buf));
        compo_textbox_set_font(textbox, UI_BUF_0FONT_FONT_NUM_48_BIN);
        compo_textbox_set_align_center(textbox, false);
        compo_textbox_set_pos(textbox,113,246 - 5 + 27 + TXT_SPACING * index_space);
    ...
    if(sport_type != NO_DATA)
    {
        ...
        /**********************************************平均配速*******************************************************/
        index_space++;
        picbox = compo_picturebox_create(frm, UI_BUF_I341001_3_EXERCISE_4_COUNTDOWN_ICON_SPORTS_PACE_BIN);
        compo_picturebox_set_pos(picbox, 20 + 66 / 2, 215 + 27 + 66 / 2 + TXT_SPACING * index_space);

        textbox = compo_textbox_create(frm, strlen(i18n[STR_AVG_PACE]));///配速文本
        compo_textbox_set_align_center(textbox, false);
        compo_textbox_set_forecolor(textbox, COLOR_GRAY);
        compo_textbox_set_location(textbox,113,207 + 27 + TXT_SPACING * index_space,220,46);
        compo_textbox_set(textbox, i18n[STR_AVG_PACE]);

        textbox = compo_textbox_create(frm, 20);///配速数据文本
        compo_textbox_set_font(textbox, UI_BUF_0FONT_FONT_NUM_48_BIN);
        compo_textbox_set_align_center(textbox, false);
        compo_textbox_set_pos(textbox,113,246 - 5 + 27 + TXT_SPACING * index_space);
        memset(txt_buf,0,sizeof(txt_buf));
        if(sport_data.saveData.avgTimeSecond || sport_data.saveData.avgTimeMinute)
        {
            snprintf(txt_buf,sizeof(txt_buf),"%d%c%d%c",sport_data.saveData.avgTimeMinute,'\'',sport_data.saveData.avgTimeSecond,'"');
        }
        else
        {
        ...
        textbox = compo_textbox_create(frm, strlen(txt_buf));///配速文本
        compo_textbox_set_align_center(textbox, false);
        compo_textbox_set_location(textbox,123 + txt_leng.wid,246 + 5 + 27 + TXT_SPACING * index_space,130,46);
        compo_textbox_set(textbox, txt_buf);
        ...
        /**********************************************最大配速*******************************************************/
        index_space++;
        picbox = compo_picturebox_create(frm, UI_BUF_I341001_3_EXERCISE_4_COUNTDOWN_ICON_SPORTS_PACE_MAX_BIN);
        compo_picturebox_set_pos(picbox, 20 + 66 / 2, 215 + 27 + 66 / 2 + TXT_SPACING * index_space);

        textbox = compo_textbox_create(frm, strlen(i18n[STR_FAST_PACE]));///配速文本
        compo_textbox_set_align_center(textbox, false);
        compo_textbox_set_forecolor(textbox, COLOR_GRAY);
        compo_textbox_set_location(textbox,113,207 + 27 + TXT_SPACING * index_space,220,46);
        compo_textbox_set(textbox, i18n[STR_FAST_PACE]);
        ...
        compo_textbox_set_pos(textbox,113,246 - 5 + 27 + TXT_SPACING * index_space);
        memset(txt_buf,0,sizeof(txt_buf));
        if(sport_data.saveData.maxTimeSecond || sport_data.saveData.maxTimeSecond)
        {
            snprintf(txt_buf,sizeof(txt_buf),"%d%c%d%c",sport_data.saveData.maxTimeMinute,'\'',sport_data.saveData.maxTimeSecond,'"');
        }
        else
        {
            snprintf(txt_buf,sizeof(txt_buf),"%d",0);
        ...
        textbox = compo_textbox_create(frm, strlen(txt_buf));///配速文本
        compo_textbox_set_align_center(textbox, false);
        compo_textbox_set_location(textbox,123 + txt_leng.wid,246 + 5 + 27 + TXT_SPACING * index_space, 130, 40);
        compo_textbox_set(textbox, txt_buf);
        ...
        /**********************************************最小配速*******************************************************/
        index_space++;
        picbox = compo_picturebox_create(frm, UI_BUF_I341001_3_EXERCISE_4_COUNTDOWN_ICON_SPORTS_PACE_MIN_BIN);
        compo_picturebox_set_pos(picbox, 20 + 66 / 2, 215 + 27 + 66 / 2 + TXT_SPACING * index_space);

        textbox = compo_textbox_create(frm, strlen(i18n[STR_SLOW_PACE]));///配速文本
        compo_textbox_set_align_center(textbox, false);
        compo_textbox_set_forecolor(textbox, COLOR_GRAY);
        compo_textbox_set_location(textbox,113,207 + 27 + TXT_SPACING * index_space,220,46);
        compo_textbox_set(textbox, i18n[STR_SLOW_PACE]);
        ...
        compo_textbox_set_pos(textbox,113,246 - 5 + 27 + TXT_SPACING * index_space);
        memset(txt_buf,0,sizeof(txt_buf));
        if(sport_data.saveData.minTimeSecond || sport_data.saveData.minTimeMinute)
        {
            snprintf(txt_buf,sizeof(txt_buf),"%d%c%d%c",sport_data.saveData.minTimeMinute,'\'',sport_data.saveData.minTimeSecond,'"');
        }
        else
        {
            snprintf(txt_buf,sizeof(txt_buf),"%d",0);
        ...
        if((sport_type == MULTIPLE_DATA) || (sport_type == LESS_DATA))
        {
            /**********************************************平均速度*******************************************************/
            index_space++;
            picbox = compo_picturebox_create(frm, UI_BUF_I341001_3_EXERCISE_4_COUNTDOWN_ICON_SPORTS_SPEED_BIN);
            compo_picturebox_set_pos(picbox, 20 + 66 / 2, 215 + 27 + 66 / 2 + TXT_SPACING * index_space);

            textbox = compo_textbox_create(frm, strlen(i18n[STR_AVG_SPEED]));//速度
            compo_textbox_set_location(textbox,113,207 + 27 + TXT_SPACING * index_space,220,46);
            compo_textbox_set_align_center(textbox, false);
            compo_textbox_set_forecolor(textbox, COLOR_GRAY);
            compo_textbox_set(textbox, i18n[STR_AVG_SPEED]);
            ...
            compo_textbox_set_pos(textbox,113,246 - 5 + 27 + TXT_SPACING * index_space);
            memset(txt_buf,0,sizeof(txt_buf));
            snprintf(txt_buf,sizeof(txt_buf),"%d%c%d", sport_data.avgSpeed / 100,'.',sport_data.avgSpeed % 100);
            compo_textbox_set(textbox, txt_buf);
            leng_size = widget_text_get_area(textbox->txt);

            textbox = compo_textbox_create(frm, strlen(i18n[STR_PACE_UINT]));
            compo_textbox_set_align_center(textbox, false);
            compo_textbox_set_pos(textbox,123 + leng_size.wid,246 + 5 + 27 + TXT_SPACING * index_space);
            compo_textbox_set(textbox, i18n[STR_PACE_UINT]);
            ...
            /**********************************************最大速度*******************************************************/
            index_space++;
            picbox = compo_picturebox_create(frm, UI_BUF_I341001_3_EXERCISE_4_COUNTDOWN_ICON_SPORTS_SPEED_MAX_BIN);
            compo_picturebox_set_pos(picbox, 20 + 66 / 2, 215 + 27 + 66 / 2 + TXT_SPACING * index_space);

            textbox = compo_textbox_create(frm, strlen(i18n[STR_MAX_SPEED]));//速度
            compo_textbox_set_location(textbox,113,207 + 27 + TXT_SPACING * index_space,220,46);
            compo_textbox_set_align_center(textbox, false);
            compo_textbox_set_forecolor(textbox, COLOR_GRAY);
            compo_textbox_set(textbox, i18n[STR_MAX_SPEED]);
            ...
            uint16_t max_speed = (sport_data.saveData.maxTimeMinute == 0 && sport_data.saveData.maxTimeSecond == 0) \
                                 ? 0 : (uint16_t)(360000 / (sport_data.saveData.maxTimeMinute * 60 + sport_data.saveData.maxTimeSecond));

            snprintf(txt_buf,sizeof(txt_buf),"%d%c%d", max_speed / 100,'.',max_speed% 100);
            compo_textbox_set(textbox, txt_buf);
            leng_size = widget_text_get_area(textbox->txt);
            ...
            /**********************************************最小速度*******************************************************/
            index_space++;
            picbox = compo_picturebox_create(frm, UI_BUF_I341001_3_EXERCISE_4_COUNTDOWN_ICON_SPORTS_SPEED_MIN_BIN);
            compo_picturebox_set_pos(picbox, 20 + 66 / 2, 215 + 27 + 66 / 2 + TXT_SPACING * index_space);

            textbox = compo_textbox_create(frm, strlen(i18n[STR_MIN_SPEED]));//速度
            compo_textbox_set_location(textbox,113,207 + 27 + TXT_SPACING * index_space,220,46);
            compo_textbox_set_align_center(textbox, false);
            compo_textbox_set_forecolor(textbox, COLOR_GRAY);
            compo_textbox_set(textbox, i18n[STR_MIN_SPEED]);
            ...
            uint16_t min_speed = (sport_data.saveData.minTimeMinute  == 0 && sport_data.saveData.minTimeSecond == 0) \
                                 ? 0 : (uint16_t)(360000 / (sport_data.saveData.minTimeMinute  * 60 + sport_data.saveData.minTimeSecond));


            snprintf(txt_buf,sizeof(txt_buf),"%d%c%d", min_speed / 100,'.',min_speed% 100);
            compo_textbox_set(textbox, txt_buf);
            leng_size = widget_text_get_area(textbox->txt);
        ...
        if((sport_type == MULTIPLE_DATA) || (sport_type == MID_DATA))
        {
            /**********************************************平均步频*******************************************************/
            index_space++;
            picbox = compo_picturebox_create(frm, UI_BUF_I341001_3_EXERCISE_4_COUNTDOWN_ICON_SPORTS_CADENCE_BIN);
            compo_picturebox_set_pos(picbox, 20 + 66 / 2, 215 + 27 + 66 / 2 + TXT_SPACING * index_space);

            textbox = compo_textbox_create(frm, strlen(i18n[STR_AVG_CADENCE]));//速度
            compo_textbox_set_location(textbox,113,207 + 27 + TXT_SPACING * index_space,220,46);
            compo_textbox_set_align_center(textbox, false);
            compo_textbox_set_forecolor(textbox, COLOR_GRAY);
            compo_textbox_set(textbox, i18n[STR_AVG_CADENCE]);
            ...
            compo_textbox_set_pos(textbox,113,246 - 5 + 27 + TXT_SPACING * index_space);
            memset(txt_buf,0,sizeof(txt_buf));
            snprintf(txt_buf,sizeof(txt_buf),"%d", sport_data.saveData.avgStepfrequency);
            compo_textbox_set(textbox, txt_buf);
            leng_size = widget_text_get_area(textbox->txt);

            textbox = compo_textbox_create(frm, strlen(i18n[STR_CADENCE_UINT]));
            compo_textbox_set_align_center(textbox, false);
            compo_textbox_set_pos(textbox,123 + leng_size.wid,246 + 5 + 27 + TXT_SPACING * index_space);
            compo_textbox_set(textbox, i18n[STR_CADENCE_UINT]);
            ...
            /**********************************************最大步频*******************************************************/
            index_space++;
            picbox = compo_picturebox_create(frm, UI_BUF_I341001_3_EXERCISE_4_COUNTDOWN_ICON_SPORTS_CADENCE_MAX_BIN);
            compo_picturebox_set_pos(picbox, 20 + 66 / 2, 215 + 27 + 66 / 2 + TXT_SPACING * index_space);

            textbox = compo_textbox_create(frm, strlen(i18n[STR_MAX_CADENCE]));//速度
            compo_textbox_set_location(textbox,113,207 + 27 + TXT_SPACING * index_space,220,46);
            compo_textbox_set_align_center(textbox, false);
            compo_textbox_set_forecolor(textbox, COLOR_GRAY);
            compo_textbox_set(textbox, i18n[STR_MAX_CADENCE]);
            ...
            compo_textbox_set_pos(textbox,113,246 - 5 + 27 + TXT_SPACING * index_space);
            memset(txt_buf,0,sizeof(txt_buf));
            snprintf(txt_buf,sizeof(txt_buf),"%d", sport_data.saveData.maxStepfrequency);
            compo_textbox_set(textbox, txt_buf);
            leng_size = widget_text_get_area(textbox->txt);
            ...
            /**********************************************最小步频*******************************************************/
            index_space++;
            picbox = compo_picturebox_create(frm, UI_BUF_I341001_3_EXERCISE_4_COUNTDOWN_ICON_SPORTS_CADENCE_MIN_BIN);
            compo_picturebox_set_pos(picbox, 20 + 66 / 2, 215 + 27 + 66 / 2 + TXT_SPACING * index_space);

            textbox = compo_textbox_create(frm, strlen(i18n[STR_MIN_CADENCE]));//速度
            compo_textbox_set_location(textbox,113,207 + 27 + TXT_SPACING * index_space,220,40);
            compo_textbox_set_align_center(textbox, false);
            compo_textbox_set_forecolor(textbox, COLOR_GRAY);
            compo_textbox_set(textbox, i18n[STR_MIN_CADENCE]);
            ...
            compo_textbox_set_pos(textbox,113,246 - 5 + 27 + TXT_SPACING * index_space);
            memset(txt_buf,0,sizeof(txt_buf));
            snprintf(txt_buf,sizeof(txt_buf),"%d", sport_data.saveData.minStepfrequency);
            compo_textbox_set(textbox, txt_buf);
            leng_size = widget_text_get_area(textbox->txt);
    ...
    /**********************************************平均心率*******************************************************/
    index_space++;
    picbox = compo_picturebox_create(frm, UI_BUF_I341001_3_EXERCISE_4_COUNTDOWN_ICON_SPORTS_HEART_BIN);
    compo_picturebox_set_pos(picbox, 20 + 66 / 2, 215 + 27 + 66 / 2 + TXT_SPACING * index_space);

    textbox = compo_textbox_create(frm, strlen(i18n[STR_AVG_HEART_RATE]));//速度
    compo_textbox_set_location(textbox,113,207 + 27 + TXT_SPACING * index_space,220,40);
    compo_textbox_set_align_center(textbox, false);
    compo_textbox_set_forecolor(textbox, COLOR_GRAY);
    compo_textbox_set(textbox, i18n[STR_AVG_HEART_RATE]);
    ...
    textbox = compo_textbox_create(frm, strlen(i18n[STR_PER_MINUTE]));
    compo_textbox_set_align_center(textbox, false);
    compo_textbox_set_pos(textbox,123 + leng_size.wid,246 + 27 + TXT_SPACING * index_space);
    compo_textbox_set(textbox, i18n[STR_PER_MINUTE]);
    ...
    /**********************************************最大心率*******************************************************/
    index_space++;
    picbox = compo_picturebox_create(frm, UI_BUF_I341001_3_EXERCISE_4_COUNTDOWN_ICON_SPORTS_HEART_MAX_BIN);
    compo_picturebox_set_pos(picbox, 20 + 66 / 2, 215 + 27 + 66 / 2 + TXT_SPACING * index_space);

    textbox = compo_textbox_create(frm, strlen(i18n[STR_MAX_HEART]));//速度
    compo_textbox_set_location(textbox,113,207 + 27 + TXT_SPACING * index_space,220,46);
    compo_textbox_set_align_center(textbox, false);
    compo_textbox_set_forecolor(textbox, COLOR_GRAY);
    compo_textbox_set(textbox, i18n[STR_MAX_HEART]);
    ...
    compo_textbox_set_pos(textbox,113,246 - 5 + 27 + TXT_SPACING * index_space);
    memset(txt_buf,0,sizeof(txt_buf));
    snprintf(txt_buf,sizeof(txt_buf),"%d", sport_data.saveData.maxHeartRate);
    compo_textbox_set(textbox, txt_buf);
    leng_size = widget_text_get_area(textbox->txt);
    ...
    /**********************************************最小心率*******************************************************/
    index_space++;
    picbox = compo_picturebox_create(frm, UI_BUF_I341001_3_EXERCISE_4_COUNTDOWN_ICON_SPORTS_HEART_MIN_BIN);
    compo_picturebox_set_pos(picbox, 20 + 66 / 2, 215 + 27 + 66 / 2 + TXT_SPACING * index_space);

    textbox = compo_textbox_create(frm, strlen(i18n[STR_MIN_HEART]));//速度
    compo_textbox_set_location(textbox,113,207 + 27 + TXT_SPACING * index_space,220,46);
    compo_textbox_set_align_center(textbox, false);
    compo_textbox_set_forecolor(textbox, COLOR_GRAY);
    compo_textbox_set(textbox, i18n[STR_MIN_HEART]);
    ...
    compo_textbox_set_pos(textbox,113,246 - 5 + 27 + TXT_SPACING * index_space);
    memset(txt_buf,0,sizeof(txt_buf));
    snprintf(txt_buf,sizeof(txt_buf),"%d", sport_data.saveData.minHeartRate);
    compo_textbox_set(textbox, txt_buf);
    leng_size = widget_text_get_area(textbox->txt);
...
///////////////////////////////////////////////////////////////////////////////////
    index_space++;
    textbox = compo_textbox_create(frm, strlen(i18n[STR_HEART_RATE_RANGE]));
    compo_textbox_set_align_center(textbox, false);
    compo_textbox_set_forecolor(textbox, COLOR_GRAY);
    compo_textbox_set_location(textbox,20,246 + 27 + TXT_SPACING * index_space,320,46);
    compo_textbox_set(textbox, i18n[STR_HEART_RATE_RANGE]);

#define HEASPACE 74


    uint16_t tempPercent[5] = {0};
    uint16_t realPercent[5] = {0};
    uint16_t tempValue = 0;
    uint16_t totalHeartRange = 0;
    totalHeartRange = sport_data.saveData.hrWarmupCnt + sport_data.saveData.hrWeightReductionCnt\
                      + sport_data.saveData.hrEnduranceCnt + sport_data.saveData.hrAnaerobicCnt + sport_data.saveData.hrExtremeCnt;
    if (totalHeartRange > 0)
    {
        tempPercent[0] = sport_data.saveData.hrWarmupCnt * 1000 / totalHeartRange;
        tempPercent[1] = sport_data.saveData.hrWeightReductionCnt * 1000 / totalHeartRange;
        tempPercent[2] = sport_data.saveData.hrEnduranceCnt * 1000 / totalHeartRange;
        tempPercent[3] = sport_data.saveData.hrAnaerobicCnt * 1000 / totalHeartRange;
        tempPercent[4] = sport_data.saveData.hrExtremeCnt * 1000 / totalHeartRange;
        tempValue = tempPercent[0];

        u8 index = 0;
        for (u8 i = 1; i < 5; i ++)
        {
            if (tempPercent[i] > tempValue)
            {
                tempValue = tempPercent[i];
                index = i;
            }
        }
        tempValue = 0;
        for (u8 i = 0; i < 5; i ++)
        {
            if (tempPercent[i] % 10 > 4)
            {
                realPercent[i] = tempPercent[i] / 10 + 1;
            }
            else
            {
                realPercent[i] = tempPercent[i] / 10;
            }
            tempValue += realPercent[i];
        }
        if (tempValue != 100)
        {
            if (tempValue < 100)
                realPercent[index] += (100 - tempValue);
            else
                realPercent[index] -= (tempValue - 100);
        }
        tempPercent[4] = realPercent[4];
        tempPercent[3] = realPercent[3];
        tempPercent[2] = realPercent[2];
        tempPercent[1] = realPercent[1];
        tempPercent[0] = realPercent[0];

    }

    textbox = compo_textbox_create(frm, strlen(i18n[STR_LIMIT]));
    compo_textbox_set_align_center(textbox, false);
    compo_textbox_set_location(textbox,20,246 + 27 + TXT_SPACING * index_space + HEASPACE,220,46);
    compo_textbox_set(textbox, i18n[STR_LIMIT]);

    textbox = compo_textbox_create(frm, 6);
    compo_textbox_set_font(textbox, UI_BUF_0FONT_FONT_NUM_38_BIN);
    compo_textbox_get_right_align(textbox,true);
    compo_textbox_set_pos(textbox,300,260 + 27 + TXT_SPACING * index_space + HEASPACE);
    memset(txt_buf,0,sizeof(txt_buf));
    snprintf(txt_buf,sizeof(txt_buf),"%d%%", tempPercent[4]);
    compo_textbox_set(textbox, txt_buf);


    shape = compo_shape_create(frm,COMPO_SHAPE_TYPE_RECTANGLE);
    ute_compo_shape_set(shape,GUI_SCREEN_CENTER_X,299 + 27 + TXT_SPACING * index_space + HEASPACE,330,14,7,0xff,make_color(0x25,0x25,0x25));

    shape = compo_shape_create(frm,COMPO_SHAPE_TYPE_RECTANGLE);
    ute_compo_shape_set(shape,19 + tempPercent[4] * 330 / 100 / 2,299 + 27 + TXT_SPACING * index_space + HEASPACE,tempPercent[4] * 330 / 100,14,7,0xff,make_color(0xff,0x57,0x1B));

    textbox = compo_textbox_create(frm, strlen(i18n[STR_ANAEROBIC_ENDURANCE]));
    compo_textbox_set_align_center(textbox, false);
    compo_textbox_set_location(textbox,20,246 + 27 + TXT_SPACING * index_space + HEASPACE * 2,220,46);
    compo_textbox_set(textbox, i18n[STR_ANAEROBIC_ENDURANCE]);

    textbox = compo_textbox_create(frm, 6);
    compo_textbox_set_font(textbox, UI_BUF_0FONT_FONT_NUM_38_BIN);
    compo_textbox_get_right_align(textbox,true);
    compo_textbox_set_pos(textbox,300,260 + 27 + TXT_SPACING * index_space + HEASPACE * 2);
    memset(txt_buf,0,sizeof(txt_buf));
    snprintf(txt_buf,sizeof(txt_buf),"%d%%", tempPercent[3]);
    compo_textbox_set(textbox, txt_buf);

    shape = compo_shape_create(frm,COMPO_SHAPE_TYPE_RECTANGLE);
    ute_compo_shape_set(shape,GUI_SCREEN_CENTER_X,299 + 27 + TXT_SPACING * index_space + HEASPACE * 2,330,14,7,0xff,make_color(0x25,0x25,0x25));

    shape = compo_shape_create(frm,COMPO_SHAPE_TYPE_RECTANGLE);
    ute_compo_shape_set(shape,19 + tempPercent[3] * 330 / 100 / 2,299 + 27 + TXT_SPACING * index_space + HEASPACE * 2,tempPercent[3] * 330 / 100,14,7,0xff,make_color(0xff,0x85,0x00));

    textbox = compo_textbox_create(frm, strlen(i18n[STR_AEROBIC_ENDURANCE]));
    compo_textbox_set_align_center(textbox, false);
    compo_textbox_set_location(textbox,20,246 + 27 + TXT_SPACING * index_space + HEASPACE * 3,220,46);
    compo_textbox_set(textbox, i18n[STR_AEROBIC_ENDURANCE]);

    textbox = compo_textbox_create(frm, 6);
    compo_textbox_set_font(textbox, UI_BUF_0FONT_FONT_NUM_38_BIN);
    compo_textbox_get_right_align(textbox,true);
    compo_textbox_set_pos(textbox,300,260 + 27 + TXT_SPACING * index_space + HEASPACE * 3);
    memset(txt_buf,0,sizeof(txt_buf));
    snprintf(txt_buf,sizeof(txt_buf),"%d%%", tempPercent[2]);
    compo_textbox_set(textbox, txt_buf);

    shape = compo_shape_create(frm,COMPO_SHAPE_TYPE_RECTANGLE);
    ute_compo_shape_set(shape,GUI_SCREEN_CENTER_X,299 + 27 + TXT_SPACING * index_space + HEASPACE * 3,330,14,7,0xff,make_color(0x25,0x25,0x25));

    shape = compo_shape_create(frm,COMPO_SHAPE_TYPE_RECTANGLE);
    ute_compo_shape_set(shape,19 + tempPercent[2] * 330 / 100 / 2,299 + 27 + TXT_SPACING * index_space + HEASPACE * 3,tempPercent[2] * 330 / 100,14,7,0xff,make_color(0xff,0xe4,0x00));

    textbox = compo_textbox_create(frm, strlen(i18n[STR_FAT_BURNING]));
    compo_textbox_set_align_center(textbox, false);
    compo_textbox_set_location(textbox,20,246 + 27 + TXT_SPACING * index_space + HEASPACE * 4,220,46);
    compo_textbox_set(textbox, i18n[STR_FAT_BURNING]);

    textbox = compo_textbox_create(frm, 6);
    compo_textbox_set_font(textbox, UI_BUF_0FONT_FONT_NUM_38_BIN);
    compo_textbox_get_right_align(textbox,true);
    compo_textbox_set_pos(textbox,300,260 + 27 + TXT_SPACING * index_space + HEASPACE * 4);
    memset(txt_buf,0,sizeof(txt_buf));
    snprintf(txt_buf,sizeof(txt_buf),"%d%%", tempPercent[1]);
    compo_textbox_set(textbox, txt_buf);

    shape = compo_shape_create(frm,COMPO_SHAPE_TYPE_RECTANGLE);
    ute_compo_shape_set(shape,GUI_SCREEN_CENTER_X,299 + 27 + TXT_SPACING * index_space + HEASPACE * 4,330,14,7,0xff,make_color(0x25,0x25,0x25));

    shape = compo_shape_create(frm,COMPO_SHAPE_TYPE_RECTANGLE);
    ute_compo_shape_set(shape,19 + tempPercent[1] * 330 / 100 / 2,299 + 27 + TXT_SPACING * index_space + HEASPACE * 4,tempPercent[1] * 330 / 100,14,7,0xff,make_color(0x00,0xf3,0xcb));

    textbox = compo_textbox_create(frm, strlen(i18n[STR_WARN_UP]));
    compo_textbox_set_align_center(textbox, false);
    compo_textbox_set_location(textbox,20,246 + 27 + TXT_SPACING * index_space + HEASPACE * 5,220,46);
    compo_textbox_set(textbox, i18n[STR_WARN_UP]);

    textbox = compo_textbox_create(frm, 6);
    compo_textbox_set_font(textbox, UI_BUF_0FONT_FONT_NUM_38_BIN);
    compo_textbox_get_right_align(textbox,true);
    compo_textbox_set_pos(textbox,300,260 + 27 + TXT_SPACING * index_space + HEASPACE * 5);
    memset(txt_buf,0,sizeof(txt_buf));
    snprintf(txt_buf,sizeof(txt_buf),"%d%%", tempPercent[0]);
    compo_textbox_set(textbox, txt_buf);

    shape = compo_shape_create(frm,COMPO_SHAPE_TYPE_RECTANGLE);
    ute_compo_shape_set(shape,GUI_SCREEN_CENTER_X,299 + 27 + TXT_SPACING * index_space + HEASPACE * 5,330,14,7,0xff,make_color(0x25,0x25,0x25));

    shape = compo_shape_create(frm,COMPO_SHAPE_TYPE_RECTANGLE);
    ute_compo_shape_set(shape,19 + tempPercent[0] * 330 / 100 / 2,299 + 27 + TXT_SPACING * index_space + HEASPACE * 5,tempPercent[0] * 330 / 100,14,7,0xff,make_color(0x00,0xb4,0xff));
