...
compo_form_t *func_test_mode_form_create(void)
{
    ...
    textbox = compo_textbox_create(frm, strlen("Mode selection"));
    compo_textbox_set(textbox, "Mode selection");
    compo_textbox_set_pos(textbox,GUI_SCREEN_CENTER_X, GUI_SCREEN_CENTER_Y/6+SPACING_Y*(1-1));

    textbox = compo_textbox_create(frm, strlen("Factory test"));
    compo_textbox_set(textbox, "Factory test");
    ...
    textbox = compo_textbox_create(frm, strlen("Aging test"));
...
static void func_test_mode_click(void)
{
    ...
    switch(id)
    {
        case MODE_FACTORY_TESTING_ID:///工厂测试
            ret = msgbox("Factory test", NULL, NULL, MSGBOX_MODE_BTN_OKCANCEL, MSGBOX_MSG_TYPE_NONE);
        ...
        case MODE_AGING_TESTING_ID:///老化测试
            ret = msgbox("Aging test", NULL, NULL, MSGBOX_MODE_BTN_OKCANCEL, MSGBOX_MSG_TYPE_NONE);
...
compo_form_t *func_test_mode_form_create(void)
{
    ...
    textbox = compo_textbox_create(frm, strlen("船运模式"));
    compo_textbox_set(textbox, "船运模式");
    compo_textbox_set_pos(textbox,GUI_SCREEN_CENTER_X, GUI_SCREEN_CENTER_Y/6+SPACING_Y*(4-1));
    compo_textbox_set_forecolor(textbox,COLOR_GREEN);

    btn = compo_button_create(frm);///透明按钮
    compo_button_set_location(btn, GUI_SCREEN_CENTER_X, GUI_SCREEN_CENTER_Y/6+SPACING_Y*(4-1), widget_text_get_area(textbox->txt).wid, widget_text_get_area(textbox->txt).hei*2);
    compo_setid(btn,MODE_SHIPPING_ID);
#endif
    return frm;
}
...
#if UTE_MODULE_SHIP_MODE_SUPPORT
        case MODE_SHIPPING_ID: /// 船运测试
            ret = msgbox("是否进入船运模式", NULL, NULL, MSGBOX_MODE_BTN_OKCANCEL, MSGBOX_MSG_TYPE_NONE);
            if (ret == MSGBOX_RES_OK)
            {
