...
#endif
#if UTE_MODULE_VCXX_HEART_BIO_CHECK_SUPPORT
compo_form_t *func_factory_testing_heart_bio(void);
#endif
...
#endif
#if UTE_MODULE_VCXX_HEART_BIO_CHECK_SUPPORT
    else if (test_data->moduleType == FACTORY_MODULE_HEART_CHECK_BIO)
    {
        frm = func_factory_testing_heart_bio();
    }
#endif
#if UTE_MODULE_BLOODOXYGEN_SUPPORT && !UTE_MODULE_BLOODOXYGEN_RANDOM_SUPPORT
    else if (test_data->moduleType == FACTORY_MODULE_SPO2)
...
static void func_factory_testing_pass_fail_bnt_click(void)
{
    ...
    if(id == PASS_ID || id == FALL_ID)
    {
        ...
        {
#if UTE_MODULE_VCXX_HEART_BIO_CHECK_SUPPORT
            if (test_data->moduleType == FACTORY_MODULE_HEART_CHECK_BIO)
            {
                uint16_t bio_max = DRV_HEART_VCXX_CFG_READ_ERR_WEAR_BIO_DEFAULT * DRV_HEART_VCXX_CFG_READ_ERR_WEAR_BIO_DEFAULT_MAX;
                uint16_t bio_min = DRV_HEART_VCXX_CFG_READ_ERR_WEAR_BIO_DEFAULT * DRV_HEART_VCXX_CFG_READ_ERR_WEAR_BIO_DEFAULT_MIN;
                uint16_t bioA, bioB, diff;
                uteModuleNewFactoryTestGetCheckLightBioValue(&bioA, &bioB, &diff);
                if (bioA == 0 || bioB == 0 || diff > bio_max || diff < bio_min || uteModuleNewFactoryTestGetCheckLightMode() == NEW_FACTORY_VCXX_TEST_MODE_BIO_A)
                {
                    return;
                }
                else
                {
                    ...
                    uteModulePlatformFlashNorRead(&snData, UTE_USER_PARAM_ADDRESS, sizeof(ute_application_sn_data_t));
                    snData.bioDiff = diff;
...
compo_form_t *func_factory_testing_heart_bio(void)
{
    uteModuleNewFactoryTestSetCheckLightMode(NEW_FACTORY_VCXX_TEST_MODE_BIO_A);
    // 新建窗体
    compo_form_t *frm = compo_form_create(true);
    char txt_buf[50];
    compo_textbox_t *textbox = NULL;
    ...
    memset(txt_buf, '\0', sizeof(txt_buf));
    snprintf((char *)txt_buf, sizeof(txt_buf), "活体标定:对空");
    textbox = compo_textbox_create(frm, strlen(txt_buf));
    compo_textbox_set(textbox, txt_buf);
    compo_textbox_set_pos(textbox, GUI_SCREEN_CENTER_X, GUI_SCREEN_HEIGHT * 1 / 7);
    compo_setid(textbox, LEAKAGE_TXT_1_ID);
    ...
    memset(txt_buf, '\0', sizeof(txt_buf));
    snprintf((char *)txt_buf, sizeof(txt_buf), "%d>b0-b1<%d", bio_max, bio_min);
    textbox = compo_textbox_create(frm, strlen(txt_buf));
    compo_textbox_set(textbox, txt_buf);
    compo_textbox_set_pos(textbox, GUI_SCREEN_CENTER_X, GUI_SCREEN_HEIGHT * 2 / 7);
    compo_setid(textbox, LEAKAGE_TXT_2_ID);
    ...
    memset(txt_buf, '\0', sizeof(txt_buf));
    snprintf((char *)txt_buf, sizeof(txt_buf), "Bio0:00000");
    textbox = compo_textbox_create(frm, strlen(txt_buf));
    compo_textbox_set(textbox, txt_buf);
    compo_textbox_set_pos(textbox, GUI_SCREEN_CENTER_X, GUI_SCREEN_HEIGHT * 4 / 7);
    compo_setid(textbox, LEAKAGE_TXT_3_ID);
    ...
    memset(txt_buf, '\0', sizeof(txt_buf));
    snprintf((char *)txt_buf, sizeof(txt_buf), "Bio1:00000");
    textbox = compo_textbox_create(frm, strlen(txt_buf));
    compo_textbox_set(textbox, txt_buf);
    compo_textbox_set_pos(textbox, GUI_SCREEN_CENTER_X, GUI_SCREEN_HEIGHT * 5 / 7);
    compo_setid(textbox, LEAKAGE_TXT_4_ID);
    ...
    compo_button_t *btn = compo_button_create(frm); // 按钮FALL
    compo_button_set_location(btn, GUI_SCREEN_CENTER_X, GUI_SCREEN_HEIGHT * 3 / 7, GUI_SCREEN_WIDTH, txt_data.hei * 2);
    compo_setid(btn, BTN_SWITCH_ID);
...
compo_form_t * func_factory_testing_heart(void)
{
    ...
    ute_application_sn_data_t snData;
    uteModulePlatformFlashNorRead(&snData, UTE_USER_PARAM_ADDRESS, sizeof(ute_application_sn_data_t));
    snprintf((char *)txt_buf, sizeof(txt_buf), "dif:-----,Max:-----,Min:-----");
    textbox = compo_textbox_create(frm, strlen(txt_buf)); /// dif___, Max___
    uint16_t bio_max = DRV_HEART_VCXX_CFG_READ_ERR_WEAR_BIO_DEFAULT * DRV_HEART_VCXX_CFG_READ_ERR_WEAR_BIO_DEFAULT_MAX;
    uint16_t bio_min = DRV_HEART_VCXX_CFG_READ_ERR_WEAR_BIO_DEFAULT * DRV_HEART_VCXX_CFG_READ_ERR_WEAR_BIO_DEFAULT_MIN;
    snprintf((char *)txt_buf, sizeof(txt_buf), "dif:%d,Max:%d,Min:%d", snData.bioDiff, bio_max, bio_min);
    compo_textbox_set_location(textbox, GUI_SCREEN_CENTER_X, GUI_SCREEN_CENTER_Y / 2, GUI_SCREEN_WIDTH - 40 * 2, GUI_SCREEN_HEIGHT / 5);
    compo_textbox_set(textbox, txt_buf);
    compo_setid(textbox, HEART_TXT_1_ID);
#endif
...
//    compo_button_t *btn;
    ...
    compo_textbox_set_forecolor(textbox, COLOR_RED);
    compo_setid(textbox,(UTE_DRV_QDEC_Y_PHB_GPIO_PIN==IO_PB1) ? QDEC_BACKWARD_TXT_ID : QDEC_FORWARD_TXT_ID);
...
#if UTE_MODULE_VCXX_HEART_BIO_CHECK_SUPPORT
static void func_factory_testing_heart_bio_click(void)
{
    int id = compo_get_button_id();
    f_factory_testing_t *f_factory_testing = (f_factory_testing_t *)func_cb.f_cb;
    switch(id)
    {
        ...
        case BTN_SWITCH_ID:
        {
            if (uteModuleNewFactoryTestGetCheckLightMode() == NEW_FACTORY_VCXX_TEST_MODE_BIO_A)
            {
                uteModuleNewFactoryTestSetCheckLightMode(NEW_FACTORY_VCXX_TEST_MODE_BIO_B);
            }
        }
        break;
    }
...
static void func_factory_testing_message(size_msg_t msg)
{
    ...
    switch (msg)
    {
        case MSG_CTP_CLICK:
        {
            ...
            {
                func_factory_testing_heart_bio_click();
            }
#endif
            func_factory_testing_pass_fail_bnt_click();
        }
        ...
        break;
        case MSG_SYS_500MS:
            f_factory_testing->refresh_500ms_flag = true;
            break;
...
#if UTE_MODULE_VCXX_HEART_BIO_CHECK_SUPPORT
static void func_factory_testing_heart_bio_process(void)
{
    compo_textbox_t *TextTitle = compo_getobj_byid(LEAKAGE_TXT_1_ID);
    compo_textbox_t *TextBio0 = compo_getobj_byid(LEAKAGE_TXT_3_ID);
    compo_textbox_t *TextBio1 = compo_getobj_byid(LEAKAGE_TXT_4_ID);
    compo_textbox_t *TextPass = compo_getobj_byid(LEAKAGE_TXT_5_ID);
    ...
    memset(txt_buf, '\0', sizeof(txt_buf));
    snprintf((char *)txt_buf, sizeof(txt_buf), "Bio0:%d",bioA);
    compo_textbox_set(TextBio0, txt_buf);

    memset(txt_buf, '\0', sizeof(txt_buf));
    snprintf((char *)txt_buf, sizeof(txt_buf), "Bio1:%d",bioB);
    compo_textbox_set(TextBio1, txt_buf);

    if (uteModuleNewFactoryTestGetCheckLightMode() == NEW_FACTORY_VCXX_TEST_MODE_BIO_B)
    {
        memset(txt_buf, '\0', sizeof(txt_buf));
        snprintf((char *)txt_buf, sizeof(txt_buf), "%s", "活体标定:活体");
        compo_textbox_set(TextTitle, txt_buf);

        uint16_t bio_max = DRV_HEART_VCXX_CFG_READ_ERR_WEAR_BIO_DEFAULT * DRV_HEART_VCXX_CFG_READ_ERR_WEAR_BIO_DEFAULT_MAX;
        uint16_t bio_min = DRV_HEART_VCXX_CFG_READ_ERR_WEAR_BIO_DEFAULT * DRV_HEART_VCXX_CFG_READ_ERR_WEAR_BIO_DEFAULT_MIN;

        if(diff > bio_max || diff < bio_min)
        {
            memset(txt_buf, '\0', sizeof(txt_buf));
            snprintf((char *)txt_buf, sizeof(txt_buf), "%s","FAIL");
            compo_textbox_set(TextPass, txt_buf);
            compo_textbox_set_forecolor(TextPass, COLOR_RED);
        }
        else
        {
            memset(txt_buf, '\0', sizeof(txt_buf));
            snprintf((char *)txt_buf, sizeof(txt_buf), "%s","PASS");
            compo_textbox_set(TextPass, txt_buf);
            compo_textbox_set_forecolor(TextPass, COLOR_GREEN);
        }
    }
    else
    {
        memset(txt_buf, '\0', sizeof(txt_buf));
        snprintf((char *)txt_buf, sizeof(txt_buf), "%s", "活体标定:对空");
        compo_textbox_set(TextTitle, txt_buf);

        memset(txt_buf, '\0', sizeof(txt_buf));
        snprintf((char *)txt_buf, sizeof(txt_buf), "%s","点击采样");
        compo_textbox_set(TextPass, txt_buf);
        compo_textbox_set_forecolor(TextPass, COLOR_GREEN);
    }
}
...
{
    f_factory_testing_t *f_factory_testing = (f_factory_testing_t *)func_cb.f_cb;
    if (f_factory_testing->refresh_500ms_flag)
    {
        f_factory_testing->refresh_500ms_flag = false;
        char txt_buf[50];
        uint32_t totalStepCnt = 0;
        uteModuleSportGetCurrDayStepCnt(&totalStepCnt, NULL, NULL); /// 获取步数
        ute_drv_gsensor_common_axis_data_t *data = NULL;
        ...
        if (x && y && z)
        {
            ...
            snprintf(txt_buf, sizeof(txt_buf), "X:%d", *axisBitChange.outputXaxis);
            compo_textbox_set(textbox2, txt_buf);
            snprintf(txt_buf, sizeof(txt_buf), "Y:%d", *axisBitChange.outputYaxis);
            compo_textbox_set(textbox3, txt_buf);
            snprintf(txt_buf, sizeof(txt_buf), "Z:%d", *axisBitChange.outputZaxis);
            compo_textbox_set(textbox4, txt_buf);
        }
...
#endif
#if UTE_MODULE_VCXX_HEART_BIO_CHECK_SUPPORT
        case FACTORY_MODULE_HEART_CHECK_BIO:
            func_factory_testing_heart_bio_process();
            break;
#endif
