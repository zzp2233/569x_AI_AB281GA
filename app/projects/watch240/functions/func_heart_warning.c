...
{
    //compo_textbox_t *textbox = compo_getobj_byid(COMPO_ID_TEXT_HEART_VALUE);
    //compo_textbox_t *text_uint = compo_getobj_byid(COMPO_ID_TEXT_UINT);
    //uint8_t heart_value = uteModuleHeartGetHeartValue();
    f_heart_warning_t* f_heart_warning = (f_heart_warning_t*)func_cb.f_cb;
    f_heart_warning->time++;
    //if(heart_value > 0 && heart_value < 0xff)
    //{
    //    char txt_buf[10];
    //    memset(txt_buf, 0, sizeof(txt_buf));
    ...
    //    compo_textbox_set(textbox, txt_buf);
    //    area_t leng_size = widget_text_get_area(textbox->txt);
    //    compo_textbox_set_location(text_uint,123 +leng_size.wid,238 + 76 / 2, 130, 40);
    //}
    if (f_heart_warning->time > 10)
    {
        f_heart_warning->time = 0;
        uteModuleGuiCommonGoBackLastScreen();
    }
}
