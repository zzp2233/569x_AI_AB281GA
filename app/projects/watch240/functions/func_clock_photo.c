...
compo_form_t *func_clock_photo_form_create(void)
{
    ...
    // 设置日期文字
    memset(timeStr, 0, timeStr_leng);
    snprintf(timeStr, timeStr_leng, "%02d/%02d %s", rtc_tm.mon, rtc_tm.day, i18n[week_char_id[rtc_tm.weekday]]);
    compo_textbox_t *txt_week = compo_textbox_create(frm, strlen(timeStr));
    compo_textbox_set(txt_week, timeStr);
    compo_textbox_set_forecolor(txt_week, color);
    compo_setid(txt_week, COMPO_ID_DATE_TEXT);

    if (config.timePosition == 0)
    {
        compo_textbox_set_right_align(textbox, true);
        compo_textbox_set_pos(textbox, 250, 60);

        compo_textbox_set_right_align(txt_week, true);
        compo_textbox_set_pos(txt_week, 250, 140);
    }
    else
    {
        compo_textbox_set_align_center(textbox, false);
        compo_textbox_set_pos(textbox, 32, 290);

        compo_textbox_set_align_center(txt_week, false);
        compo_textbox_set_pos(txt_week, 32, 390);
    }
