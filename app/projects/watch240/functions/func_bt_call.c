...
#define TXT_X_MIN 20
#define SPACING_COUNT (-(GUI_SCREEN_HEIGHT/12))
...
//        }
    ...
    if (sys_cb.hfp_vol > 15)
        compo_picturebox_cut(volume_pic, 14, 15);
    else if (sys_cb.hfp_vol < 1)
        compo_picturebox_cut(volume_pic, 0, 15);
    else
        compo_picturebox_cut(volume_pic, sys_cb.hfp_vol-1, 15);
}
static void func_bt_call_volume_pic_update(void)
{
    ...
    if(call_volume_flag)
    {
        compo_picturebox_set_visible(volume_pic, true);
        compo_shape_set_visible(shape, true);
    }
    else
    {
        compo_picturebox_set_visible(volume_pic, false);
...
void bt_incall_time_update(void)
{
    ...
    {
        sprintf(call_time_str, "%02d:%02d", minutes, seconds);
    }
//    printf("call_time_str: %s\n", call_time_str);
...
//    compo_form_set_title(frm, i18n[STR_HEART_RATE]);
    ...
    uint16_t calling_second = uteModuleCallGetCallingTimeSecond();
    if (calling_second >= 60 * 60)
    {
        sprintf(str_time, "%02d:%02d:%02d", calling_second / 3600, (calling_second % 3600) / 60, calling_second % 60);
    }
    else
    {
        sprintf(str_time, "%02d:%02d", calling_second / 60, calling_second % 60);
    }
    compo_textbox_set(time_txt, str_time);
    compo_setid(time_txt, COMPO_ID_TXT_TIME);
    compo_textbox_set_forecolor(time_txt, COLOR_WHITE);
    ...
    compo_setid(btn, COMPO_ID_BTN_REJECT);
    compo_button_set_pos(btn, GUI_SCREEN_CENTER_X, 310+48);

    //静音按钮
    btn = compo_button_create_by_image(frm, UI_BUF_I341001_11_CALL_BUTT_MICROPHONE00_BIN);
    compo_setid(btn, COMPO_ID_BTN_MIC);
    compo_button_set_pos(btn, 20+48, 310+48);

    //音量按钮
    btn = compo_button_create_by_image(frm, UI_BUF_I341001_11_CALL_BUTT_SOUND_BIN);
    compo_setid(btn, COMPO_ID_BTN_VOLUME);
    compo_button_set_pos(btn, 252+48, 310+48);
...
//    新建窗体, 呼出页面
    ...
    compo_textbox_t *txt = compo_textbox_create(frm, strlen(i18n[STR_IN_CALL]));
    compo_setid(txt, COMPO_ID_TXT_IN_CALL);
    compo_textbox_set_location(txt, GUI_SCREEN_CENTER_X, 191+widget_text_get_height()/2, GUI_SCREEN_WIDTH/1.2, widget_text_get_max_height());
    compo_textbox_set(txt, i18n[STR_IN_CALL]);
    compo_textbox_set_forecolor(txt, COLOR_WHITE);
    txt_leng = widget_text_get_area(txt->txt).wid;
    txt_x = GUI_SCREEN_CENTER_X-txt_leng/2;
...
void func_bt_call_up_date_process(void)
{
    ...
    {
        if (strlen(f_bt_call->pbap_result_Name) == 0)
        {
...
//        printf("COMPO_ID_BTN_REJECT\n");
            ...
            if(f_bt_call->call_mute_flag)
            {
                ...
                audio_path_exit(AUDIO_PATH_BTMIC);
#endif
                compo_button_set_bgimg(btn,UI_BUF_I341001_11_CALL_BUTT_MICROPHONE01_BIN);
            }
            else
            {
#if EQ_DBG_IN_UART || EQ_DBG_IN_SPP
                ...
                bt_sco_pcm_set_dump_pass_cnt(5);
#endif
                compo_button_set_bgimg(btn,UI_BUF_I341001_11_CALL_BUTT_MICROPHONE00_BIN);
            }
            break;
        case COMPO_ID_BTN_VOLUME:           //进入音量设置界面
            if(sys_cb.incall_flag)
...
static void func_bt_call_volume_handle(void)
{
    ...
    compo_picturebox_t *volume_pic = compo_getobj_byid(COMPO_ID_PIC_VOLUME);
    compo_picturebox_cut(volume_pic,sys_cb.hfp_vol >= 15 ? 16:sys_cb.hfp_vol,17);
}

static void func_bt_call_volume_pic_update(void)
{
    compo_picturebox_t *volume_pic = compo_getobj_byid(COMPO_ID_PIC_VOLUME);
    compo_picturebox_cut(volume_pic,sys_cb.hfp_vol-1,17);
    compo_shape_t *shape = compo_getobj_byid(COMPO_ID_COLOR_BLACK);
    if(call_volume_flag)
    {
        compo_listbox_set_visible(volume_pic,true);
...
//    compo_form_set_title(frm, i18n[STR_HEART_RATE]);
    ...
    compo_setid(btn, COMPO_ID_BTN_REJECT);
    compo_button_set_pos(btn, GUI_SCREEN_CENTER_X, 272+80/2);

    //静音按钮
    btn = compo_button_create_by_image(frm, UI_BUF_I343001_11_CALL_MICROPHONE_BIN);
    compo_setid(btn, COMPO_ID_BTN_MIC);
    compo_button_set_pos(btn, 222+80/2, 272+80/2);

    //音量按钮
    btn = compo_button_create_by_image(frm, UI_BUF_I343001_11_CALL_SOUND_BIN);
    compo_setid(btn, COMPO_ID_BTN_VOLUME);
    compo_button_set_pos(btn, 18+80/2, 272+80/2);
    ...
    compo_shape_set_color(shape, COLOR_BLACK);
    //音量
    compo_picturebox_t *volume_pic = compo_picturebox_create(frm, UI_BUF_I343001_11_CALL_VOLUME_BIN);
    compo_setid(volume_pic, COMPO_ID_PIC_VOLUME);
    compo_picturebox_set_pos(volume_pic, GUI_SCREEN_CENTER_X, GUI_SCREEN_CENTER_Y);
    compo_picturebox_cut(volume_pic,sys_cb.hfp_vol-1,17);
...
//        printf("COMPO_ID_BTN_REJECT\n");
            ...
            if(f_bt_call->call_mute_flag)
            {
                ...
                audio_path_exit(AUDIO_PATH_BTMIC);
#endif
                compo_button_set_bgimg(btn,UI_BUF_I343001_11_CALL_MICROPHONE_01_BIN);
            }
            else
            {
#if EQ_DBG_IN_UART || EQ_DBG_IN_SPP
                ...
                bt_sco_pcm_set_dump_pass_cnt(5);
#endif
                compo_button_set_bgimg(btn,UI_BUF_I343001_11_CALL_MICROPHONE_BIN);
            }
            break;
        case COMPO_ID_BTN_VOLUME:           //进入音量设置界面
            printf("COMPO_ID_BTN_VOLUME %d\n", sys_cb.incall_flag);
            if(sys_cb.incall_flag)
            {
                call_volume_flag = true;
                func_bt_call_volume_pic_update();
...
//                bt_call_terminate();                        //挂断当前通话
...
#else
#if UTE_MODULE_CALL_ALLOW_SCREEN_OFF_SUPPORT
            if (bt_cb.disp_status < BT_STA_INCALL)
#endif
            {
    ...
    || GUI_SCREEN_SIZE_368X448RGB_I345001_SUPPORT || GUI_SCREEN_SIZE_320X380RGB_I343001_SUPPORT
        ...
        case MSG_CTP_SHORT_LEFT:
            break;
#if GUI_SCREEN_SIZE_240X284RGB_I335001_SUPPORT || GUI_SCREEN_SIZE_240X240RGB_I342001_SUPPORT \
    || GUI_SCREEN_SIZE_368X448RGB_I345001_SUPPORT || GUI_SCREEN_SIZE_360X360RGB_I340001_SUPPORT \
