...
static int vc30fx_spo2_hr_calculate(vcare_ppg_device_t *pdev)
{
    AlgoInputData_t algoInputData_t = {0};
    AlgoOutputData_t algoOutputData = {0};
    StressOutputData_t StressData;
    ...
    for (int i = 0; i < result_ir->ppg_num; i++)
    {
        ...
        algoInputData_t.axes.y = vc30fx_gsensor_data.yData[i];
        algoInputData_t.axes.z = vc30fx_gsensor_data.zData[i];
        algoInputData_t.envSample = result_red->env_data;
        algoInputData_t.ppgSample = result_ir->ppg_buffer[i];
        /*跟手表重量相关，手表重量超过36g，才写1.Casen modify 23-04-27*/
        Algo_Input( &algoInputData_t, 1000/25, 0, 0, 0);
        VCARE_DBG_LOG("[%d] ppg_ir=%d, ppg_red=%d, x=%d, y=%d, z=%d", i, result_ir->ppg_buffer[i], result_red->ppg_buffer[i],
                      vc30fx_gsensor_data.xData[i], vc30fx_gsensor_data.yData[i], vc30fx_gsensor_data.zData[i]);
    }
    /************************************添加一键测量功能起始位置 2/2**************************************/

    //if(vc921x_dev.workmode != WORK_MODE_SPO2_TIMER) //此处if语句可修改为，当模式为 一键测量模式 时，才进入
    //{
    Algo_Output( &algoOutputData );
    vc30fx_dev.heart_rate = algoOutputData.hrData;  //这是你要的一键测量的心率值
    printf("heart_rate = %d\n",vc30fx_dev.heart_rate);
    //===================STRESS============//
    StressEst_Input(algoOutputData.hrData, algoOutputData.reliability, algoInputData_t.axes.x, algoInputData_t.ppgSample);
    StressEst_Output(&StressData);
    printf("StressData.errType = %d\n",StressData.errType);
    if(StressData.errType == 0)
    {
        vc30fx_dev.stress = StressData.strData + (vc30fx_dev.heart_rate / 3);  //这是你要的一键测量的压力值
    }
    ...
    switch (vc30fx_dev.workmode)
    {
        ...
        case WORK_MODE_HRSPO2:
            if (vc30fx_drv_is_fifo_event(&vc30fx_dev))
            {
                ...
                {
#if UTE_MODULE_ONE_CLICK_MEASURE_SUPPORT
                    vc30fx_dev.spo2 = vc30fx_spo2_hr_calculate(&vc30fx_dev);
                    uteModuleOneClickMeasureSetTestValue(vc30fx_dev.heart_rate, vc30fx_dev.spo2, vc30fx_dev.stress);
#endif
                    uteModuleSportInputDataBeforeAlgo();
                }
                else
        ...
        case WORK_MODE_FACTORY0:
            ...
            else if (uteModuleFactoryTestGetCheckLightMode() == FACTORY_VCXX_TEST_MODE_BIO_A)
            {
                uteModuleFactoryTestSetVkBioData(sample_result_info_ptr->extra_result.bioext_data, true);
            }
            else if (uteModuleFactoryTestGetCheckLightMode() == FACTORY_VCXX_TEST_MODE_BIO_B)
            {
                uteModuleFactoryTestSetVkBioData(sample_result_info_ptr->extra_result.bioext_data, false);
            }
#endif
            else
            {
            ...
            // sample_result_info_ptr->extra_result.bioext_data; //实时活体值 标定是用 对空状态 - 导电布状态
            break;
